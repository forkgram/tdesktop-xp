...
if (NOT TGVOIP_FOUND)
    ...
    if (WIN32)
        ...
        # XP walk: WaveOut audio only (WASAPI is Vista+) + XP IP_ADAPTER_ADDRESSES
        # layout. The two WASAPI .cpp sources are dropped from the list below.
        target_compile_definitions(lib_tgvoip_bundled PRIVATE TGVOIP_WINXP_COMPAT)
    elseif (LINUX)
    ...
    if (WIN32)
        # XP walk: external_webrtc (dropped on XP) used to provide the WebRTC
        # platform defines the bundled webrtc_dsp needs -- rtc_base/platform_file.h
        # #errors "Unsupported platform" without WEBRTC_WIN. Re-add them here.
        target_compile_definitions(lib_tgvoip_bundled
        PRIVATE
            WEBRTC_WIN
            WEBRTC_ENABLE_PROTOBUF=0
            # XP walk: bundled-webrtc_dsp defines that v2.5.9 dropped (it expected
            # the external tg_owt WebRTC). TGVOIP_USE_DESKTOP_DSP_BUNDLED makes
            # EchoCanceller use the bundled (older) NoiseSuppression API instead of
            # the tg_owt AudioProcessing::Config::NoiseSuppression one.
            TGVOIP_USE_DESKTOP_DSP_BUNDLED
            WEBRTC_APM_DEBUG_DUMP=0
            WEBRTC_NS_FLOAT
        )

        # XP walk: v2.5.9 made WebRTC mandatory and dropped the bundled webrtc_dsp
        # source list from lib_tgvoip (expecting external tg_owt). Re-add it so the
        # bundled DSP (Config/AudioProcessingBuilder/AudioFrame/...) is compiled.
        nice_target_sources(lib_tgvoip_bundled ${tgvoip_loc}
        PRIVATE
            # WebRTC APM
    ...
    PRIVATE
        ...
        # not the external tg_owt WebRTC (absent on XP) -- linking external_webrtc
        # would validate its non-existent tg_owt/src include path at generate time.
