	...
	for (const auto iconSize : _iconSizes) {
		...
		result.addPixmap(Ui::PixmapFromImage(_new.counter > 0
			? Window::WithSmallCounter(std::move(currentImageBack), {
				// XP walk: designated -> positional (C7555). CounterLayerArgs:
				// size, devicePixelRatio(=1.), count, bg, fg.
				iconSize, // size
				1., // devicePixelRatio (struct default; {} would be 0.)
				_new.counter, // count
				_new.muted ? st::trayCounterBgMute : st::trayCounterBg, // bg
				st::trayCounterFg, // fg
