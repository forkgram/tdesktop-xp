...
void Manager::Private::showNotification(
		...
		const QString &msg,
		DisplayOptions options) {
	const auto key = FullPeer{ peer->session().uniqueId(), peer->id };
	const auto notificationId = NotificationId{ key, msgId };
...
void Manager::Private::clearFromItem(not_null<HistoryItem*> item) {
	const auto key = FullPeer{ item->history()->session().uniqueId(), item->history()->peer->id };
...
void Manager::Private::clearFromHistory(not_null<History*> history) {
	const auto key = FullPeer{ history->session().uniqueId(), history->peer->id };
	const auto i = _notifications.find(key);
	if (i != _notifications.cend()) {
