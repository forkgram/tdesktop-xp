...
void OverlayWidget::showPremiumDownloadPromo() {
	...
	// Theirs sets text + filter + adaptive + duration.
	auto config = Ui::Toast::Config();
	config.text = tr::lng_stories_save_promo(
		tr::now,
		lt_link,
		Ui::Text::Link(
			Ui::Text::Bold(
	...
	config.duration = kStorySavePromoDuration;
...
OverlayWidget::ContentGeometry OverlayWidget::storiesContentGeometry(
		...
		float64 scale) const {
	return {
		QRectF(layout.geometry), // rect
		{}, // rotation
		kStoriesControlsOpacity, // controlsOpacity
		layout.fade, // fade
		scale, // scale
		{}, // bottomShadowSkip
		layout.radius, // roundRadius
		!layout.headerOutside, // topShadowShown
	};
}
			...
			if (CanCreateModerateMessagesBox(list)) {
				...
				window->show(
					Box(
						...
						ModerateMessagesBoxEntry{ list }, // items
						nullptr,
						opt),
					Ui::LayerOption::CloseOther);
			} else {
...
void OverlayWidget::refreshTimestampDividers(
	...
	for (const auto &entity : caption.entities) {
		...
		timestamps.push_back({
			float64(time) / duration, // position
		...
		if (_document->sticker()) {
			if (const auto image = _documentMedia->getStickerLarge()) {
				...
				setStaticContent(thumbnail->pix(
					...
					{ nullptr, Images::Option::Blur } // colored, options
				).toImage());
			}
		} else {
			initSponsoredButton();
			...
			if (_themePreview) {
				...
				if (const auto slug = _themeCloudData.slug; !slug.isEmpty()) {
					...
					_themeShare->setClickedCallback([=] {
						...
						uiShow()->showToast(std::move(toast));
					});
				} else {
					_themeShare.destroy();
				}
...
void OverlayWidget::restartAtSeekPosition(crl::time position) {
	...
	auto options = Streaming::PlaybackOptions{
		...
		((overrideDuration
			...
			? _document->duration()
			: crl::time(0)), // durationOverride
		1., // speed
		{}, // audioId
		true, // syncVideoByAudio
		{}, // waitForMarkAsShown
		Core::App().settings().hardwareAcceleratedVideo(), // hwAllowed
		!_stories, // seekable
	};
	if (!_streamed->withSound) {
...
void OverlayWidget::refreshSystemMediaControls() {
	...
	auto video = SystemMediaControlsVideoSink::VideoState{
		title, // title
		channel ? filename : _fromName, // artist
		std::max(state.position, int64(0)), // position
		std::max(state.length, int64(0)), // duration
		(!player.paused() && !player.finished()), // playing
		_rightNavVisible, // nextAvailable
		_leftNavVisible, // previousAvailable
	};
...
void OverlayWidget::validatePhotoImage(Image *image, bool blurred) {
	...
	setStaticContent(image->pixNoCache(
		use,
		{ nullptr, (blurred ? Images::Option::Blur : Images::Option()) } // colored, options
	).toImage());
	_blurred = blurred;
}
...
void OverlayWidget::tryStartTextRecognition() {
	...
	const auto id = RecognitionId{
		...
		forPhoto ? _photo->id : PhotoId(), // photoId
		forDocument ? _document->id : DocumentId(), // documentId
	};
...
void OverlayWidget::paintCaptionContent(
	...
	if (inner.intersects(clip)) {
		...
		_caption.draw(p, {
			...
			{}, // clip
			&st::mediaviewTextPalette, // palette
			&_captionPreCache, // pre -- XP walk: v6.9.4 wires quote caches
			&_captionBlockquoteCache, // blockquote -- XP walk: v6.9.4 wires quote caches
			{}, // colors
...
void OverlayWidget::setupSwipeNavigation() {
	...
	Ui::Controls::SetupSwipeHandler({
		_body, // widget
		{}, // scroll -- HIDDEN GAP: designated init skipped scroll (field 1)
