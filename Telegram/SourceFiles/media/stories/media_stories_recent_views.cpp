...
void RecentViews::addMenuRow(Data::StoryView entry, const QDateTime &now) {
	...
	const auto prepare = [&](Ui::PeerUserpicView &view) {
		...
		return Ui::WhoReactedEntryData{
			// XP walk: designated -> positional (C7555). v4.13.0 changed
			// WhoReactedEntryData: dropped dateReacted+preloader, added `type`.
			// Fields: text, date, type, customEntityData, userpic, callback.
			peer->name(), // text
			status, // date
			type, // type
	...
	if (_menuPlaceholderCount > 0) {
		...
		_menuEntries.push_back({
			// XP walk: designated -> positional (C7555). v4.13.0 MenuEntry replaced
			// `date` with type+status. Fields: action, peer, type, status,
			// customEntityData, callback, view, key.
			raw, // action
			peer, // peer
			type, // type
			status, // status
...
void RecentViews::addMenuRowPlaceholder(not_null<Main::Session*> session) {
	auto action = base::make_unique_q<Ui::WhoReactedEntryAction>(
		...
		_menu->menu()->st(),
		// XP walk: designated -> positional (C7555). WhoReactedEntryData fields:
		// text, date, type; Preloader placeholder sets type only.
		Ui::WhoReactedEntryData{ {}, {}, Ui::WhoReactedType::Preloader }); // text, date, type
	const auto raw = action.get();
	_menu->addAction(std::move(action));
	_menuEntries.push_back({ raw }); // action
		...
		for (auto i = begin(_waitingForUserpics)
			...
			if (update) {
				...
				entry.action->setData({
					// XP walk: designated -> positional (C7555). WhoReactedEntryData:
					// text, date, type, customEntityData, userpic, callback.
					peer->name(), // text
					entry.status, // date
					entry.type, // type
