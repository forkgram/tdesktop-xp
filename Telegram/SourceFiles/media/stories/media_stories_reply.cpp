...
bool ReplyArea::send(
	...
	// topicRootId, forward, story, text, messagesCount, ignoreSlowmodeCountdown.
	auto request = SendingErrorRequest();
	request.topicRootId = MsgId(0);
	request.text = &message.textWithTags;
	request.ignoreSlowmodeCountdown = (message.action.options.scheduled != 0);
	...
	return _data.peer
		&& _sendPayment.check(
			...
			std::move(withPaymentApproved),
			{
				...
				(st3 ? st3->chooseDateTimeArgs.labelStyle : nullptr), // label
				(st2 ? st2->checkbox : nullptr), // checkbox
			});
}
...
Api::SendAction ReplyArea::prepareSendAction(
	...
	result.replyTo.storyId = { _data.peer->id, _data.id };
	return result;
}
