...
void Controller::PhotoPlayback::callback() {
	...
	_controller->updatePhotoPlayback({
		...
		state, // state
		elapsed, // position
		kPhotoDuration, // receivedTill
		kPhotoDuration, // length
		1000, // frequency
	});
}
...
, _recentViews(std::make_unique<RecentViews>(this)) {
	...
	_reactions->chosen(
		...
		startReactionAnimation(id.id, {
			...
			{}, // localId
			id.globalGeometry, // globalStartGeometry
			id.icon, // frame
		});
...
void Controller::initLayout() {
	...
	_layout = _wrap->sizeValue(
		...
		layout.siblingLeft = {
			left, // geometry
			userpic(left), // userpic
			nameBoundingRect(left, true), // nameBoundingRect
			nameFontSize, // nameFontSize
		};
		layout.siblingRight = {
			right, // geometry
			userpic(right), // userpic
			nameBoundingRect(right, false), // nameBoundingRect
			nameFontSize, // nameFontSize
		};
...
void Controller::preloadNext() {
	...
	for (auto i = _index + 1; i != till; ++i) {
		ids.push_back({ user->id, shownId(i) }); // peer, story
	}
...
void Controller::show(
	...
	_header->show({
		...
		_sliderCount ? shownCount() : 0, // fullCount
	...
	_session->changes().storyUpdates(
		...
		if (update.flags & Data::StoryUpdate::Flag::Edited) {
			...
			_recentViews->show({
				...
				update.story->peer()->isSelf(), // valid
			});
		}
	}, _sessionLifetime);
...
ViewsSlice Controller::views(PeerId offset) {
	...
	if (!offset) {
		...
		return { {}, _viewsSlice.left }; // list, left
	}
	return _viewsSlice;
}
	...
	return crl::guard(&_viewsLoadGuard, [=](
		...
		if (_viewsSlice.list.empty()) {
			...
			if (const auto maybeStory = stories.lookup(_shown)) {
				_viewsSlice = {
					result, // list
					(*maybeStory)->views() - int(result.size()), // left
				};
			} else {
				_viewsSlice = {};
			}
...
void Controller::deleteRequested() {
	...
	uiShow()->show(Ui::MakeConfirmBox({
		tr::lng_stories_delete_one_sure(), // text
		confirmed, // confirmed
		v::null, // cancelled
		tr::lng_selected_delete(), // confirmText
		{}, // cancelText
		{}, // confirmStyle
		{}, // cancelStyle
		&st::storiesBoxLabel, // labelStyle
	}));
}
...
void Controller::startReactionAnimation(
	...
	auto args = Ui::ReactionFlyAnimationArgs{
		id, // id
		from.frame, // flyIcon
		_wrap->mapFromGlobal(from.globalStartGeometry), // flyFrom
		st::fadeWrapDuration * 2, // scaleOutDuration
	};
