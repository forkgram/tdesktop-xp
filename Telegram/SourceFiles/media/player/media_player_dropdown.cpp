...
void Dropdown::paintEvent(QPaintEvent *e) {
	...
	const auto fill = Ui::CornersPixmaps{
		{ QPixmap(), QPixmap(), corners.p[2], corners.p[3] },
	};
	Ui::FillRoundRect(
		p,
		shadowedRect.x(),
	...
	const auto addOrderAction = [&](OrderMode mode) {
		...
		const auto fields = [&]() -> Fields {
			...
			case OrderMode::Reverse: return {
				tr::lng_audio_player_reverse(tr::now), // label
				st::mediaPlayerOrderIconReverse, // icon
				st::mediaPlayerOrderIconReverseActive, // activeIcon
			};
			case OrderMode::Shuffle: return {
				tr::lng_audio_player_shuffle(tr::now), // label
				st::mediaPlayerOrderIconShuffle, // icon
				st::mediaPlayerOrderIconShuffleActive, // activeIcon
			};
			}
