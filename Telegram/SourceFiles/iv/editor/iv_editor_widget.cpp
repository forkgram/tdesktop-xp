...
/*XP:thread_local pulls 14.44 CRT TLS helpers absent in 14.16; UI is main-thread*/ static Widget *PreservingExternalFieldRestore = nullptr;
...
[[nodiscard]] SplitCommittedFieldOffset SplitCommittedFieldOffsetAt(
	...
	for (auto i = 0, count = int(chunks.size()); i != count; ++i) {
		...
		if ((offset <= consumed + size) || (i + 1 == count)) {
			return {
				i, // chunkIndex
				std::clamp(offset - consumed, 0, size), // localOffset
			};
		}
		consumed += size;
	}
	...
	if (capture.leaf.kind == StateLeafKind::BlockText) {
		...
		if (!block) {
			...
			return StateLeafPath{
				StateLeafKind::BlockText, // kind
				{
					capture.leaf.block.container, // container
					capture.leaf.block.index + chunkIndex, // index
				}, // block
			};
			...
			return StateLeafPath{
				StateLeafKind::BlockText, // kind
				{
					BlockChildrenContainer(capture.leaf.block), // container
					chunkIndex, // index
				}, // block
			};
		}
		...
		return StateLeafPath{
			StateLeafKind::BlockText, // kind
			{
				ListItemChildrenContainer(
					capture.leaf.block,
					capture.leaf.listItemIndex), // container
				chunkIndex, // index
			}, // block
		};
	}
...
MapCommittedFieldSelectionAfterCommit(
	...
	const auto fallback = [&](int anchor, int cursor)
		...
		return CommittedFieldSelectionRestore{
			ordinal, // ordinal
			std::clamp(anchor, 0, length), // anchorOffset
			std::clamp(cursor, 0, length), // cursorOffset
		};
	};
	...
	if (chunks.size() <= 1) {
		...
		if (ordinal >= 0) {
			...
			return CommittedFieldSelectionRestore{
				ordinal, // ordinal
				std::clamp(anchorOffset, 0, length), // anchorOffset
				std::clamp(cursorOffset, 0, length), // cursorOffset
			};
		}
	...
	if (anchor.chunkIndex >= 0
		...
		if (const auto leaf = SplitCommittedFieldLeafAt(
			...
			if (ordinal >= 0 && rich) {
				...
				return CommittedFieldSelectionRestore{
					ordinal, // ordinal
					std::clamp(anchor.localOffset, 0, length), // anchorOffset
					std::clamp(cursor.localOffset, 0, length), // cursorOffset
				};
			}
		}
	}
...
[[nodiscard]] StateBlockPath ToStateBlockPath(
	...
	return {
		ToStateBlockContainerPath(path.container), // container
		path.index, // index
	};
}
...
[[nodiscard]] StateBlockContainerPath BlockChildrenContainer(
	...
	result.steps.push_back({
		StateBlockContainerKind::BlockChildren, // kind
...
[[nodiscard]] StateBlockContainerPath ListItemChildrenContainer(
	...
	auto result = std::move(path.container);
	result.steps.push_back({
		StateBlockContainerKind::ListItemChildren, // kind
		path.index, // blockIndex
	...
	if (action == ToolbarFormatAction::PlainText) {
		...
		if (!_settingField
			...
			if (owner && (owner->kind == RichPage::BlockKind::Heading)) {
				insertBlock({
					State::InsertBlockType::Heading, // type
					owner->headingLevel, // headingLevel
				});
				return;
			} else if (owner && (owner->kind == RichPage::BlockKind::Footer)) {
				insertBlock({ State::InsertBlockType::Footer }); // type
				return;
			}
		}
	}
