...
void PaintTextLeaf(
	...
	const auto makeContext = [&] {
		auto result = Ui::Text::PaintContext();
		...
		result.palette = &p.textPalette();
		result.pre = context.caches.pre;
		result.blockquote = context.caches.blockquote;
		result.colors = context.caches.colors;
		result.spoiler = Ui::Text::DefaultSpoilerCache();
		result.now = context.now;
		result.elisionLines = elisionLines;
		return result;
	};
