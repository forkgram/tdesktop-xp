...
void PaintTextLeaf(
	...
	if (context.reveal && !elisionLines) {
		...
		if (const auto articlePostprocess = context.reveal->postprocess) {
			...
			linePostprocess.emplace(Ui::Text::LinePostprocess{
				...
				articlePostprocess->cache, // cache
			});
		}
	}
	p.save();
	...
	paintContext.position = rect.topLeft();
	paintContext.availableWidth = availableWidth;
	paintContext.geometry = elisionLines
		? Ui::Text::SimpleGeometry(availableWidth, elisionLines, 0, true)
		: TextGeometry(availableWidth);
	paintContext.align = align;
	paintContext.clip = context.clip;
	paintContext.palette = &p.textPalette();
	paintContext.pre = context.caches.pre;
	paintContext.blockquote = context.caches.blockquote;
	paintContext.colors = context.caches.colors;
	paintContext.spoiler = Ui::Text::DefaultSpoilerCache();
	paintContext.now = context.now;
	paintContext.selection = selection.value_or(TextSelection());
	paintContext.elisionLines = elisionLines;
	paintContext.linePostprocess = linePostprocess ? &*linePostprocess : nullptr;
	leaf.draw(p, paintContext);
	p.restore();
}
