...
// AppleClang :/
	// XP walk: operator<=> + std::strong_ordering + defaulted == are C++20; convert to
	// manual ==/!=/</>/<=/>= (compare _whole, then _nano, then _ton only when non-zero,
	// matching upstream's <=>: a bare-zero amount is currency-agnostic).
	[[nodiscard]] friend inline constexpr bool operator==(CreditsAmount a, CreditsAmount b) {
		return (a._whole == b._whole)
			&& (a._nano == b._nano)
			&& ((!a._whole && !a._nano) || (a._ton == b._ton));
	}
	[[nodiscard]] friend inline constexpr bool operator!=(CreditsAmount a, CreditsAmount b) {
		return !(a == b);
	}
	[[nodiscard]] friend inline constexpr bool operator<(CreditsAmount a, CreditsAmount b) {
		if (a._whole != b._whole) {
			return a._whole < b._whole;
		} else if (a._nano != b._nano) {
			return a._nano < b._nano;
		}
		return (a._whole || a._nano) ? (int(a._ton) < int(b._ton)) : false;
	}
	[[nodiscard]] friend inline constexpr bool operator>(CreditsAmount a, CreditsAmount b) {
		return b < a;
	}
