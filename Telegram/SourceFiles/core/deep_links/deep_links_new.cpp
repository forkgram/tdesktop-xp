		...
		result.match([&](const MTPDcontacts_resolvedPeer &data) {
			...
			if (const auto managerBot = strong->session().data().userLoaded(
				...
				ShowCreateManagedBotBox({
					...
					true, // viaDeeplink
					[weak, managerBot](not_null<UserData*> createdBot) { // done
						if (const auto strong = weak.get()) {
							strong->showPeerHistory(createdBot);
							auto config = Ui::Toast::Config();
							config.title = tr::lng_managed_bot_created_title(
								tr::now,
								lt_name,
								createdBot->name());
							config.text = { tr::lng_managed_bot_created_text(
								tr::now,
								lt_parent_name,
								managerBot->name()) };
