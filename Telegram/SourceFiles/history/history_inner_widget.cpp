...
Ui::ChatPaintContext HistoryInner::preparePaintContext(
	...
	return _controller->preparePaintContext({
		...
		_visibleAreaBottom - _visibleAreaTop, // visibleAreaHeight
	});
}
					...
					Menu::AddTimecodeAction(
						...
						*t,
						...
						[=] {
							...
							_widget->replyToMessage({ msgId }); // FullReplyTo.messageId@0
...
void HistoryInner::setupThanosEffect() {
	...
	_thanosController = std::make_unique<Ui::ThanosEffectController>(
		...
		Ui::ThanosEffectController::Delegate{ // XP walk: designated -> positional (C7555)
			...
			[=] { return _scroll->scrollTop(); }, // scrollTop
			[=] { return _scroll->scrollTopMax(); }, // scrollTopMax
			[=]() -> not_null<QWidget*> { // scrollWidget
				return _scroll;
			},
			[=](int y) { // scrollToY
...
void HistoryInner::elementOpenPhoto(
	...
	_controller->openPhoto(photo, { context, {}, {}, draw }); // id, topicRootId, monoforumPeerId, showDrawButton
}
...
void HistoryInner::elementOpenDocument(
	...
	_controller->openDocument(
		document,
		showInMediaView,
		{ context, {}, {}, showDrawButton }); // id, topicRootId, monoforumPeerId, showDrawButton
}
...
void HistoryInner::changeAccessibilitySelection(
	...
	accessibilityChildStateChanged(
		index,
		{ {}, {}, {}, {}, {}, {}, {}, true }); // XP walk: AccessibilityState.selected@7
	accessibilityChildNameChanged(index);
}
...
void HistoryInner::deleteAsGroup(FullMsgId itemId) {
	if (const auto item = session().data().message(itemId)) {
		...
		if (!group) {
			...
			_controller->show(Box(
				CreateModerateMessagesBox,
				ModerateMessagesBoxEntry{ group->items }, // items
				nullptr,
...
Ui::AccessibilityState HistoryInner::accessibilityState() const {
	...
	return { {}, {}, true, true }; // extSelectable@2, multiSelectable@3
}
