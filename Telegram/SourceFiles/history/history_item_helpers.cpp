...
void ShowSendPaidConfirm(
	...
	const auto check = [=] {
		...
		MaybeRequestBalanceIncrease(
			show,
			required,
			// XP walk: take theirs (suggestStarsPrice path). Wrap both ternary branches
			// in SmallBalanceSource (variant `using variant::variant` -> MSVC 14.16 needs
			// a common type; implicit alt->variant in a ?: is unreliable).
			(suggestStarsPrice
				? SmallBalanceSource(SmallBalanceForSuggest{ singlePeerId })
				: SmallBalanceSource(SmallBalanceForMessage{ singlePeerId })),
			done);
	};
	auto usersOnly = true;
...
void ConfirmDeleteSelectedEphemeral(
	...
	// XP walk: designated -> named-local (C7555); ConfirmBoxArgs is large and
	// non-contiguous here (cancelled/cancelText skipped).
	auto args = Ui::ConfirmBoxArgs();
	args.text = tr::lng_selected_delete_sure(tr::now, lt_count, count);
...
not_null<HistoryItem*> GenerateJoinedMessage(
	...
	return history->makeMessage({
		...
		MessageFlag::Local | MessageFlag::ShowSimilarChannels, // flags
		{}, // from
		{}, // replyTo
		inviteDate, // date
...
void CheckPollVoteNotificationSchedule(
	...
	for (const auto &answer : poll->answers) {
		for (const auto &voter : answer.recentVoters) {
			...
			const auto notification = Data::ItemNotification{
				item, // item
				user, // reactionOrVoteSender
				Data::ItemNotificationType::PollVote, // type
			};
			item->notificationThread()->pushNotification(notification);
			Core::App().notifications().schedule(notification);
			return;
