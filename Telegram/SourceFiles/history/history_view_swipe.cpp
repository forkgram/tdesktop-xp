	...
	const auto filter = [=](not_null<QEvent*> e) {
		...
		case QEvent::TouchCancel: {
			...
			if (cancel) {
				...
				// XP walk: designated -> named-local (C7555). UpdateArgs:
				// globalCursor, position, delta, touch.
				auto args = UpdateArgs();
				args.globalCursor = (touchscreen
					? touches[0].screenPos().toPoint()
					: QCursor::pos());
				args.position = touches[0].pos();
				args.delta = state->startAt - touches[0].pos();
				args.touch = true;
#ifdef Q_OS_MAC
