...
Ui::ChatPaintHighlight ElementHighlighter::AnimationManager::state() const {
	if (anim::Disabled()) {
		return {
			// XP walk: designated -> positional (C7555). ChatPaintHighlight: opacity, collapsion, range.
			!_timer ? 0. : 1., // opacity
			!_timer ? 0. : _fadingOut ? 1. : 0., // collapsion
		};
	}
	return {
		// XP walk: designated -> positional (C7555).
		((!_fadingOut && _collapsing)
			? 1.
			: _simple.value(_fadingOut ? 0. : 1.)), // opacity
		((!_withTextPart || !_collapsing)
			? 0.
			: _fadingOut
			? 1.
			: _simple.value(1.)), // collapsion
	};
}
