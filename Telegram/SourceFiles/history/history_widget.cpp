			...
			Ui::PostponeCall(_creatingBotTopic, [=] {
				...
				controller->showSection(
					std::make_shared<ChatMemento>(ChatViewId{
						topic->owningHistory(), // history
						topic->rootId(), // repliesRootId
					}),
...
Dialogs::EntryState HistoryWidget::computeDialogsEntryState() const {
	...
	result.currentSuggest = suggestOptions();
	return result;
}
				...
				if (result.open) {
					...
					if (const auto photo = request.photo()) {
						controller()->openPhoto(
							photo,
							{ {}, {}, {}, showDrawButton }); // id, topicRootId, monoforumPeerId, showDrawButton
...
void HistoryWidget::showAiComposeBox() {
	...
	HistoryView::Controls::ShowComposeAiBox(controller()->uiShow(), {
		&session(), // session
		text, // text
		_fieldChatStyle, // chatStyle
		crl::guard(this, [=](const TextWithEntities &result) {
...
void HistoryWidget::sendTextWithTags(
	...
	if (hasText
		...
		controller()->sendingAnimation().appendSending({
			...
			_field->mapToGlobal(Rect(_field->size())), // globalStartGeometry
		});
	}
...
void HistoryWidget::updateSendButtonType() {
	...
	const auto messages = !_peer
		...
		: ComputeSendingMessagesCount(_history, {
			// XP walk: designated -> positional (C7555); gap-fill topicRootId@0,
			// story@2, messagesCount@4, ignoreSlowmodeCountdown@5.
			{}, // topicRootId
			&_forwardPanel->items(), // forward
			{}, // story
			richMessage ? nullptr : &_field->getTextWithTags(), // text
			{}, // messagesCount
			{}, // ignoreSlowmodeCountdown
...
bool HistoryWidget::showSendMessageError(
	...
	// XP walk: take theirs (adds messagesCount + error toast). Designated -> named-local
	// (C7555); SendingErrorRequest fields non-contiguous.
...
Data::SendError HistoryWidget::computeSendRestriction() const {
	if (!_canSendMessages
		...
		return Data::SendError({
			tr::lng_monoforum_choose_to_reply(tr::now), // text
			0, // boostsToLift -- XP walk: designated -> positional gap-fill
			true, // monoforumAdmin
		});
	}
		...
		if (timestamp.has_value()) {
			...
			if (document && document->isVideoFile()) {
				...
				controller()->openDocument(
					document,
					true,
					{ item->fullId(), {}, {}, draw }, // id, topicRootId, monoforumPeerId, showDrawButton
					nullptr,
					timestamp);
			}
		}
	...
	const auto checkState = [=] { // XP walk: dropped explicit this capture (C3791)
		...
		if (state == State::AppendToTopBar) {
			...
			if (checkSponsoredMessageBarVisibility()) {
				...
				_list->heightValue(
				) | rpl::on_next([=] { // XP walk: dropped explicit this capture (C3791)
...
void HistoryWidget::replyToMessage(
	...
	// XP walk: took theirs (uses the pre-existing `fields` FullReplyTo local).
