...
Data::ResolvedForwardDraft History::resolveForwardDraft(
	...
	if (result.items.size() != draft.ids.size()) {
		// XP walk: take theirs (monoforumPeerId param); designated -> positional.
		setForwardDraft(topicRootId, monoforumPeerId, {
			owner().itemsToIds(result.items),
			result.options,
		});
	}
	return result;
	...
	data.vaction().match([&](const MTPDmessageActionChatAddUser &data) {
		...
		if (const auto payment = item->Get<HistoryServicePayment>()) {
			...
			if (paid) {
				...
				// XP walk: take theirs (Core::MarkedTextContext -> Core::TextContext).
				// Ui::Toast::Config has a move-only member -> named-local; the
				// TextContextArgs designated init -> positional (session@0).
				auto config = Ui::Toast::Config();
				config.text = tr::lng_payments_success(
					tr::now,
					lt_amount,
					Ui::Text::Wrapped(
						payment->amount,
...
void History::newItemAdded(not_null<HistoryItem*> item) {
	...
	auto notification = Data::ItemNotification{ item, {}, Data::ItemNotificationType::Message };
...
void History::checkNewPeerMessages() {
	...
	const auto add = [&](tr::phrase<lngtag_when> phrase, TimeId date) {
		const auto result = makeMessage({
			// XP walk: designated -> positional (C7555); gap-fill from@2, replyTo@3.
			owner().nextLocalMessageId(), // id
			MessageFlag::Local | MessageFlag::HideDisplayDate, // flags
			{}, // from
			{}, // replyTo
			(--firstDate), // date
