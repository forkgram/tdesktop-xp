...
: HistoryItem(
	history,
	{ id, localFlags }, // XP walk: designated -> positional (C7555): id, flags
...
: HistoryItem(history, fields) {
	...
	args.spoiler = fields.mediaSpoiler;
...
void HistoryItem::updateServiceDependent(bool force) {
	...
	if (!dependent->lnk) {
		...
		dependent->lnk = JumpToMessageClickHandler(
			...
			fullId(),
			// XP walk: designated -> positional (C7555).
			// MessageHighlightId: quote, quoteOffset, todoItemId, pollOption.
			{ {}, 0, todoItemId, pollOption });
	}
...
void HistoryItem::applyEditionToHistoryCleared() {
	...
	action.replyTo = FullReplyTo{
		// XP walk: designated -> positional (C7555).
		FullMsgId(_history->peer->id, topicPost ? rootId : 0), // messageId
		{}, // quote
		{}, // storyId
		rootId, // topicRootId
	};
	...
	if (const auto reply = Get<HistoryMessageReply>()) {
		if (reply->storyId()) {
			const auto peerId = reply->externalPeerId()
				? reply->externalPeerId()
				: _history->peer->id;
			// XP walk: designated -> positional (C7555); .story via storyId()
			return { peerId, reply->storyId() };
		}
	}
	return {};
}
...
FullReplyTo HistoryItem::replyTo() const {
	// XP walk: take theirs (adds monoforumPeerId); FullReplyTo designated -> positional.
...
ItemPreview HistoryItem::toPreview(ToPreviewOptions options) const {
	if (isService()) {
		...
		return {
			...
			// first field. Upstream Wrapped(..., PlainLink) -> Colorized(...).
			Ui::Text::Colorized(notificationText()),
	...
	const auto sender = [&]() -> std::optional<QString> {
		if (options.hideSender || isPostHidingAuthor() || isEmpty()) {
			return {};
		// XP walk: v4.12.0 dropped the HistoryMessageSponsored sender branch
		// here (sponsored messages reworked). Took theirs.
...
void HistoryItem::setServiceMessageByAction(const MTPmessageAction &action) {
	...
	auto prepareChatCreate = [this](const MTPDmessageActionChatCreate &action) {
		...
		if (_history->peer->isMonoforum()) {
			...
			result.text = tr::lng_action_created_chat(
				...
				fromLinkText(), // Link 1.
				lt_title,
				{ qs(action.vtitle()) }, // XP walk: designated -> positional
				Ui::Text::WithEntities);
		}
		return result;
	};
	...
	auto prepareSuggestProfilePhoto = [this](const MTPDmessageActionSuggestProfilePhoto &action) {
		...
		const auto isVideo = action.vphoto().match([&](const MTPDphoto &data) {
			return (data.vvideo_sizes() != nullptr)
...
void HistoryItem::processAction(const MTPMessageAction &action) {
	action.match([&](const MTPDmessageActionChatAddUser &data) {
		...
		data.vtheme().match([](const MTPDchatTheme &) {
			...
			const auto session = &history()->session();
				if (const auto gift = Api::FromTL(session, data.vgift())) {
					// XP walk: GiftCode designated -> named-local (C7555).
					auto code = Data::GiftCode();
					code.unique = gift->unique;
					code.type = Data::GiftType::ChatTheme;
					_media = std::make_unique<Data::MediaGiftBox>(
						this,
						_from,
						std::move(code));
		...
		auto code = Data::GiftCode();
		code.slug = qs(data.vtransaction_id());
		code.channel = history()->owner().channel(
			peerToChannel(peerFromMTP(data.vboost_peer())));
		code.giveawayMsgId = data.vgiveaway_msg_id().v;
		code.count = int(data.vstars().v);
		code.type = Data::GiftType::Credits;
		code.viaGiveaway = true;
		code.unclaimed = data.is_unclaimed();
		_media = std::make_unique<Data::MediaGiftBox>(
			this,
			_from,
		...
		// XP walk: designated -> named-local (C7555; GiftCode large/non-contiguous).
		...
		fields.channelFrom = ((service && from)
				...
				: nullptr);
		fields.channelSavedId = channelSavedId;
		fields.giftPrepayUpgradeHash = qs(
				data.vprepaid_upgrade_hash().value_or_empty());
		fields.giftTitle = title;
		fields.realGiftMsgId = realGiftMsgId;
		...
		fields.starsBid = bid;
		fields.giftNum = data.vgift_num().value_or_empty();
		...
		fields.channelSavedId = channelSavedId;
		fields.realGiftMsgId = realGiftMsgId;
		fields.starsForDetailsRemove = int(
		...
		// XP walk: designated -> named-local (C7555; GiftCode large/non-contiguous).
		auto code = Data::GiftCode();
		code.count = Data::Birthday(
			fields.vday().v,
			fields.vmonth().v,
			fields.vyear().value_or_empty()).serialize();
		code.type = Data::GiftType::BirthdaySuggest;
		_media = std::make_unique<Data::MediaGiftBox>(
			this,
			_from,
			std::move(code));
		...
		if (const auto suggestion = this->Get<HistoryMessageSuggestion>()) {
			...
			auto code = Data::GiftCode();
			code.unique = suggestion->gift;
			code.type = Data::GiftType::GiftOffer;
			_media = std::make_unique<Data::MediaGiftBox>(
				this,
				_from,
				std::move(code));
...
PreparedServiceText HistoryItem::preparePollAppendAnswerText() {
	...
	return {
		tr::lng_action_poll_added_answer(
			tr::now,
			lt_from,
			fromLinkText(),
			lt_option,
...
PreparedServiceText HistoryItem::preparePollDeleteAnswerText() {
	...
	return {
		tr::lng_action_poll_deleted_answer(
			tr::now,
			lt_from,
			fromLinkText(),
			lt_option,
