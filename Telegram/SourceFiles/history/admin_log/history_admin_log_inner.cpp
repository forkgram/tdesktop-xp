	...
	_controller->uiShow()->show(Box([=](not_null<Ui::GenericBox*> box) {
		...
		box->addButton(tr::lng_settings_save(), [=] {
			...
			callback(FilterValue{
				collectFlags(), // flags @0
				(admins.size() == users.size())
					? std::nullopt
					: std::optional(users), // admins @1
			});
		});
...
void InnerWidget::expandGroupContaining(not_null<HistoryItem*> item) {
	...
	for (const auto &group : _deleteGroups) {
		...
		if (group.eventCount > 3
			&& (_expandedGroups.find(group.eventId) == _expandedGroups.end())) {
	...
	const auto finalizeGroup = [&](int endIndex) {
		if (groupEventCount > 0) {
			...
			_deleteGroups.push_back({
				...
				groupAdmin, // adminId
				groupStart, // startIndex
				endIndex, // endIndex
				groupEventCount, // eventCount
			});
...
OwnedItem InnerWidget::createGroupSummaryItem(
	...
	return OwnedItem(this, _history->makeMessage({
		...
		MessageFlag::AdminLogEntry, // flags
		peerFromUser(group.adminId), // from
		{}, // replyTo
		TimeId(date), // date
	}, std::move(message)));
}
...
Ui::ChatPaintContext InnerWidget::preparePaintContext(QRect clip) const {
	...
	return _controller->preparePaintContext({
		...
		_visibleBottom - _visibleTop, // visibleAreaHeight
	});
}
...
void InnerWidget::suggestRestrictParticipant(
...

	{
		...
		// XP walk: designated -> named-local (C7555). v6.4.0 adds report_and_ban.
		const auto addAction = Ui::Menu::CreateAddActionCallback(_menu);
		{
			auto a = Ui::Menu::MenuCallback::Args();
			a.text = tr::lng_context_ban_user(tr::now);
			a.handler = handler;
			a.icon = &st::menuIconBlockAttention;
			a.isAttention = true;
			addAction(std::move(a));
		}
		if (realId) {
			auto a = Ui::Menu::MenuCallback::Args();
			a.text = tr::lng_report_and_ban(tr::now);
			a.handler = [=, show = _controller->uiShow()] {
				Api::ReportSpam(participant, { realId });
				handler();
				show->showToast(tr::lng_report_spam_done(tr::now));
			};
			a.icon = &st::menuIconReportAttention;
			a.isAttention = true;
			addAction(std::move(a));
		}
	}
	...
	if (activated) {
		...
		ActivateClickHandler(window(), activated, {
			...
			QVariant::fromValue(ClickHandlerContext{
				// XP walk: designated -> positional (C7555); base::make_weak(this).get().
				{}, // itemId
				[weak = base::make_weak(this)] { // elementDelegate
