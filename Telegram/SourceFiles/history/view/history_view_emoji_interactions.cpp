...
bool EmojiInteractions::playPremiumEffect(
	...
	if (replacing) {
		// XP walk: ranges::find with a projection fails on v141_xp; std::find_if.
		const auto i = std::find_if(begin(_plays), end(_plays), [&](
				const Play &p) {
			return p.view == replacing;
		});
		if (i != end(_plays)) {
			if (already) {
				_plays.erase(i);
...
void EmojiInteractions::play(
	...
	_plays.push_back({
		...
		shift,
		inner,
		((type == Stickers::EffectType::PremiumSticker)
			...
			: HistoryView::Sticker::MessageEffectSize()), // outer
		0, // frame
		0, // framesCount
		0, // frameRate
		type,
