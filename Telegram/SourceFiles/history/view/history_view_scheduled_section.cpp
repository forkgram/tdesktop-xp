...
, _sentToScheduledId(sentToScheduledId) {
	...
	if (sentToScheduledId) {
		// XP walk: designated -> positional (C7555). ScrollTopState{ item@0 },
		// MessagePosition{ fullId@0 }, FullMsgId{ peer@0, msg@1 }.
		_list.setScrollTopState({ { { _history->peer->id, sentToScheduledId } } });
...
, _composeControls(std::make_unique<ComposeControls>(
	this,
	// XP walk: designated -> positional (C7555). ComposeControlsDescriptor:
	// stOverride@0(nullptr), show@1, unavailableEmojiPasted@2, mode@3,
	// sendMenuDetails@4, regularWindow@5, stickerOrEmojiChosen@6.
...
void ScheduledWidget::restoreState(not_null<ScheduledMemento*> memento) {
	...
	if (const auto id = memento->sentToScheduledId()) {
		...
		if (item) {
			...
			// non-contiguous, skips non-trivial defaults).
			auto toast = Ui::Toast::Config();
			toast.title = tr::lng_scheduled_video_tip_title(tr::now);
			toast.text = { tr::lng_scheduled_video_tip_text(tr::now) };
			toast.attach = RectPart::Top;
			toast.duration = kVideoProcessingInfoDuration;
...
void ScheduledWidget::listOpenPhoto(
	...
	controller()->openPhoto(photo, { context, {}, {}, draw }); // id, topicRootId, monoforumPeerId, showDrawButton
}
...
void ScheduledWidget::listOpenDocument(
	...
	controller()->openDocument(
		document,
		showInMediaView,
		{ context, {}, {}, draw }); // id, topicRootId, monoforumPeerId, showDrawButton
}
...
bool ShowScheduledVideoPublished(
	...
	toastConfig.padding = rpl::single(additional);
	toastConfig.st = &st;
	toastConfig.attach = RectPart::Top;
	toastConfig.acceptinput = true;
	toastConfig.duration = kVideoProcessingInfoDuration;
	const auto weak = Ui::Toast::Show(parent, std::move(toastConfig));
