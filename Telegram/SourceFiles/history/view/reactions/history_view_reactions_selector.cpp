...
Selector::ExpandingRects Selector::updateExpandingRects(float64 progress) {
	...
	_shadowSkip = (_useTransparency && categories < radius)
		...
		: 0;
	return {
		QRect(inner.x(), inner.y(), inner.width(), categories), // categories
		inner.marginsRemoved({ 0, categories, 0, 0 }), // list
		radius, // radius
		expanding, // expanding
		height() - margins.bottom(), // finalBottom
		frame, // frame
		outer, // outer
	};
}
...
void Selector::createList() {
	...
	_list = _scroll->setOwnedWidget(
		object_ptr<EmojiListWidget>(_scroll, EmojiListDescriptor{
			// XP walk: designated -> positional (C7555). EmojiListDescriptor:
			// show, mode, customTextColor, paused, customRecentList,
			// customRecentFactory, st, features. Took theirs (unified factory
			// owner list/factory for custom reactions).
			_show, // show
			_listMode, // mode
	...
	_list->customChosen(
		...
		_chosen.fire({
			// XP walk: designated -> positional (C7555). ChosenReaction: context,
			// id, icon, localGeometry, globalGeometry. Took theirs (id resolved
			// via unified factory owner).
			{}, // context
			_unifiedFactoryOwner->lookupReactionId(data.document->id), // id
			data.messageSendingFrom.frame, // icon
			{}, // localGeometry
			data.messageSendingFrom.globalStartGeometry, // globalGeometry
		});
	}, _list->lifetime());
