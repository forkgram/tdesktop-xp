...
-> ExpandingRects {
	...
	_shadowSkip = (_useTransparency && categories < radius)
		...
		: 0;
	return {
		QRect(inner.x(), inner.y(), inner.width(), categories),
		inner.marginsRemoved({ 0, categories, 0, 0 }),
		radius,
...
void Selector::createList(not_null<Window::SessionController*> controller) {
	...
	_list->customChosen(
		...
		_chosen.fire({
			{},
			reactionId,
			data.messageSendingFrom.frame,
			{},
			data.messageSendingFrom.globalStartGeometry,
		});
	}, _list->lifetime());
