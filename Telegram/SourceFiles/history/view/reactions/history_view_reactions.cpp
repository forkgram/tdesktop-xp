...
InlineList::Button InlineList::prepareButtonWithId(const ReactionId &id) {
	// XP walk: designated -> named-local (C7555). Take theirs: id + paid
	// (v5.4.0 added paid); all other Button fields keep defaults.
	auto result = Button();
	result.id = id;
	result.paid = id.paid();
	...
	for (const auto &button : _buttons) {
		...
		if (animating) {
			animations.push_back({
				button.animation.get(), // animation
				textFg.color(), // textColor
				image, // target
			});
		}
...
void InlineList::paintCustomFrame(
	...
	emoji->paint(q, {
		...
		{}, // scale
		{}, // position
		context.paused || On(PowerSaving::kEmojiChat), // paused
	});
	q.end();
