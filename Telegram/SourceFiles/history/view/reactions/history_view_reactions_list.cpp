...
base::unique_qptr<Ui::PopupMenu> Controller::rowContextMenu(
	...
	args.text = tr::lng_context_delete_this_reaction(tr::now);
	args.handler = [=] {
	...
	Ui::Menu::CreateAddActionCallback(result.get())(std::move(args));
	return result;
}
...
void ShowModerateReactionBox(
	...
	controller->show(Box(
		...
		ModerateMessagesBoxEntry{ // XP walk: designated -> positional (C7555)
			{}, // items
			ModerateReactionEntry{
				originPeer, // peer
				originMsgId, // msgId
				participant, // participant
				std::move(reaction), // reaction
			}, // reaction
		},
		nullptr,
...
not_null<Tabs*> CreateReactionsTabs(
	...
	if (whoReadIds && !whoReadIds->list.empty()) {
		map.push_back({
			Data::ReactionId{ u"read"_q }, // XP walk: designated -> positional (C7555)
...
PreparedFullList FullListController(
	...
	return { // XP walk: designated -> positional (C7555)
		std::make_unique<Controller>(
			...
			tab->events(),
			whoReadIds),
		[=](Data::ReactionId id) { tab->fire_copy(id); },
	};
}
