...
void ReadMetricsTracker::finalize(
	...
	const auto seenRange = (tracked.maxItemHeight > 0)
		? std::clamp(
			...
			1000)
		: 0;
	_peer->session().api().readMetrics().add(_peer, { // XP walk: designated -> positional (C7555)
		msgId, // msgId
		tracked.viewId, // viewId
		int(tracked.totalInView), // timeInViewMs
		int(tracked.activeInView), // activeTimeInViewMs
		heightRatio, // heightToViewportRatioPermille
		seenRange, // seenRangeRatioPermille
	});
}
