...
void BottomInfo::paintEffect(
	...
	if (_effect) {
		...
		if (animating) {
			...
			animations.push_back({ _effect->animation.get(), image });
		}
		x += width + add;
		widthLeft -= width + add;
	}
...
void BottomInfo::layoutDateText() {
	...
	auto helper = Ui::Text::CustomEmojiHelper(
		Core::TextContext({ &_reactionsOwner->session() })); // session
		...
		marked.append(
			...
			Ui::Emoji::SinglePixmap(
				...
				Ui::Emoji::GetSizeNormal()).toImage().scaledToHeight(
					...
					Qt::SmoothTransformation), // image
			QMargins(0, st::stakeIconEmojiTop, 0, 0), // margin
			false, // textColor
		})).append("  ");
	}
	marked.append(full);
