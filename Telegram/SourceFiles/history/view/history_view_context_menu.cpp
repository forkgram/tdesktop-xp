	...
	field->paintRequest() | rpl::on_next([=](QRect clip) {
		...
		if (const auto custom = state->custom.get()) {
			...
			custom->paint(p, { // XP walk: designated -> positional (C7555)
				...
				crl::now(), // now
				{}, // scale
				QPoint(0, top), // position
				inactive || On(PowerSaving::kEmojiChat), // paused
			});
		} else {
...
void FillPollOptionPage(
	...
	if (const auto addedBy = a->addedBy) {
		...
		const auto refresh = [=] {
			raw->setData({
				tr::lng_polls_option_added_by(
					tr::now,
					lt_user,
					addedBy->shortName()), // text
				date, // date
				Ui::WhoReactedType::RefRecipient, // type
				{}, // customEntityData
				thumbnail->image(size), // userpic
				[=] { controller->showPeerInfo(addedBy); }, // callback
			});
		};
...
void AddPollActions(
		...
		if (!Ui::SkipTranslate(text)) {
			menu->addAction(tr::lng_context_translate(tr::now), [=] {
				controller->show(Box(
					...
					item->history()->peer,
					MsgId(),
					// XP walk: poll->question is now TextWithEntities; pass directly.
					std::move(text),
					item->forbidsForward()));
			}, &st::menuIconTranslate);
		}
	...
	if (item->canStopPoll()) {
		menu->addAction(tr::lng_polls_stop(tr::now), [=] {
			controller->show(Ui::MakeConfirmBox({ tr::lng_polls_stop_warning(), [=](Fn<void()> &&close) {
					close();
