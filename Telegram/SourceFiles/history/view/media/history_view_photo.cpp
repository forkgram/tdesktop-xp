...
void Photo::draw(Painter &p, const PaintContext &context) const {
	...
	if (_serviceWidth > 0) {
		...
		const auto pix = [&] {
			...
			const auto args = Images::PrepareArgs{ {}, Images::RoundOptions(roundRadius, roundCorners), QSize(paintw, painth) };
...
void Photo::paintUserpicFrame(
	...
	const auto pix = [&] {
		const auto size = QSize(_pixw, _pixh);
		const auto args = Images::PrepareArgs{ {}, Images::Option::RoundCircle };
		if (const auto large = _dataMedia->image(PhotoSize::Large)) {
