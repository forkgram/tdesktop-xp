...
void Poll::Header::draw(
	...
	if (const auto descriptionHeight
		...
		auto descriptionContext = Ui::Text::PaintContext();
		descriptionContext.position = { left, tshift };
		descriptionContext.outerWidth = outerWidth;
		descriptionContext.availableWidth = innerWidth;
		descriptionContext.spoiler = Ui::Text::DefaultSpoilerCache();
		descriptionContext.now = context.now;
		descriptionContext.pausedEmoji = context.paused;
		descriptionContext.pausedSpoiler = context.paused;
		descriptionContext.selection = context.selection;
		descriptionContext.useFullWidth = true;
		_description.draw(p, descriptionContext);
	...
	auto questionContext = Ui::Text::PaintContext();
	questionContext.position = { left, tshift };
	questionContext.outerWidth = outerWidth;
	questionContext.availableWidth = innerWidth;
	questionContext.spoiler = Ui::Text::DefaultSpoilerCache();
	questionContext.now = context.now;
	questionContext.pausedEmoji = context.paused;
	questionContext.pausedSpoiler = context.paused;
	questionContext.selection = toQuestionSelection(context.selection);
	_question.draw(p, questionContext);
...
void Poll::Header::updateDescription() {
	...
	const auto context = Core::TextContext({
		&_owner->_poll->session(), // session
		{}, // details
		[=] { _owner->_parent->customEmojiRepaint(); }, // repaint
		2, // customEmojiLoopLimit
	});
...
void Poll::Header::validateTopMediaCache(QSize size) const {
	...
	auto prepared = Images::Prepare(
		source,
		size * ratio,
		{ nullptr, {}, size }); // colored, options, outer
...
void Poll::Options::updateAnswers() {
	const auto context = Core::TextContext({
		// XP walk: designated -> positional (C7555); _owner-> per Options/Part.
		&_owner->_poll->session(), // session
		{}, // details
		[=] { _owner->repaint(); }, // repaint
		2, // customEmojiLoopLimit
	...
	if (_voted != voted) {
		...
		if (_voted) {
			...
			if (_optionsPart->_votedFromHere
				...
				config.text = tr::lng_polls_results_after_close(
					tr::now,
					tr::marked);
				config.iconLottie = u"toast_hide_results"_q;
...
void Poll::Header::paintSolutionBlock(
	...
	auto solutionContext = Ui::Text::PaintContext();
	solutionContext.position = { innerLeft, yshift };
	solutionContext.outerWidth = _owner->width();
	solutionContext.availableWidth = textWidth;
	solutionContext.spoiler = Ui::Text::DefaultSpoilerCache();
	solutionContext.now = context.now;
	solutionContext.pausedEmoji = context.paused;
	solutionContext.pausedSpoiler = context.paused;
	solutionContext.selection = toSolutionSelection(context.selection);
	_solutionText.draw(p, solutionContext);
...
int Poll::Options::paintAnswer(
	...
	p.setPen(stm->historyTextFg);
	auto answerContext = Ui::Text::PaintContext();
	answerContext.position = { aleft, top };
	answerContext.outerWidth = outerWidth;
	answerContext.availableWidth = textWidth;
	answerContext.spoiler = Ui::Text::DefaultSpoilerCache();
	answerContext.now = context.now;
	answerContext.pausedEmoji = context.paused;
	answerContext.pausedSpoiler = context.paused;
	answer.text.draw(p, answerContext);
