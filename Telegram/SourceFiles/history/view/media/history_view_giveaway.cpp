...
: Media(parent) {
	generate([&](std::unique_ptr<Part> part) {
		_entries.push_back({
			std::move(part), // XP walk: designated -> positional (C7555); Entry::object
		});
	});
}
...
void TextMediaInBubblePart::draw(
	...
	_text.draw(p, {
		// XP walk: designated -> positional (C7555). PaintContext: position,
		// outerWidth, availableWidth, geometry, align, clip, palette, pre,
		// blockquote, colors, spoiler, now, ...
		{ (outerWidth - width()) / 2, _margins.top() }, // position
		outerWidth, // outerWidth
		width(), // availableWidth
		{}, // geometry
		style::al_top, // align
		{}, // clip
		&context.messageStyle()->textPalette, // palette
		{}, // pre
		{}, // blockquote
		{}, // colors
		{}, // spoiler
...
void TextDelimeterPart::draw(
	...
	_text.draw(p, {
		// XP walk: designated -> positional (C7555). PaintContext through
		// elisionLines(19); fullWidthSelection(16) default true kept explicit.
		{ _margins.left(), _margins.top() }, // position
		outerWidth, // outerWidth
...
: _parent(parent) {
	for (const auto &peer : list) {
		_peers.push_back({
			// XP walk: designated -> positional (C7555). Peer: name, thumbnail,
			// geometry, link, ripple, corners, bg, colorIndex.
			Ui::Text::String(
				st::semiboldTextStyle,
				peer->name(),
				kDefaultTextOptions,
				st::msgMinWidth), // name
			Dialogs::Stories::MakeUserpicThumbnail(peer), // thumbnail
			{}, // geometry
			peer->openLink(), // link
			{}, // ripple
			{}, // corners
...
void PeerBubbleListPart::draw(
	...
	for (const auto &peer : _peers) {
		...
		peer.name.draw(p, {
			// XP walk: designated -> positional (C7555). Full 22-field PaintContext;
			// fullWidthSelection(16)=true, elisionLines(19)=1, elisionBreakEverywhere(21)=true.
