...
QSize WebPage::countOptimalSize() {
	...
	if (_description.isEmpty()
		...
		// XP walk: designated -> positional (C7555); nested details.
		auto context = Core::TextContext({
			&history()->session(), // session
			{ // details { session(default), type }
				nullptr,
...
void WebPage::draw(Painter &p, const PaintContext &context) const {
	...
	if (_descriptionLines) {
		...
		textContext.availableWidth = descriptionWidth;
		textContext.geometry = previewGeometry
		...
		textContext.elisionHeight = ((_descriptionLines > 0)
			? (_descriptionLines * lineHeight)
			: 0);
		textContext.elisionRemoveFromEnd = (_descriptionLines > 0) ? endskip : 0;
