...
base::options::option<int> OptionStickerSize({
	// XP walk: designated -> positional (C7555). descriptor: id, name.
	"sticker-size", // id
	"Sticker size", // name
});
...
QPixmap Sticker::paintedPixmap(const PaintContext &context) const {
	...
	if (image) {
		return image->pix(useSize, { colored }); // XP walk: positional; _sensitiveBlurred v5.16.5
	//
	...
	//		{ .colored = colored, .options = Images::Option::Blur });
		...
		return good->pix(useSize, { colored }); // XP walk: positional; _size->useSize
	} else if (const auto thumbnail = _dataMedia->thumbnail()) {
		return thumbnail->pix(
			useSize,
			{ colored, Images::Option::Blur }); // XP walk: positional
	}
	return QPixmap();
}
