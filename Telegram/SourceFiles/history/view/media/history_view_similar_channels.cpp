...
void SimilarChannels::draw(Painter &p, const PaintContext &context) const {
	...
	Ui::PaintBubble(
		p,
		// XP walk: designated inits (C++20) -> positional; Ui::SimpleBubble (9 fields)
		// NOTE: 'shadowed' (field 7) defaults true and is a skipped gap -> must be explicit true
		Ui::SimpleBubble{
			context.st, // st
			geometry, // geometry
			...
			width(), // outerWidth
			{}, // selected
			true, // shadowed (defaults true)
			{}, // outbg
			{ large, large, large, large }, // rounding
		});
...
QSize SimilarChannels::countOptimalSize() {
	...
	for (const auto &peer : peers) {
		...
		// XP walk: designated inits (C++20) -> positional; Channel struct (12 fields)
		_channels.push_back({
			QRect(QPoint(x, y), outer.size()), // geometry
			Ui::Text::String(
				st::chatSimilarName,
				(moreCounter
					...
					: channel->name()),
				kDefaultTextOptions,
				st::chatSimilarChannelPhoto), // name
			Ui::MakeUserpicThumbnail(channel), // thumbnail
			{}, // link
			{}, // counter
			{}, // counterRect
			{}, // counterBg
			{}, // ripple
			uint32(moreCounter), // more
			uint32((moreCounter && !premium) ? 1 : 0), // moreLocked
		});
