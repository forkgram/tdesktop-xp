...
void SummaryHeader::paint(
	...
	if (w > st::historyReplyPadding.left()) {
		...
		if (namew > 0) {
			...
			textContext.availableWidth = textw;
			textContext.palette = replyToTextPalette;
			textContext.spoiler = Ui::Text::DefaultSpoilerCache();
			textContext.now = context.now;
			textContext.pausedEmoji = (context.paused
				|| On(PowerSaving::kEmojiChat));
			textContext.pausedSpoiler = (context.paused
...
void SummaryHeader::ensureLottie() const {
	_lottie = Lottie::MakeIcon(Lottie::IconDescriptor{
		...
		Size(st::historySummaryHeaderIconSize // sizeOverride
			- st::historySummaryHeaderIconSizeInner * 2),
		{}, // frame
		{}, // limitFps
