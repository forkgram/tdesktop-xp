...
[[nodiscard]] BadgePillGeometry ComputeBadgePillGeometry(
	...
	const auto height = padding.top()
		...
		+ padding.bottom();
	return {
		// XP walk: designated -> positional (C7555). BadgePillGeometry:
		// textWidth, width, height.
		textWidth, // textWidth
		std::max(contentWidth, height), // width
		height, // height
	};
}
...
void Message::initPaidInformation() {
	...
	const auto media = this->media();
	const auto mine = PaidInformation{ // XP walk: designated -> positional (C7555)
		1, // messages
		item->starsPaid(), // stars
	};
	...
	auto text = PreparedServiceText{ // XP walk: .text@0 designated -> positional (C7555)
		item->out()
	...
	if (statusWidth && availableWidth > statusWidth) {
		...
		if (_fromNameStatus->custom) {
			...
			_fromNameStatus->custom->paint(p, {
				...
				context.paused || On(PowerSaving::kEmojiStatus), // paused
			});
		} else {
	...
	if (badgeWidth) {
		...
		if (const auto badge = Get<RightBadge>()) {
			...
			if (badge->role != BadgeRole::User) {
				...
				p.setPen(badgeColor);
				badge->tag.draw(p, { // XP walk: designated -> positional (C7555)
					QPoint(
						badgeLeft + (pill.width - pill.textWidth) / 2,
						badgeTop + padding.top()), // position
					{}, // outerWidth
					pill.textWidth, // availableWidth
					{}, // geometry
					style::al_left, // align
					{}, // clip
					{}, // palette
				...
				badge->tag.draw(p, { // XP walk: designated -> positional (C7555)
					QPoint(badgeLeft, trect.top()), // position
					{}, // outerWidth
					badge->tag.maxWidth(), // availableWidth
					{}, // geometry
					style::al_left, // align
					{}, // clip
					{}, // palette
			...
			if (!badge->boosts.isEmpty()) {
				...
				p.setPen(badgeColor);
				badge->boosts.draw(p, { // XP walk: designated -> positional (C7555)
					QPoint(
						trect.left() + trect.width() - boostWidth,
						trect.top()), // position
					{}, // outerWidth
					boostWidth, // availableWidth
					{}, // geometry
					style::al_left, // align
					{}, // clip
					{}, // palette
...
void Message::paintText(
	...
	if (appearingClip) {
		...
		linePostprocess.emplace(Ui::Text::LinePostprocess{
			[=](int lineIndex) -> Fn<void(QImage&)> { // method
	...
	tcontext.highlight = needRippleMask
		? &rippleRequest
		: (highlightRequest ? &*highlightRequest : nullptr);
	tcontext.useFullWidth = true;
	tcontext.linePostprocess = linePostprocess ? &*linePostprocess : nullptr;
	...
	if (selection.isFlat()) {
		...
		return MessageSelection::RichPage(
			...
			MarkdownArticleSelectionEndpoints{
				selection.anchor.richPage, // from
				selection.focus.richPage, // to
			},
			anchor,
			focus,
...
Reactions::ButtonParameters Message::reactionButtonParameters(
	...
	auto result = ButtonParameters{ data()->fullId() }; // context -- XP walk: positional (outbg dropped, theirs)
