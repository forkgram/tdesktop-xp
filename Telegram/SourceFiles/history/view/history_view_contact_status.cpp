	...
	if (const auto user = peer->asUser()) {
		...
		return rpl::combine(
			...
			if (flags.value & Flag::Blocked) {
				...
				return {
					// XP walk: take theirs; designated -> positional (C7555). State:
					// type0 starsPerMessage1 requestChatName2 requestDate3 requestChatIsBroadcast4.
					Type::RequestChatInfo, // type
					0, // starsPerMessage
					peer->requestChatTitle(), // requestChatName
					peer->requestChatDate(), // requestDate
					!!(settings.value
						& PeerBarSetting::RequestChatIsBroadcast), // requestChatIsBroadcast
				};
...
void ContactStatus::setupState(not_null<PeerData*> peer, bool showInForum) {
	...
	_context = Core::TextContext({ &peer->session() }); // XP walk: take theirs; designated -> positional (C7555)
...
void PaysStatus::setupHandlers() {
	_inner->removeClicks(
		...
		// XP walk: took v5.16.3 refactor (extracted Window::PeerMenuConfirmToggleFee),
		// dropping OURS' inline confirm box + ToggleNoPaidMessagesException call.
