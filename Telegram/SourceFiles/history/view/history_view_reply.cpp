...
void Reply::update(
	...
	auto helper = Ui::Text::CustomEmojiHelper(Core::TextContext({
		&view->history()->session(), {}, // XP walk: designated -> positional; session@0, details@1
		repaint, // repaint@2
	}));
	const auto text = (!_displaying && data->unavailable())
		...
		? Ui::Text::Colorized(task->completionDate
			? helper.image({
				MakeTaskDoneImage(), // image (XP walk: designated -> positional)
				QMargins(0, st::lineWidth, st::lineWidth, 0), // margin
			})
			: helper.image({
				MakeTaskImage(), // image (XP walk: designated -> positional)
				QMargins(0, st::lineWidth, st::lineWidth, 0), // margin
			})).append(task->text)
		: pollAnswer
	...
	if (w > st::historyReplyPadding.left()) {
		if (_displaying) {
			...
			if (namew > 0) {
				...
				_text.draw(p, {
					// XP walk: designated -> positional (C7555)
					{ textLeft, textTop }, // position
					{}, // outerWidth
					{}, // availableWidth
					textGeometry(textw, firstLineSkip), // geometry
					style::al_left, // align
					{}, // clip
