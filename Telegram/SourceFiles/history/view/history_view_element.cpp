...
void ForumThreadBar::init(
	...
	if (sublist) {
		...
		text.setMarkedText(
			...
			kMarkupTextOptions,
			// XP walk: designated -> positional (C7555). TextContextArgs:
			// session, details, repaint, customEmojiLoopLimit. v6.4.0: loopLimit=-1.
			Core::TextContext({ &topic->session(), {}, {}, -1 }));
	}
...
void ForumThreadBar::Paint(
	...
	p.setPen(st->msgServiceFg());
		// XP walk: take theirs; designated init (C7555) -> named local; PaintContext
		// is large/non-contiguous (position@0, availableWidth@2, elisionLines@19).
		auto textContext = Ui::Text::PaintContext();
		textContext.position = { textLeft, textTop };
		textContext.availableWidth = available;
		textContext.paused = true;
...
void ServicePreMessage::init(
	...
	text = Ui::Text::String(
		...
		st::msgMinWidth,
		Core::TextContext({
			&view->history()->session(), // session // XP walk: designated -> positional
			{}, // details
			[=] { view->customEmojiRepaint(); }, // repaint
		}));
...
void ServicePreMessage::paint(
	...
	if (media && media->hideServiceText()) {
		...
		p.setFont(st::msgServiceFont);
		// XP walk: take theirs; designated init -> positional (C7555). fullWidthSelection
		// (@15, default true) set false; selection (@16) left default; geometry {} = SimpleGeometry.
		text.draw(p, {
...
void Element::refreshMedia(Element *replacing) {
	...
	if (const auto media = item->media()) {
		...
		_media = std::make_unique<MediaGeneric>(
			...
			MediaGenericDescriptor{
				st::chatSuggestInfoWidth, // maxWidth // XP walk: designated -> positional
				{}, // paintBg
				decision->lnk, // fullAreaLink
				true, // service
				true, // hideServiceText
			});
	} else {
