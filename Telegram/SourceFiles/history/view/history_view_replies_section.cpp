...
void RepliesWidget::setupComposeControls() {
	...
	_composeControls->setHistory({
		// XP walk: designated -> positional (C7555). SetHistoryArgs order:
		// history, topicRootId, showSlowmodeError, sendActionFactory,
		...
		SlowmodeSecondsLeft(_history->peer), // slowmodeSecondsLeft
		SendDisabledBySlowmode(_history->peer), // sendDisabledBySlowmode
		{}, // liked
...
void RepliesWidget::send(Api::SendOptions options) {
	...
	request.topicRootId = _topic ? _topic->rootId() : MsgId(0);
	request.forward = &_composeControls->forwardItems();
	request.text = &message.textWithTags;
	request.ignoreSlowmodeCountdown = (options.scheduled != 0);
...
SendMenu::Details RepliesWidget::sendMenuDetails() const {
	...
	const auto type = (_topic && !_history->peer->starsPerMessageChecked())
		? Type::Scheduled
		: Type::SilentOnly;
	return SendMenu::Details{ type }; // XP walk: take theirs; designated -> positional (C7555)
}
...
void RepliesWidget::refreshTopBarActiveChat() {
	...
	const auto state = EntryState{
		// XP walk: designated -> positional (C7555)
		(_topic ? Key{ _topic } : Key{ _history }), // key
		EntryState::Section::Replies, // section
		{}, // filterId
