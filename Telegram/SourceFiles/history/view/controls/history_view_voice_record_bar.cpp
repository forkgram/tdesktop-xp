...
QImage SoundedPreview::image(int size) {
	...
	const auto frame = streamed->frameWithInfo({ // XP walk: designated -> positional (C7555); .rounding@2 gap-filled
		full, // resize
		full, // outer
		{}, // rounding
		_roundingMask, // mask
...
[[nodiscard]] Ui::RoundVideoResult ToRoundVideoResult(
	...
	return Ui::RoundVideoResult{
		// XP walk: designated -> positional (C7555). RoundVideoResult:
		// content, waveform, duration, minithumbs, minithumbsCount, minithumbSize.
		std::move(data.bytes), // content
		std::move(data.waveform), // waveform
	...
	_parent->paintRequest(
...

		{
			...
			const auto trimGeometry = (progress == 1.)
				? TrimGeometry{
					// XP walk: designated -> positional (C7555). TrimGeometry:
					// frame, leftHandle, rightHandle.
					_trimFrameRect, // frame
					_trimLeftHandleRect, // leftHandle
					_trimRightHandleRect, // rightHandle
				}
...
std::optional<ListenWrap::TrimRange> ListenWrap::selectedTrimRange() const {
	...
	// XP walk: designated -> positional (C7555). TrimRange: from, till.
	return TrimRange{ crl::time(from), crl::time(till) };
}
...
void CancelButton::init() {
	...
	paintRequest(
		...
		_text.draw(p, {
			// XP walk: designated -> positional (C7555). position, outerWidth, availableWidth, geometry, align.
			QPoint(0, (height() - _text.minHeight()) / 2), // position
			width(), // outerWidth
	...
	auto appearanceCallback = [=] {
		...
		if (_paused.current()) {
			...
			if (_videoRecorder) {
				...
				_videoRecorder->resume({ // XP walk: designated -> positional (C7555)
					std::move(_data), // video
				});
				clearResumePrefix();
			} else {
				instance()->pause(false, nullptr);
...
Ui::RoundVideoResult VoiceRecordBar::mergeWithResumePrefix(
	...
	return Ui::RoundVideoResult{
		// XP walk: designated -> positional (C7555). RoundVideoResult:
		// content, waveform, duration, minithumbs, minithumbsCount, minithumbSize.
		std::move(combined.content), // content
	...
	if (type == StopType::Cancel) {
			...
			_videoRecorder->hide([=](Ui::RoundVideoResult data) {
				crl::on_main([=, data = std::move(data)]() mutable {
					if (weak) {
						window()->raise();
						window()->activateWindow();
						// XP walk: designated -> named-local (C7555); SendOptions.ttlSeconds set by name.
						auto options = Api::SendOptions();
						options.ttlSeconds = (ttlBeforeHide
								? std::numeric_limits<int>::max()
								: 0);
						_sendVoiceRequests.fire({ // XP walk: designated -> positional (C7555); .waveform@1 gap-filled
							data.content, // bytes
							{}, // waveform
							data.duration, // duration
							options, // options
		...
		instance()->stop(crl::guard(this, [=](Result &&data) {
			...
			options.ttlSeconds = (ttlBeforeHide
					? std::numeric_limits<int>::max()
					: 0);
			_sendVoiceRequests.fire({ // XP walk: designated -> positional (C7555)
				_data.content, // bytes
				_data.waveform, // waveform
...
void VoiceRecordBar::drawMessage(QPainter &p, float64 recordActive) {
	...
	_message.draw(p, {
		// XP walk: designated -> positional (C7555). position, outerWidth, availableWidth, geometry, align.
		_messageRect.topLeft(), // position
		_messageRect.width(), // outerWidth
		_messageRect.width(), // availableWidth
		{}, // geometry
		style::al_center, // align
	});
...
void VoiceRecordBar::showDiscardBox(
	...
	_show->showBox(Ui::MakeConfirmBox({
		// XP walk: designated -> positional (C7555); .cancelled@2 .cancelText@4 gap-filled
		(isListenState() // text
			? (_recordingVideo
...
bool VoiceRecordBar::createVideoRecorder() {
	...
	_videoRecorder = std::make_unique<Ui::RoundVideoRecorder>(
		Ui::RoundVideoRecorderDescriptor{ // XP walk: designated -> positional (C7555)
			_outerContainer, // container
			hiding, // hiding
			hidden, // hidden
			std::move(capturer), // capturer
			std::move(track), // track
			_show->session().local().readRoundPlaceholder(), // placeholder
		});
