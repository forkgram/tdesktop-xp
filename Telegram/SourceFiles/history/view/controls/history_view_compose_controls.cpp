			...
			if (isLeftButton && inPhotoEdit) {
				...
				if (_preview.parsed) {
					...
					if (_suggestOptions) {
						...
						// XP walk: take theirs; designated -> positional (messageId@0).
...
void FieldHeader::setShownMessage(HistoryItem *item) {
	...
	if (isEditingMessage()) {
		...
		const auto context = Core::TextContext({
			// XP walk: take theirs; designated -> positional (C7555); gap-fill details@1, repaint@2.
...
MessageToEdit FieldHeader::queryToEdit() {
	...
	// XP walk: take theirs; designated -> named-local (C7555).
...
, _ownedSelector((_regularWindow && _features.commonTabbedPanel)
	...
	: std::make_unique<ChatHelpers::TabbedSelector>(
		...
		ChatHelpers::TabbedSelectorDescriptor{
			...
			(_features.emojiOnlyPanel // mode (XP walk: v6.3.0 emojiOnlyPanel)
...
void ComposeControls::editStarsFrom(int selected) {
	...
	_show->show(Calls::Group::MakeVideoStreamStarsBox({
		// XP walk: designated -> positional (C7555). VideoStreamStarsBoxArgs: show, top,
		// min, current, sending, save, name.
		_show, // show
		{}, // top
		min, // min
		...
		_history ? _history->peer->shortName() : QString(), // name
	}));
}
	...
	if (!count) {
		...
		_starsReaction->clicks(
			...
			if (_chosenStarsCount && button == Qt::LeftButton) {
				_starsReactionIncrements.fire({ 1 }); // count (XP walk: designated -> positional)
				startStarsSendEffect();
			} else {
				_show->show(Calls::Group::MakeVideoStreamStarsBox({
					// XP walk: designated -> positional (C7555); VideoStreamStarsBoxArgs.
					_show, // show
					_starsReactionTop.current(), // top
					0, // min
					0, // current
					true, // sending
					!_chosenStarsCount, // admin
					crl::guard(_starsReaction, [=](int count) { // save
...
void ComposeControls::init() {
	...
	_header->editOptionsRequests(
		...
		// XP walk: designated -> positional (C7555).
		EditDraftOptions({
			// XP walk: take theirs (adds suggest to Draft); designated -> positional.
			_show, // show
			history, // history
			Data::Draft(_field, reply, suggest, _preview->draft()), // draft
			_preview->link(), // usedLink
			_header->forwardDraft(), // forward
	...
	_expand->setClickedCallback([=] {
		...
		if (_mode == Mode::Scheduled) {
			Iv::Editor::ShowComposeBox(
				...
				Iv::Editor::ComposeBoxOptions{
					// XP walk: designated -> positional (C7555).
					Iv::Editor::ComposeBoxOptions::Scope::Detached, // scope
					Iv::Editor::ComposeBoxOptions
						::SubmitPolicy::Schedule, // submitPolicy
					crl::guard( // returnText
						_wrap.get(),
						[=](TextWithTags text) {
							setText(text);
						}),
		...
		if (_currentDialogsEntryState.section
			...
			Iv::Editor::ShowComposeBox(
				...
				Iv::Editor::ComposeBoxOptions{
					// XP walk: designated -> positional (C7555); submitPolicy@1 skipped.
					Iv::Editor::ComposeBoxOptions::Scope::Detached, // scope
					{}, // submitPolicy
					crl::guard( // returnText
						_wrap.get(),
...
void ComposeControls::fireSendTextAsFile(
	...
	_show->show(Box<SendFilesBox>(SendFilesBoxDescriptor{
		...
		TextWithTags{}, // caption
		peer, // toPeer
		DefaultLimitsForPeer(peer), // limits
		DefaultCheckForPeer(_show, peer), // check
		sendType, // sendType
...
void ComposeControls::updateSendButtonType() {
	...
	using namespace Calls::Group::Ui;
	const auto &appConfig = _show->session().appConfig();
	// XP walk: designated -> positional (C7555). State: type, fillBgOverride,
	// slowmodeDelay, starsToSend.
	_send->setState({
		type, // type
		(_chosenStarsCount.value_or(0) // fillBgOverride
			? Ui::ColorFromSerialized(StarsColoringForCount(
				appConfig.groupCallColorings(),
				*_chosenStarsCount).bgLight)
			: QColor()),
		delay, // slowmodeDelay
		(ephemeralReply ? 0 : shownStarsPerMessage()), // starsToSend
		forbidden, // forbidden
	});
...
void ComposeControls::showAiComposeBox() {
	...
	Controls::ShowComposeAiBox(_show, {
		...
		crl::guard(_wrap.get(), [=](TextWithEntities result) {
...
void ComposeControls::editMessage(not_null<HistoryItem*> item) {
	...
	_history->setDraft(
		...
		std::make_unique<Data::Draft>(
			editData,
			std::move(editReplyTo), // XP walk: take theirs; named-local (above) == theirs' FullReplyTo
			SuggestOptions(), // XP walk: take theirs (Draft ctor gained SuggestOptions)
			cursor,
			Data::WebPageDraft::FromItem(item)));
	applyDraft();
