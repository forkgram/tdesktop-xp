...
void PreviewWrap::showForwardSelector(Data::ResolvedForwardDraft draft) {
	...
	for (const auto &source : draft.items) {
		...
		fields.id = _history->nextNonHistoryEntryId();
		fields.flags = (MessageFlag::FakeHistoryItem
			| MessageFlag::Outgoing
			| MessageFlag::HasFromId
			| (source->invertMedia()
				? MessageFlag::InvertMedia
				: MessageFlag()));
		fields.from = _history->session().userPeerId();
		fields.date = base::unixtime::now();
		fields.groupedId = groupedId;
		fields.ignoreForwardFrom = dropNames;
		fields.ignoreForwardCaptions = dropCaptions;
		const auto item = _history->addNewLocalMessage(std::move(fields), source);
...
rpl::producer<SelectedQuote> PreviewWrap::showQuoteSelector(
	...
	_entries.push_back({
		...
		nullptr, // item
		leader->createView(_delegate.get()), // view
	});
...
rpl::producer<QString> PreviewWrap::showLinkSelector(
	...
	fields.replyTo = {};
	fields.date = base::unixtime::now();
	const auto item = _history->addNewLocalMessage(std::move(fields), HighlightParsedLinks({
		message.text,
...
void DraftOptionsBox(
	...
	const auto setupLinkActions = [=] {
		...
		Settings::AddButtonWithIcon(
			...
			// XP walk: designated -> named-local (C7555).
			auto webpageRemoved = Data::WebPageDraft();
			webpageRemoved.removed = true;
			finish(resolveReply(), std::move(webpageRemoved), options);
		});
	...
	const auto setupForwardActions = [=] {
		...
		Settings::AddButtonWithIcon(
			...
			Window::ShowForwardMessagesBox(show, {
				// XP walk: designated -> positional (C7555).
				show->session().data().itemsToIds(draft.items), // ids
