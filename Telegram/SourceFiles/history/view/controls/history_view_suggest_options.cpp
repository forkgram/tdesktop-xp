...
void ChooseSuggestTimeBox(
	...
	Ui::ChooseDateTimeBox(box, {
		...
		((args.mode == SuggestMode::Publish)
			...
			: tr::lng_suggest_options_update_date()), // title (XP walk: positional; v5.16.1 text ->_date)
		done,
		[=] { return now + min; },
		value,
		[=] { return now + max; },
...
StarsTonPriceInput AddStarsTonPriceInput(
	...
	const auto makeIcon = [&](
		...
		return Ui::CreateChild<Ui::FlatLabel>(
			...
			st::defaultFlatLabel,
			st::defaultPopupMenu,
			Core::TextContext({ session })); // XP walk: designated -> positional (C7555); session@0
	};
	...
	return {
		// XP walk: designated -> positional (C7555); StarsTonPriceInput order:
		// focusCallback, computeResult, submits, updates, result.
		std::move(focusCallback),
		std::move(computeResult),
...
void ChooseSuggestPriceBox(
	...
	// XP walk: took theirs; ours' duplicate makeIcon already exists above (reorg).
	...
	auto priceInput = AddStarsTonPriceInput(container, {
		...
		// starsMin, starsMax, nanoTonMin, nanoTonMax, starsAbout, tonAbout.
		session,
		state->ton.value(),
		args.value.price(),
