		...
		if (item->chosenReaction() != reaction.emoji) {
			...
			if (const auto top = itemTop(view); top >= 0) {
				view->animateReaction({
					reaction.emoji,
					reaction.icon,
					reaction.geometry.translated(0, -top),
				});
			}
		}
	}, lifetime());
...
bool ListWidget::showCopyRestriction(HistoryItem *item) {
	...
	Ui::ShowMultilineToast({ Window::Show(_controller).toastParent(), { (type == CopyRestrictionType::Channel)
...
void ListWidget::leaveEventHook(QEvent *e) {
	_reactionsManager->updateButton({ {}, {}, {}, {}, {}, {}, {}, {}, true });
