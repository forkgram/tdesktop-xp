...
, _mediaState(std::make_shared<PollMediaUpload::PollMediaState>()) {
	...
	if (peer) {
		_uploader = std::make_unique<PollMediaUpload::PollMediaUploader>(
			PollMediaUpload::PollMediaUploader::Args{ // XP walk: designated -> positional (C7555)
				_session, // session
				peer, // peer
				[=](const QString &text) { // showError
...
void AddPollOptionWidget::setupField() {
	...
	_field->setCustomTextContext(Core::TextContext({ // XP walk: designated -> positional (C7555)
		_session, // session
	}));
...
void AddPollOptionWidget::setupAttach() {
	...
	_attach->addClickHandler([=] {
		if (ShowPollMediaPreview(_controller, _mediaState, { // XP walk: designated -> positional (C7555)
			[=] { // choosePhotoOrVideo
			...
			[=] { showStickerPanel(); }, // chooseSticker
			[=](Ui::PreparedList list) { // editPhoto
				_uploader->applyPreparedPhotoList(
					_mediaState,
					std::move(list));
			},
			[=] { _uploader->clearMedia(_mediaState); }, // remove
		})) {
			return;
		}
...
void AddPollOptionWidget::showStickerPanel() {
	...
	if (!_stickerPanel) {
		...
		_stickerPanel->selector()->fileChosen(
			...
			_uploader->setMedia(
				_mediaState,
				PollMedia{ // XP walk: designated -> positional (C7555)
					{}, // photo
