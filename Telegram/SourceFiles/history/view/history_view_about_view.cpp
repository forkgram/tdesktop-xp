...
-> Fn<void(
	...
	return [=](
		...
		const auto context = Core::TextContext({
			// XP walk: designated -> positional (C7555); gap-fill details@1.
			&parent->history()->session(), // session
			{}, // details
			[parent] { parent->repaint(); }, // repaint
		});
...
void AboutView::make(Data::ChatIntro data, bool preview) {
	...
	owned->overrideMedia(std::make_unique<HistoryView::MediaGeneric>(
		...
		GenerateChatIntro(
			...
			helloChosen,
			sendIntroSticker),
		HistoryView::MediaGenericDescriptor{ // XP walk: take theirs; designated -> positional (C7555)
			st::chatIntroWidth, // maxWidth
			{}, // paintBg
			{}, // fullAreaLink (theirs leaves default; service link set via setCustomServiceLink)
			true, // service
...
AdminLog::OwnedItem AboutView::makeNewBotThread() {
	...
	result->overrideMedia(std::make_unique<MediaGeneric>(
		...
		HistoryView::MediaGenericDescriptor{
			...
			st::newThreadAboutMaxWidth, // maxWidth
			{}, // paintBg
			{}, // fullAreaLink
			true, // service
			true, // hideServiceText
