...
[[nodiscard]] Data::Username UsernameFromTL(const MTPUsername &username) {
	return {
		qs(username.data().vusername()),
		username.data().is_active(),
		username.data().is_editable(),
	};
}
...
rpl::producer<> Usernames::reorder(
	...
	return [=](auto consumer) {
		...
		auto tlUsernames = QVector<MTPstring>();
		tlUsernames.reserve(usernames.size());
		for (const auto &username : usernames) {
			tlUsernames.push_back(MTP_string(username));
		}
