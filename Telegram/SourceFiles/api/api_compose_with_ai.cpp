...
mtpRequestId ComposeWithAi::request(
	...
	return _api.request(MTPmessages_ComposeMessageWithAI(
		...
		auto parsed = Result{
			ParseTextWithEntities(session, data.vresult_text()), // resultText
		};
...
ComposeWithAi::Diff ComposeWithAi::ParseDiff(
	...
	auto result = Diff{
		ParseTextWithEntities(session, text), // text
	};
	...
	for (const auto &entity : entities) {
		entity.match([&](const MTPDmessageEntityDiffInsert &data) {
			result.entities.push_back({
				DiffEntity::Type::Insert, // type
				data.voffset().v, // offset
				data.vlength().v, // length
			});
		}, [&](const MTPDmessageEntityDiffReplace &data) {
			result.entities.push_back({
				DiffEntity::Type::Replace, // type
				data.voffset().v, // offset
				data.vlength().v, // length
				qs(data.vold_text()), // oldText
			});
		}, [&](const MTPDmessageEntityDiffDelete &data) {
			result.entities.push_back({
				DiffEntity::Type::Delete, // type
				data.voffset().v, // offset
				data.vlength().v, // length
			});
		}, [](const auto &) {
