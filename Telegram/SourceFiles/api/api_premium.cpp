	...
	return [=](auto consumer) {
		...
		_api.request(MTPpayments_GetPremiumGiftCodeOptions(
			...
			for (const auto &tlOption : result.v) {
				...
				_stores[token] = Store{
					// XP walk: designated -> positional (C7555). Store: amount, currency,
					// product, quantity (+currency@1 in v5.12.0).
					data.vamount().v, // amount
					qs(data.vcurrency()), // currency
					qs(data.vstore_product().value_or_empty()), // product
					data.vstore_quantity().value_or_empty(), // quantity
				};
...
Payments::InvoicePremiumGiftCode PremiumGiftCodeOptions::invoice(
	...
	// XP walk: designated -> named-local (C7555; InvoicePremiumGiftCode
	// non-contiguous: skips purpose@0 and giveawayCredits@3).
	auto result = Payments::InvoicePremiumGiftCode();
	result.currency = store.currency;
	result.storeProduct = store.product;
	result.randomId = randomId;
	result.amount = store.amount;
	result.storeQuantity = store.quantity;
	result.users = token.users;
	result.months = token.months;
	return result;
}
...
std::vector<GiftOptionData> PremiumGiftCodeOptions::optionsForPeer() const {
	...
	if (!_optionsForOnePerson.currencies.empty()) {
		...
		for (auto i = 0; i != count; ++i) {
			...
			result.push_back({
				// XP walk: designated -> positional (C7555). GiftOptionData: cost,
				// currency, months.
				_optionsForOnePerson.totalCosts[i], // cost
				_optionsForOnePerson.currencies[i], // currency
...
MessageMoneyRestriction ResolveMessageMoneyRestrictions(
	...
	if (const auto channel = peer->asChannel()) {
		return {
			...
			channel->starsPerMessageChecked(), // starsPerMessage
			{}, // premiumRequired
			true, // known
		};
	}
	...
	if (!user) {
		...
		return {
			// XP walk: designated -> positional (C7555).
			user->starsPerMessageChecked(), // starsPerMessage
			(user->requiresPremiumToWrite()
				&& !user->session().premium()), // premiumRequired
			true, // known
		};
...
std::optional<Data::StarGift> FromTL(
	...
	return gift.match([&](const MTPDstarGift &data) {
		...
		return std::optional<Data::StarGift>(Data::StarGift{
			...
			{}, // unique
			background(), // background@2 (NEW v6.3.6)
			...
			data.vauction_start_date().value_or_empty(), // auctionStartDate@13 (NEW v6.3.6)
			...
			data.vupgrade_variants().value_or_empty(), // upgradeVariants@18 (NEW v6.3.6)
			...
			std::make_shared<Data::UniqueGift>(Data::UniqueGift{
				...
				*pattern, // pattern
				...
				(data.vvalue_amount() // value (v6.1.0)
					...
					int64(data.vvalue_usd_amount().value_or_empty()), // valuePriceUsd (NEW v6.3.6)
						})
					: nullptr),
	...
	return Data::SavedStarGift{
		...
		(data.vcollection_id()
			? (data.vcollection_id()->v
				...
				| ranges::to_vector)
			: std::vector<int>()),
		// XP walk: designated -> positional (C7555; SavedStarGift
		// message@3, starsConverted@4, starsUpgradedBySender@5, fromId@6;
		// took theirs' ParseTextWithEntities refactor).
		(data.vmessage()
		...
		(data.vfrom_id()
			...
			: PeerId()), // fromId
		data.vdate().v, // date
		data.vgift_num().value_or_empty(), // giftNum@10 (NEW v6.3.6)
