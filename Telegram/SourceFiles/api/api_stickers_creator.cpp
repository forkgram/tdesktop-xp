...
void AddAddToStickerSetAction(
	...
	args.text = tr::lng_stickers_add_to_set(tr::now);
	args.icon = &st::menuIconStickerAdd;
	args.fillSubmenu = [show, document](not_null<Ui::PopupMenu*> submenu) {
		FillChooseStickerSetMenu(submenu, show, document);
	};
	args.submenuSt = &st::popupMenuWithIcons;
...
void AddAddToEmojiSetAction(
	...
	// XP walk: designated -> named-local (C7555).
	auto args = Ui::Menu::MenuCallback::Args();
	args.text = tr::lng_emoji_add_to_set(tr::now);
	args.icon = &st::menuIconEmoji;
	args.fillSubmenu = [show, document](not_null<Ui::PopupMenu*> submenu) {
