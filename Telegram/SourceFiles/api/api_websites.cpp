...
Websites::Entry ParseEntry(
	...
	auto result = Websites::Entry{
		data.vhash().v, // hash
		owner->user(data.vbot_id()), // bot
		{}, // activeTime
		{}, // active
		qs(data.vplatform()), // platform
		qs(data.vdomain()), // domain
		qs(data.vbrowser()), // browser
		qs(data.vip()), // ip
		qs(data.vregion()), // location
	};
...
void Websites::reload() {
	...
	_requestId = _api.request(MTPaccount_GetWebAuthorizations(
		...
		auto list = List();
		const auto &authorizations = data.vauthorizations().v;
		list.reserve(authorizations.size());
		for (const auto &auth : authorizations) {
			list.push_back(ParseEntry(owner, auth.data()));
		}
		_list = std::move(list);
		_listChanges.fire({});
	}).fail([=] {
