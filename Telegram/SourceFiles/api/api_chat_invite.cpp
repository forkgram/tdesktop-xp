...
[[nodiscard]] ChatInvite ParseInvite(
	...
	// XP walk: designated -> positional (C7555); ChatInvite fields in order.
	return {
		qs(data.vtitle()), // title
		data.vabout().value_or_empty(), // about
		(photo->isNull() ? nullptr : photo.get()), // photo
		data.vparticipants_count().v, // participantsCount
		std::move(participants), // participants
		data.is_public(), // isPublic
		data.is_channel(), // isChannel
		data.is_megagroup(), // isMegagroup
		data.is_broadcast(), // isBroadcast
		data.is_request_needed(), // isRequestNeeded
		data.is_fake(), // isFake
		data.is_scam(), // isScam
			...
			if (const auto image = state->photoMedia->image(small)) {
				p.drawPixmap(
					...
					image->pix(
						Size(photoSize),
						// XP walk: designated -> positional (C7555); .colored gap-filled nullptr.
						Images::PrepareArgs{ nullptr, Images::Option::RoundCircle }));
			}
		} else if (state->photoEmpty) {
			state->photoEmpty->paintCircle(
				p,
