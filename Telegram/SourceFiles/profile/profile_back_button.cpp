...
void BackButton::paintEvent(QPaintEvent *e) {
	...
	context.position = QPoint{ textX, startY };
	context.outerWidth = width();
	context.availableWidth = _elisionWidth;
	...
	if (!_subtext.isEmpty()) {
		...
		subtextContext.position = { textX, startY + textHeight + st::lineWidth * 2 };
		subtextContext.outerWidth = width();
		subtextContext.availableWidth = _elisionWidth;
		subtextContext.elisionLines = 1;
