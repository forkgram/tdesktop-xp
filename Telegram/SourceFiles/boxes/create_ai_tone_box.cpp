...
void ShowToneToast(
	...
	config.title = (created
		...
		: tr::lng_ai_compose_tone_updated)(
			...
			lt_title,
			tone.title);
	config.text = tr::lng_ai_compose_tone_created_description(
		tr::now,
		Ui::Text::WithEntities);
	config.iconContent = Ui::MakeCustomEmojiToastIcon(
		session,
		tone.emojiId,
		size);
	config.iconPadding = st::aiComposeToneToastIconPadding;
	config.duration = kAiComposeToneToastDuration;
...
void ChooseToneIconBox(
	...
	auto descriptor = EmojiListDescriptor();
	descriptor.show = controller->uiShow();
	descriptor.mode = EmojiListWidget::Mode::TopicIcon;
	descriptor.paused = Window::PausedIn(
		controller,
		Window::GifPauseReason::Layer);
	descriptor.customRecentList = DocumentListToRecent(icons->list());
	descriptor.customRecentFactory = std::move(factory);
	descriptor.st = &st::reactPanelEmojiPan;
	const auto selector = body->add(
		object_ptr<EmojiListWidget>(body, std::move(descriptor)),
...
void ConfirmDeleteAiTone(
	...
	if (!tone.creator) {
		...
		args.text = tr::lng_ai_compose_tone_remove_sure();
		...
		args.confirmText = tr::lng_box_remove();
		show->show(Ui::MakeConfirmBox(std::move(args)));
		return;
	}
	auto args = Ui::ConfirmBoxArgs();
	args.text = tr::lng_ai_compose_tone_delete_sure();
	args.confirmed = [=](Fn<void()> &&close) {
		close();
	...
	args.title = tr::lng_ai_compose_tone_delete();
	show->show(Ui::MakeConfirmBox(std::move(args)));
}
