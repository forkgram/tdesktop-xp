...
object_ptr<Ui::RpWidget> AddReactionsSelector(
	...
	auto context = Core::TextContext({ session });
...
void EditAllowedReactionsBox(
	...
	reactions->add(AddReactionsSelector(reactions, {
		...
		args.customReactionsHardLimit, // customHardLimit
		!args.isGroup, // all
	}), st::boxRowPadding);
...
void SaveAllowedReactions(
	...
	auto ids = QVector<MTPReaction>();
	ids.reserve(allowed.some.size());
