...
[[nodiscard]] Ui::Text::PaletteDependentEmoji MakeTagPillEmoji(
	...
	return { [=] { // factory
		...
		context.availableWidth = textWidth;
		string.draw(p, context);
		p.end();
		return result;
	}, st::customEmojiTextBadgeMargin }; // margin
}
...
void TagPreviewsWidget::paintBubbleToImage(
	...
	if (previewRole != BadgeRole::User) {
		...
		context.availableWidth = badgeTextWidth;
		badgeString.draw(p, context);
	} else if (badgeTextWidth > 0) {
		p.setPen(st::rankUserFg);
		auto context = Ui::Text::PaintContext();
		context.position = QPoint(innerRight - badgeTextWidth, badgeTop);
		context.availableWidth = badgeTextWidth;
		badgeString.draw(p, context);
	}
