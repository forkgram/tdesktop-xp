	...
	for (const auto &my : boosts) {
		...
		if (!peerId && cooldown <= now) {
			...
			result.other.push_back({
				...
				// TakenBoostSlot: id, expires, peerId, cooldown.
				id, // id
				data.vexpires().v, // expires
				peerId, // peerId
				cooldown, // cooldown
			});
		}
...
Ui::BoostCounters ParseBoostCounters(
	...
	return {
		// XP walk: designated -> positional (C7555). Ui::BoostCounters:
		// level, boosts, thisLevelBoosts, nextLevelBoosts, mine.
		data.vlevel().v, // level
		data.vboosts().v, // boosts
		data.vcurrent_level_boosts().v, // thisLevelBoosts
		data.vnext_level_boosts().value_or_empty(), // nextLevelBoosts
...
[[nodiscard]] Sources SourcesCount(
	...
	return {
		// XP walk: designated -> positional (C7555). Sources: groups, channels.
		int(groups.size()), // groups
...
object_ptr<Ui::RpWidget> CreateUserpicsTransfer(
	...
	// XP walk: take theirs (st->button); qualify Ui::CreateChild (MSVC 14.16
	// won't ADL a template-id `f<T>()`), matching the sibling below.
...
[[nodiscard]] PaintRoundImageCallback GenerateGiftUniqueUserpicCallback(
	...
	return [=](QPainter &p, int x, int y, int outerw, int size) {
		...
		state->sticker->paint(q, {
			...
			crl::now(), // now
			{}, // scale
			QPoint((ideal - esize) / 2, (ideal - esize) / 2), // position
		});
		q.end();
