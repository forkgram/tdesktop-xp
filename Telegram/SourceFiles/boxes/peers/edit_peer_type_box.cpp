...
void Controller::checkUsernameAvailability() {
	...
	_checkUsernameRequestId = _api.request(MTPchannels_CheckUsername(
		...
		if (type == u"CHANNEL_PUBLIC_GROUP_NA"_q) {
			...
			_goodUsername = false;
			_usernameCheckInfo.fire({ UsernameCheckInfo::Type::PurchaseAvailable });
...
void Controller::showUsernamePending() {
	_usernameCheckInfoLifetime.destroy();
	_usernameCheckInfo.fire({
		UsernameCheckInfo::Type::Default,
		{ tr::lng_create_channel_link_pending(tr::now) },
	});
}

void Controller::showUsernameEmpty() {
	_usernameCheckInfoLifetime.destroy();
	_usernameCheckInfo.fire({ UsernameCheckInfo::Type::Default });
}
		...
		addButton(tr::lng_settings_save(), [=] {
			...
			local(EditPeerTypeData{
				...
				(v == Privacy::HasUsername
					...
					: std::vector<QString>()),
				{},
				controller->noForwards(),
				controller->joinToWrite(),
