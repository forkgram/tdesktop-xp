...
base::unique_qptr<Ui::RpWidget> CreateEmptyPlaceholder(
	...
	auto icon = Settings::CreateLottieIcon(
		container,
		{
			...
			// name@0, path@1, json@2, color@3, sizeOverride@4. v6.2.6 changed
			// sizeOverride to st::normalBoxLottieSize.
			u"my_gifts_empty"_q, // name
			{}, // path
			{}, // json
			{}, // color
...
, _replyItem(_history->addNewLocalMessage({
	...
	(MessageFlag::FakeHistoryItem
		...
		| MessageFlag::Post), // flags
	_fake->id, // from
	FullReplyTo{ _replyToItem->fullId() }, // replyTo
	base::unixtime::now(), // date
}, TextWithEntities{ _peer->isSelf()
...
void LevelBadge::updateText() {
	...
	_text.setMarkedText(
		...
		text,
		kMarkupTextOptions,
		Ui::Text::MarkedContext{ [=] { update(); } }); // XP walk: designated -> positional (repaint@0)
...
void Apply(
	...
	if (colorMatch
		...
		CheckBoostLevel(show, peer, [=](int level) {
			...
			// XP walk: v4.13.0 refactor - CheckBoostLevel now takes an askMore
			// callback returning optional<AskBoostReason> plus a cancel callback,
			// and shows the AskBoostBox internally.
...
[[nodiscard]] object_ptr<Ui::SettingsButton> CreateEmojiIconButton(
	...
	right->paintRequest(
		...
		if (state->emoji
			...
			state->emoji->paint(p, {
				// XP walk: designated -> positional (C7555). CustomEmoji::Context:
				// textColor@0, position@4, internal@7 (colorized@0, forceFirstFrame@1).
				textColor, // textColor
				{}, // size
				{}, // now
				{}, // scale
				QPoint(added, (height - button.emojiWidth) / 2), // position
				{}, // paused
				{}, // scaled
				{ // internal
					{}, // colorized
	...
	right->paintRequest(
		...
		if (state->emoji) {
			state->emoji->paint(p, {
				...
				// textColor, size, now, scale, position, ...
				anim::color(
	...
	raw->setClickedCallback([=] {
		...
		if (controller) {
			state->panel.show({
				...
				// button, animationSizeTag, ensureAddedEmojiId, customTextColor,
				// backgroundEmojiMode, channelStatusMode.
				controller, // controller
				right, // button
				{}, // animationSizeTag
		...
		if (const auto emoji = state->icon) {
			...
			if (state->custom) {
				state->custom->paint(p, Ui::Text::CustomEmoji::Context{
					// XP walk: designated -> positional (C7555). CustomEmojiPaintContext:
					// textColor, size, now, scale, position, paused, scaled, internal.
	...
	const auto preview = container->add(
		object_ptr<Info::Profile::TopBar>(
			...
			Info::Profile::TopBar::Descriptor {
				// XP walk: designated -> positional (C7555). Descriptor: controller,
				// key, wrap, source, peer, backToggles, showFinished.
				show->resolveWindow(), // controller
				Info::Key(peer), // key
				rpl::single(Info::Wrap::Side), // wrap
				Info::Profile::TopBar::Source::Preview, // source
				peer, // peer
				rpl::single(false), // backToggles
...
void EditPeerColorSection(
	...
	button->setClickedCallback([=] {
		...
		const auto profileValues = SetValues{
			// XP walk: designated -> positional (C7555). SetValues: colorIndex,
			// backgroundEmojiId, colorCollectible, statusId, statusUntil,
			// statusChanged, forProfile.
			state->profileIndex.current(), // colorIndex
			state->profileEmojiId.current(), // backgroundEmojiId
			state->collectible.current(), // colorCollectible
			{}, // statusId
			0, // statusUntil
			false, // statusChanged
			true, // forProfile
...
void EditPeerProfileColorSection(
	...
	button->setClickedCallback([=] {
		...
		const auto values = SetValues{
			// XP walk: designated -> positional (C7555). SetValues fields.
			state->index.current(), // colorIndex
			state->patternEmojiId.current(), // backgroundEmojiId
			std::nullopt, // colorCollectible
			state->wearable.current(), // statusId
			0, // statusUntil
...
void SetupPeerColorSample(
	...
	const auto sample = Ui::CreateChild<Ui::ColorSample>(
		button.get(),
		// XP walk: designated -> positional (TextContextArgs: session@0).
		[=] { return Core::TextContext({ &peer->session() }); },
	...
	emojiStatusWidget->paintOn([=](QPainter &p) {
		...
		emojiState->emoji->paint(p, {
			// XP walk: designated -> positional (C7555). CustomEmojiPaintContext:
			// textColor, size, now (textColor is required<QColor>).
			st::windowFg->c, // textColor
			{}, // size
			crl::now(), // now
		});
...
ButtonWithEmoji ButtonStyleWithRightEmoji(
	...
	return {
		// XP walk: designated -> positional (C7555). ButtonWithEmoji: st, emojiWidth, noneWidth, added.
		ButtonStyleWithAddedPadding(
			parent,
			parentSt,
			QMargins(0, 0, added + rightAdded, 0)), // st
