...
std::vector<RestrictionLabel> RestrictionLabels(
	...
	if (!options.isForum) {
		result.erase(
			std::remove_if(begin(result), end(result), [](const RestrictionLabel &l) {
				return l.flags == Flag::CreateTopics;
			}),
			end(result));
	}
	return result;
...
std::vector<AdminRightLabel> AdminRightLabels(
	...
	if (options.isGroup) {
		...
		if (!options.isForum) {
			result.erase(
				std::remove_if(begin(result), end(result), [](const AdminRightLabel &l) {
					return l.flags == Flag::ManageTopics;
				}),
				end(result));
		}
		return result;
