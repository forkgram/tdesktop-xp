...
void Controller::fillPrivacyTypeButton() {
	...
	_typeDataSavedValue = EditPeerTypeData{
		...
		{}, // requestToJoinApplyToInvites
		guardBotUsername,
		guardBotUsername.isEmpty()
			? QString()
			: _peer->session().createInternalLink(guardBotUsername),
	};
...
void Controller::fillAutoTranslateButton() {
	...
	const auto reason = Ui::AskBoostReason{
		// XP walk: designated -> positional (AskBoostReason.data@0; requiredLevel@0).
		Ui::AskBoostAutotranslate{ requiredLevel }, // data
	};
...
void Controller::fillManageSection() {
	...
	if (hasStarRef) {
		...
		AddButtonWithCount(
			...
			rpl::single(QString()), // Empty count.
			std::move(callback),
			{ &st::menuIconStarRefShare }); // XP walk: IconDescriptor positional (C7555)
	}
...
void Controller::editReactions() {
	...
	if (!_peer->isBroadcast()) {
		_navigation->uiShow()->show(Box(
			...
			// XP walk: designated -> positional (C7555). EditAllowedReactionsArgs:
			// navigation, allowedCustomReactions, customReactionsHardLimit, isGroup,
			// list, allowed, askForBoosts, save.
			EditAllowedReactionsArgs{
				_navigation, // navigation
				{}, // allowedCustomReactions
				{}, // customReactionsHardLimit
				true, // isGroup
				_navigation->session().data().reactions().list(
					Data::Reactions::Type::Active), // list
				Data::PeerAllowedReactions(_peer), // allowed
				{}, // askForBoosts
				done, // save
			}));
		return;
		...
		const auto askForBoosts = [=](int required) {
			if (const auto strong = weak.get()) {
				...
				strong->show(Box(Ui::AskBoostBox, Ui::AskBoostBoxData{
					...
					link, // link
					counters, // boost
					(_peer->isChannel() // features
						? LookupBoostFeatures(_peer->asChannel())
						: Ui::BoostFeatures()),
					{ Ui::AskBoostCustomReactions{ required } }, // reason
					!_peer->isBroadcast(), // group
				}, openStatistics, nullptr));
			}
		};
		_navigation->uiShow()->show(Box(
			EditAllowedReactionsBox,
			// XP walk: designated -> positional (C7555). isGroup (field 4) gap.
			EditAllowedReactionsArgs{
				_navigation, // navigation
				counters.level, // allowedCustomReactions
				Data::PremiumLimits(
					&_peer->session()).maxBoostLevel(), // customReactionsHardLimit
				{}, // isGroup
				_navigation->session().data().reactions().list(
					Data::Reactions::Type::Active), // list
				Data::PeerAllowedReactions(_peer), // allowed
