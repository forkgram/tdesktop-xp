...
void Controller::choose(not_null<ChannelData*> chat) {
	...
	// XP walk: take theirs' peerListUiShow()->showBox API; ConfirmBoxArgs
	// positional (C7555): text, confirmed, cancelled, confirmText.
	delegate()->peerListUiShow()->showBox(Ui::MakeConfirmBox({
		text, // text
		sure, // confirmed
		v::null, // cancelled
		tr::lng_manage_discussion_group_link(tr::now), // confirmText
	}));
}
...
[[nodiscard]] object_ptr<Ui::BoxContent> EditDiscussionLinkBox(
	...
	const auto init = [=](not_null<ListBox*> box) {
		...
		Settings::AddDividerTextWithLottie(above, {
			...
			u"discussion"_q, // lottie
			{}, // lottieRepeat
			{}, // lottieSize
			{}, // lottieMargins
			box->showFinishes(), // showFinished
			About(channel, chat), // about
		});
		if (!chat) {
