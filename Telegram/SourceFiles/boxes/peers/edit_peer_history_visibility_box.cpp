...
void EditPeerHistoryVisibilityBox(
	...
	const auto tryClose = [=] {
		...
		// XP walk: designated -> named-local (C7555; ConfirmBoxArgs sparse).
		auto args = Ui::ConfirmBoxArgs();
		args.text = tr::lng_bot_close_warning();
		args.confirmed = crl::guard(box, [=](Fn<void()> close) {
			close();
			box->closeBox();
		});
		args.confirmText = tr::lng_bot_close_warning_sure();
		args.cancelText = tr::lng_create_group_back();
