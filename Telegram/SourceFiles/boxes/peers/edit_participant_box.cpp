			...
			if (!isTargetCreator) {
				...
				dismissButton->setClickedCallback([=] {
					if (const auto callback = _saveCallback) {
						...
						args.text = tr::lng_profile_sure_remove_admin(
							tr::now,
							lt_user,
							user()->firstName);
						args.confirmed = confirmed;
						args.confirmText = tr::lng_box_remove();
						uiShow()->showBox(Ui::MakeConfirmBox(std::move(args)));
					}
				});
			}
		}
	...
	if (canSave()) {
		...
		removeButton->setClickedCallback([=] {
			if (const auto callback = _saveCallback) {
				...
				// XP walk: designated -> named-local (C7555). ConfirmBoxArgs.
				auto args = Ui::ConfirmBoxArgs();
				args.text = tr::lng_profile_sure_kick(
					tr::now,
					lt_user,
					user()->firstName);
				args.confirmed = confirmed;
...
void EditRestrictedBox::showRestrictUntil() {
	uiShow()->showBox(Box([=](not_null<Ui::GenericBox*> box) {
		...
		ChooseDateTimeBox(box, {
			tr::lng_rights_chat_banned_until_header(),
			tr::lng_settings_save(),
			save,
			[=] { return now; },
			time,
