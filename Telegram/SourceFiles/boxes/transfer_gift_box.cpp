...
[[nodiscard]] ExportOption MakeExportOption(
	...
	const auto activate = [=] {
		...
		args.text = tr::lng_gift_transfer_unlocks_about(
			lt_when,
			((hours >= 24)
				...
				: tr::lng_gift_transfer_unlocks_when_hours(
					lt_count,
					rpl::single(hours * 1.))));
		args.title = tr::lng_gift_transfer_unlocks_title();
...
bool ShowResaleGiftLater(
	...
	config.title = tr::lng_gift_resale_transfer_early_title(tr::now);
	config.text = { tr::lng_gift_resale_early(tr::now, lt_duration, days
...
bool ShowTransferGiftLater(
	...
	auto config = Ui::Toast::Config();
	config.title = tr::lng_gift_resale_transfer_early_title(tr::now);
	config.text = { tr::lng_gift_transfer_early(tr::now, lt_duration, days
			? tr::lng_days(tr::now, lt_count, days)
			: hours
			? tr::lng_hours(tr::now, lt_count, hours)
			: tr::lng_minutes(tr::now, lt_count, minutes)) };
