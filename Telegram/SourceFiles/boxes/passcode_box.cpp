...
void PasscodeBox::save(bool force) {
	...
	if (!onlyCheck && pwd != conf) {
		...
		if (!onlyCheck && !_recoverEmail->isHidden() && email.isEmpty() && !force) {
			...
			_replacedBy = getDelegate()->show(Ui::MakeConfirmBox({ { tr::lng_cloud_password_about_recover() }, crl::guard(this, [this] { save(true); }), {}, tr::lng_cloud_password_skip_email(), {}, &st::attentionBoxButton }));
		} else if (onlyCheck) {
...
void PasscodeBox::submitOnlyCheckCloudPassword(const QString &oldPassword) {
	...
	if (_cloudFields.turningOff && _cloudFields.notEmptyPassport) {
		...
		getDelegate()->show(Ui::MakeConfirmBox({ tr::lng_cloud_password_passport_losing(), [=](Fn<void()> &&close) { send(); close(); }, {}, tr::lng_continue() }));
	} else {
		send();
	}
}
...
void PasscodeBox::suggestSecretReset(const QString &newPassword) {
	...
	getDelegate()->show(Ui::MakeConfirmBox({ { Lang::Hard::PassportCorruptedChange() }, std::move(resetSecretAndSave), {}, Lang::Hard::PassportCorruptedReset() }));
}
...
void PasscodeBox::recoverByEmail() {
	if (!_cloudFields.hasRecovery) {
		...
		getDelegate()->show(Ui::MakeConfirmBox({ tr::lng_cloud_password_reset_no_email(tr::now), reset, {}, tr::lng_cloud_password_reset_ok(tr::now) }));
...
, _closeParent(std::move(closeParent)) {
	if (_cloudFields.pendingResetDate != 0 || !session) {
		...
		_noEmailAccess->setClickedCallback([=] {
			...
			getDelegate()->show(Ui::MakeConfirmBox({ tr::lng_cloud_password_reset_with_email(), reset, {}, tr::lng_cloud_password_reset_ok() }));
		});
	}
}
