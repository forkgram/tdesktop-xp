...
struct AttributeId {
	...
	// XP walk: defaulted spaceship/== (C++20, C7589) -> manual ==/!=/< (value, type).
	[[nodiscard]] friend inline bool operator==(AttributeId a, AttributeId b) {
...
[[nodiscard]] AttributeId FromTL(const MTPStarGiftAttributeId &id) {
	return id.match([&](const MTPDstarGiftAttributeIdBackdrop &data) {
		return AttributeId{
			uint64(uint32(data.vbackdrop_id().v)), // value
			AttributeIdType::Backdrop, // type
		};
	}, [&](const MTPDstarGiftAttributeIdModel &data) {
		return AttributeId{
			data.vdocument_id().v, // value
			AttributeIdType::Model, // type
		};
	}, [&](const MTPDstarGiftAttributeIdPattern &data) {
		return AttributeId{
			data.vdocument_id().v, // value
			AttributeIdType::Pattern, // type
		};
	});
}
...
[[nodiscard]] AttributeId IdFor(const Data::UniqueGiftBackdrop &value) {
	return {
		uint64(uint32(value.id)), // value
		AttributeIdType::Backdrop, // type
	};
}

[[nodiscard]] AttributeId IdFor(const Data::UniqueGiftModel &value) {
	return { value.document->id, AttributeIdType::Model };
}

[[nodiscard]] AttributeId IdFor(const Data::UniqueGiftPattern &value) {
	return { value.document->id, AttributeIdType::Pattern };
}
	...
	return [=](auto consumer) {
		...
		api->requestStarGifts(
			...
			for (auto &gift : gifts) {
				...
				// default-constructible). transferId@0, info@1, ..., resale@8, mine@9.
				list.push_back({ {}, gift });
				if (gift.resellCount > 0) {
					list.push_back({ {}, gift, {}, {}, {}, {}, {}, {}, true });
				}
			}
...
[[nodiscard]] ResaleTabs MakeResaleTabs(
	...
	const auto showMenu = [=](int index) {
		...
		// XP walk: designated .session -> positional (C7555); took theirs' show
		// (window is out of scope in MakeResaleTabs).
	...
	raw->paintRequest() | rpl::start_with_next([=] {
		...
		for (const auto &button : state->buttons) {
			...
			// XP walk: designated -> named-local (Ui::Text::PaintContext non-contiguous).
	...
	return {
		// XP walk: designated -> positional (ResaleTabs: filter, widget).
...
void SendGiftBox(
	...
	// XP walk: designated -> named-local (C7555; GiftDetails skips text@1,
	// anonymous@3 -- non-contiguous). Took theirs (+upgraded).
	auto details = GiftDetails();
	details.descriptor = descriptor;
	details.randomId = base::RandomValue<uint64>();
	details.upgraded = disallowLimited && (costToUpgrade > 0);
	...
	if (stars) {
		if (costToUpgrade > 0 && !peer->isSelf() && !disallowLimited) {
			...
			AddUpgradeButton(container, costToUpgrade, peer, [=](bool on) {
				...
				*showing = true;
				// XP walk: designated -> positional (C7555; not_null controller/peer,
				// required stargiftId block named-local; savedId@4 gap-filled default).
...
[[nodiscard]] rpl::producer<ResaleGiftsDescriptor> ResaleGiftsSlice(
	...
	return [=](auto consumer) {
		...
		const auto requestId = session->api().request(
			...
			// XP walk: designated -> named-local (giftId@0, offset@2, count@8 non-contiguous).
		...
		const auto ensureButton = [&, window, peer, raw, state, single, extend, first, last](int index) {
			...
			raw->setClickedCallback([=] {
				...
				if (premiumNeeded && !peer->session().premium()) {
					...
					window->showToast({
						// XP walk: designated -> positional (C7555; Toast::Config title@0 gap-filled, text@1).
						QString(), // title
...
void GiftBox(
	...
	if (!allStarsDisallowed) {
		AddBlock(content, window, {
			...
			MakeStarsGifts(window, peer, std::move(my)), // content
		});
	}
}
	...
	content->add(MakeGiftsList(window, peer, rpl::single(
		...
		for (const auto &gift : state->data.list) {
			result.list.push_back(GiftTypeStars{
				// XP walk: designated -> positional (C7555; not default-constructible).
				// v6.0.0 inserted forceTon (@5) between pinnedSelection and userpic.
				{}, // transferId
				gift, // info
				{}, // from
				{}, // date
				{}, // pinnedSelection
				forceTon, // forceTon (v6.0.0)
				{}, // userpic
				{}, // pinned
				{}, // hidden
				true, // resale
...
void UniqueGiftSellBox(
	...
	auto priceInput = HistoryView::AddStarsTonPriceInput(container, {
		...
		// showTon, price, starsMin, starsMax, nanoTonMin, nanoTonMax.
		session, // session
...
void ShowResaleGiftBoughtToast(
	...
	auto config = Ui::Toast::Config();
	config.title = tr::lng_gift_sent_title(tr::now);
	config.text = TextWithEntities{ (to->isSelf()
