...
-> Fn<void(
	...
	return [=](
		...
		auto description = data.text.empty()
			...
			: data.text;
		// XP walk: designated -> positional TextContext (C7555).
		const auto markedContext = Core::TextContext({
		...
		api->request() | rpl::on_error_done([=](QString error) {
			...
			for (const auto &option : options) {
				if (option.currency != kCreditsCurrency) {
					// XP walk: designated -> positional (C7555). GiftTypePremium{cost,currency,stars,months,discountPercent}.
					list.push_back({
						option.cost, // cost
...
void SendGift(
	...
	v::match(details.descriptor, [&](const GiftTypePremium &gift) {
		if (details.byStars && gift.stars) {
			...
			// InvoicePremiumGiftCode{purpose,currency,storeProduct,giveawayCredits,randomId,amount,storeQuantity,users,months};
			// InvoicePremiumGiftCodeUsers{users,boostPeer,message}.
			auto invoice = Payments::InvoicePremiumGiftCode{
				Payments::InvoicePremiumGiftCodeUsers{
					{ peer->asUser() }, // users
					nullptr, // boostPeer
					details.text, // message
				}, // purpose
				Ui::kCreditsCurrency, // currency
				{}, // storeProduct
				{}, // giveawayCredits
				details.randomId, // randomId
				uint64(gift.stars), // amount
				1, // storeQuantity
				1, // users
				gift.months, // months
			};
		...
		Payments::CheckoutProcess::Start(Payments::InvoiceStarGift{
			...
			gift.info.perUserTotal, // perUserLimit (NEW v6.1.1)
			details.anonymous, // anonymous
			details.upgraded, // upgraded
...
void ShowUpgradeGiftedToast(
	...
	if (const auto strong = weak.get()) {
		// XP walk: Toast::Config designated -> named-local (C7555).
		auto config = Ui::Toast::Config();
		config.title = tr::lng_gift_upgrade_gifted_title(tr::now);
		config.text = { (peer->isBroadcast()
...
void AddSoldLeftSlider(
	...
	// XP walk: CreateChild -> Ui::CreateChild (P0846); took theirs (above->parentWidget).
	const auto slider = Ui::CreateChild<RpWidget>(above->parentWidget());
	struct State {
		Text::String still;
		Text::String sold;
		int height = 0;
...
[[nodiscard]] object_ptr<RpWidget> MakePremiumGifts(
	...
	// XP walk: designated -> positional (C7555). GiftsListArgs{window,mode,peer,
	// gifts,selected,loadMore,handler}; not_null members -> must be positional.
	auto result = MakeGiftsList({
		window, // window
		GiftsListMode::Send, // mode
		peer, // peer
...
void GiftBox(
	...
	if (peer->isUser() && !peer->isSelf() && !premiumDisallowed) {
		...
		// XP walk: designated -> positional (C7555; AddBlockArgs contiguous 0-3).
...
[[nodiscard]] CustomList MakeCustomList(
	...
	// XP walk: designated -> positional (C7555). CustomList:
	// content, overrideKey, activate, hasSelection.
	return {
...
void ShowStarGiftBox(
	...
	i->second = Session{ peer }; // XP walk: designated -> positional (C7555)
...
void UpdateGiftSellPrice(
	...
	session->api().request(MTPpayments_UpdateStarGiftPrice(
		...
		session->data().notifyGiftUpdate({
			...
			savedId, // id
			unique->slug, // slug
			Data::GiftUpdate::Action::ResaleChange, // action
		});
	}).fail([=](const MTP::Error &error) {
	...
	return [=](auto consumer) {
		...
		const auto put = [=] {
			...
			consumer.put_next(UniqueGiftCover{ Data::UniqueGift{
				...
				nullptr, // releasedBy
				nullptr, // themeUser
				-1, // nanoTonForResale@11 (TRAP)
				{}, // craftChancePermille@12 (NEW)
				-1, // starsForResale@13 (TRAP)
				-1, // starsForTransfer@14 (TRAP)
				-1, // starsMinOffer@15 (TRAP)
				{}, // number@16
				{}, // onlyAcceptTon@17
				{}, // canBeTheme@18
				{}, // crafted@19 (NEW)
...
void AddUpgradeGiftCover(
	...
	AddUniqueGiftCover(container, std::move(gifts), {
		{}, // pretitle@0
		std::move(numberText), // numberText@1 (NEW v6.5.0)
		std::move(subtitle), // subtitle@2
		hasAll ? showAll : Fn<void()>(), // subtitleClick@3
		hasAll, // subtitleLinkColored@4
		hasAll, // subtitleOutlined@5
		std::move(resalePrice), // resalePrice@6
		std::move(resaleClick), // resaleClick@7
		false, // attributesInfo@8
		{}, // repaintedHook@9
		upgradeSpinner, // upgradeSpinner@10
	});
}
...
Data::CreditsHistoryEntry EntryForUpgradedGift(
	...
	// XP walk: designated -> named-local (C7555); large/sparse CreditsHistoryEntry.
	auto result = Data::CreditsHistoryEntry();
	//result.description = data.message;
	result.date = base::unixtime::parse(gift->date);
	result.credits = CreditsAmount(gift->info.stars);
	result.bareMsgId = uint64(gift->manageId.userMessageId().bare);
	//result.barePeerId = data.fromId.value;
	result.bareGiftStickerId = gift->info.document->id;
	result.bareGiftOwnerId = unique->ownerId.value;
	result.bareGiftHostId = unique->hostId.value;
	//result.bareActorId = data.fromId.value;
	result.bareEntryOwnerId = chatGiftPeer ? chatGiftPeer->id.value : 0;
	result.giftChannelSavedId = gift->manageId.chatSavedId();
	result.stargiftId = gift->info.id;
	result.giftTitle = gift->info.resellTitle;
	result.uniqueGift = unique;
	result.nextToUpgradeStickerId = nextToUpgradeStickerId;
	result.nextToUpgradeShow = nextToUpgradeShow;
	result.craftAnotherCallback = craftAnother;
	result.peerType = Data::CreditsHistoryEntry::PeerType::Peer;
	result.limitedCount = gift->info.limitedCount;
	result.limitedLeft = gift->info.limitedLeft;
	result.starsToUpgrade = int(gift->info.starsToUpgrade);
	result.starsForDetailsRemove = int(gift->starsForDetailsRemove);
	result.giftNumber = unique->number;
	result.converted = false;
	result.stargift = true;
	result.savedToProfile = gift->saved;
	result.in = (unique->ownerId == selfId);
	result.gift = true;
	return result;
}
...
void DefaultGiftHandler(
	...
	if (unique && star->resale) {
		...
		// XP walk: Toast::Config designated -> named-local (C7555).
		auto config = Ui::Toast::Config();
		config.text = tr::lng_gift_sent_finished(
			tr::now,
			lt_count,
			star->info.perUserTotal,
			tr::rich);
...
object_ptr<RpWidget> MakeGiftsList(GiftsListArgs &&args) {
	...
	const auto buttonMode = (mode == GiftsListMode::Craft)
		...
		: GiftButtonMode::Full;
	// XP walk: designated -> positional (C7555). State{delegate,handlerState,...};
	// nested DefaultGiftHandlerState{window,peer,api,...}; not_null members -> positional.
	const auto state = raw->lifetime().make_state<State>(State{
		Delegate(session, buttonMode), // delegate
		{ // handlerState
			window, // window
			peer, // peer
		},
...
void SendGiftBox(
	...
	state->details = GiftSendDetails{
		// XP walk: designated -> positional (C7555). GiftSendDetails:
		// descriptor, text, randomId, anonymous, upgraded, byStars.
		descriptor, // descriptor
		{}, // text
		base::RandomValue<uint64>(), // randomId
		{}, // anonymous
		disallowLimited && (costToUpgrade > 0) && !disallowUnique, // upgraded
	};
	peer->updateFull();
	...
	Emoji::SuggestionsController::Init(
		...
		text,
		session,
		{ true, true, allow }); // XP walk: positional; suggestExactFirstWord(default true), suggestCustomEmoji, allowCustomWithoutPremium
	if (stars) {
	...
	const auto button = box->addButton(rpl::single(QString()), [=] {
		...
		if (stars && stars->info.auction()) {
			const auto bidBox = window->show(MakeAuctionBidBox({
				// XP walk: designated -> positional (C7555). AuctionBidBoxArgs.
				peer, // peer
				window->uiShow(), // show
				state->auction.value(), // state
				std::make_unique<GiftSendDetails>(details), // details
			}));
			bidBox->boxClosing(
			) | rpl::on_next([=] {
				box->closeBox();
	...
	const auto checkAction = [&](const MTPDmessageService &message) {
		...
		action.match([&](const MTPDmessageActionStarGiftUnique &data) {
			if (const auto gift = Api::FromTL(session, data.vgift())) {
				...
				// XP walk: designated -> positional (C7555). GiftUpgradeResult{info,
				// manageId,date,starsForDetailsRemove,saved}; StarGift member -> positional.
					...
					Data::GiftUpgradeResult{
						*gift, // info
						...
						int(
							...
							).value_or_empty()), // starsForDetailsRemove
						data.is_saved(), // saved
					});
			}
		}, [](const auto &) {});
