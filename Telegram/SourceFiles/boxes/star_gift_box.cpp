...
void SendGiftBox(
	...
	state->details = GiftSendDetails{
		...
		{}, // anonymous
		disallowLimited && (costToUpgrade > 0), // upgraded
	};
	peer->updateFull();
			...
			AddUpgradeButton(container, costToUpgrade, peer, [=](bool on) {
				...
				*showing = true;
				ShowStarGiftUpgradeBox({
					// XP walk: designated -> positional (C7555). StarGiftUpgradeArgs:
					// controller, stargiftId, ready, peer, savedId, giftPrepayUpgradeHash, cost.
					window, // controller
					id, // stargiftId
