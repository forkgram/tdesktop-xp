...
void PreviewAiToneExampleCard::showExample(
	...
	const auto context = Core::TextContext({ _session });
...
void ShowToneAddedToast(
	...
	config.title = tr::lng_ai_compose_tone_added(tr::now);
	config.text = tr::lng_ai_compose_tone_added_description(
		tr::now,
		lt_name,
...
void ShowToneRemovedToast(std::shared_ptr<Ui::Show> show, bool deleted) {
	...
	config.icon = &st::aiComposeToneRemovedToastIcon;
	config.duration = kToastDuration;
	show->showToast(std::move(config));
}
...
void PreviewAiToneBox(
	...
	if (!text.empty()) {
		...
		attribution->setClickHandlerFilter([=](
			...
			context.sessionWindow = controller;
