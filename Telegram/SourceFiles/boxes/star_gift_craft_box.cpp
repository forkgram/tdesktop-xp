...
[[nodiscard]] CraftingView MakeCraftingView(
	...
	state->refreshButton = [=](int index) {
		...
		if (entry.gift) {
			...
			entry.button->setDescriptor(GiftTypeStars{
				...
				Data::StarGift{ // info
					entry.gift.unique->initialGiftId, // id
					entry.gift.unique, // unique
					nullptr, // background
					0, // stars
					0, // starsConverted
					0, // starsToUpgrade
					0, // starsResellMin
					entry.gift.unique->model.document, // document
				},
	...
	return {
		...
		state->editRequests.events(), // editRequests
		state->removeRequests.events(), // removeRequests
		std::move(grabForAnimation), // grabForAnimation
	};
}
	...
	auto gifts = rpl::single(
		...
		for (const auto &gift : state->data.list) {
			result.list.push_back(Info::PeerGifts::GiftTypeStars{
				...
				nullptr, // from
				0, // date
				false, // pinnedSelection
				false, // forceTon
				false, // userpic
				false, // pinned
				false, // hidden
				true, // resale
				(gift.info.unique->ownerId == selfId), // mine
				});
		}
...
void ShowSelectGiftBox(
	...
	state->resaleLifetime = Data::ResaleGiftsSlice(
		session,
		giftId,
		{ 0, {}, Data::ResaleGiftsSort::Price, true }
	...
	auto startRequest = [=](Fn<void(CraftResult)> done) {
		...
		base::call_delayed(delay, box, [=] {
			...
			if (shouldSucceed && count > 0) {
				...
				auto info = Data::StarGift{
					chosen.unique->initialGiftId, // id
					chosen.unique, // unique
					nullptr, // background
					0, // stars
					0, // starsConverted
					0, // starsToUpgrade
					0, // starsResellMin
					chosen.unique->model.document, // document
				};
				auto result = std::make_shared<Data::GiftUpgradeResult>(
					Data::GiftUpgradeResult{
						std::move(info), // info
						chosen.manageId, // manageId
					});
		...
		session->api().request(MTPpayments_CraftStarGift(
			...
			if (type.startsWith(waitPrefix)) {
					...
					type.mid(waitPrefix.size()).toInt(), // seconds
				});
			} else {
		...
		for (const auto &gift : gifts) {
			if (gift.unique) {
				cs->lostGifts.push_back({
					-1, // cornerIndex
					u"#"_q + Lang::FormatCountDecimal(gift.unique->number), // number
				});
			}
		}
...
void ShowGiftCraftInfoBox(
	...
	controller->show(Box([=](not_null<GenericBox*> box) {
		...
		auto coverArgs = UniqueGiftCoverArgs();
		coverArgs.subtitle = tr::lng_gift_craft_info_about(tr::marked);
		AddUniqueGiftCover(container, std::move(cover), std::move(coverArgs));
		AddSkip(container);
...
void ShowCraftLaterError(
	...
	args.text = tr::lng_gift_craft_when(
		lt_date,
