	...
	if (_state->attrs) {
		...
		_state->attrs->paintOn([this, astate, attrsHeight](QPainter &p) {
			...
			const auto paint = [&](int x, const AttributeState &state) {
				...
				nameContext.position = QPoint(
					x + (single - namew) / 2,
					innert + padding.top());
				nameContext.availableWidth = namew;
				nameContext.elisionLines = 1;
				state.name.draw(p, nameContext);
				p.setPen(_state->released.fg);
				const auto typew = std::min(inner, state.type.maxWidth());
				auto typeContext = Text::PaintContext();
				typeContext.position = QPoint(
					x + (single - typew) / 2,
					innert + padding.top() + state.name.minHeight());
				typeContext.availableWidth = typew;
				state.type.draw(p, typeContext);
				p.setPen(Qt::NoPen);
				...
				state.percent.draw(p, percentContext);
			};
...
void UniqueGiftCoverWidget::paintEvent(QPaintEvent *e) {
	...
	const auto context = PaintContext{
		width(), // width
		st::uniqueGiftSubtitleTop, // patternAreaHeight
	};
...
bool UniqueGiftCoverWidget::paintModel(
	...
	const auto request = Lottie::FrameRequest{
		QSize(lottieSize, lottieSize) * factor, // box
	};
...
QRect UniqueGiftCoverWidget::prepareCraftFrame(
	...
	paintPattern(p, _state->now.pattern, _state->now.backdrop, {
		size.width() + 2 * skip, // width
		pointsHeight, // patternAreaHeight
	}, bgProgress);
...
bool UniqueGiftCoverWidget::paintGift(
	...
	if (gift.gift->crafted) {
		...
		auto badge = Info::PeerGifts::GiftBadge{
			tr::lng_gift_crafted_tag(tr::now), // text
			gift.gift->backdrop.edgeColor, // bg1
			gift.gift->backdrop.patternColor, // bg2
			QColor(0, 0, 0, 0), // border
			gift.gift->backdrop.textColor, // fg
		};
