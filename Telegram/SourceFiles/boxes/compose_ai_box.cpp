...
[[nodiscard]] Ui::LabeledEmojiTab ResolveStyleDescriptor(
	...
	return {
		tone.isDefault ? tone.defaultType : QString::number(tone.id), // id
		tone.title, // label
		{}, // emoji
		tone.emojiId
...
[[nodiscard]] std::vector<Ui::LabeledEmojiTab> ResolveTranslateStyleDescriptors(
	...
	result.push_back({
		QString(), // id
		tr::lng_ai_compose_style_neutral(tr::now), // label
		{}, // emoji
		Data::SerializeCustomEmojiId(neutral->id), // customEmojiData
	});
...
-> std::vector<Ui::LabeledEmojiTab> {
	tabs.push_back({ // XP walk: designated -> positional (C7555)
		u"_add_style"_q, // id
		tr::lng_ai_compose_tone_create(tr::now), // label
		nullptr, // emoji
		QString(), // customEmojiData
		&st::aiComposeAddStyleIcon, // icon
		&st::aiComposeAddStyleIconOver, // iconActive
	});
	return tabs;
}
...
ComposeAiPreviewCard::ComposeAiPreviewCard(
	...
	std::shared_ptr<const Iv::RichPage> richSource)
: RpWidget(parent)
, _context(Core::TextContext({ session }))
		...
		label->setClickHandlerFilter([=](
			...
			if (dynamic_cast<Ui::Text::PreClickHandler*>(handler.get())) {
				...
				ActivateClickHandler(label, handler, ClickContext{
					button, // button
					QVariant::fromValue(context), // other
				});
				return false;
			}
			return true;
...
, _authorLabel(Ui::CreateChild<Ui::FlatLabel>(
	...
	_authorLabel->setClickHandlerFilter([=](
			...
			Qt::MouseButton button) {
		if (dynamic_cast<Ui::Text::PreClickHandler*>(handler.get())) {
			// XP walk: designated -> named-local + positional (C7555).
			auto context = ClickHandlerContext();
			context.show = show;
...
void ComposeAiContent::request() {
	...
	auto request = Api::ComposeWithAi::Request{
		...
		{}, // changeTone
		{}, // proofread
		(_mode != ComposeAiMode::Fix) && _emojify, // emojify
	};
	switch (_mode) {
	case ComposeAiMode::Translate: {
	...
	if (const auto user = _session->data().userLoaded(authorId)) {
		...
		_authorLabel->setMarkedText(
			tr::lng_ai_compose_author(
				...
				std::move(mention),
				tr::marked),
			Core::TextContext({ _session })); // XP walk: designated -> positional
		...
		*sendButton = nullptr;
		...
		if (*premiumFlooded) {
			...
			if (args.send) {
				...
				send->setState({ Ui::SendButton::Type::Send });
