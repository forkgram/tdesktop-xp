	...
	if (const auto window = Core::App().findWindow(box)) {
		...
		if (controller) {
			const auto chatStyle = InitMessageFieldHandlers({
				...
				&st.files.caption, // fieldStyle
			});
			const auto aiButton = Ui::SetupCaptionAiButton({
				field->parentWidget(), // parent
				field, // field
				&controller->session(), // session
				controller->uiShow(), // show
				chatStyle, // chatStyle
...
: SendFilesBox(nullptr, {
	...
	std::move(list), // list
	caption, // caption
	toPeer, // captionToPeer
	DefaultLimitsForPeer(toPeer), // limits
	DefaultCheckForPeer(controller, toPeer), // check
	sendType, // sendType
...
bool SendFilesBox::checkWith(
	...
	// range-v3 0.12 lacks a working views::concat here; iterate each list.
