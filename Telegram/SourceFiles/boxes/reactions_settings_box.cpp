...
void ReactionsSettingsBox(
	...
	state->refresh = [=] {
		...
		const auto &active = reactions->list(Data::Reactions::Type::Active);
		// XP walk: range-v3 0.12's concat view fails on MSVC 14.16 (C2672)
		// -> walk the three lists in order, stopping once the slots are full.
		const auto fill = [&](const auto &from) {
			for (const auto &reaction : from) {
	...
	const auto startFly = [=](
		...
		state->fly = std::make_unique<Ui::ReactionFlyAnimation>(
			...
			Ui::ReactionFlyAnimationArgs{
				...
				// ReactionFlyAnimationArgs: id0, flyIcon1, flyFrom2, flyUp3,
				// scaleOutDuration4, scaleOutTarget5, miniCopyMultiplier6
				// (default 1., NOT {}), centerSizeMultiplier7, flyKeepSize8.
				id, // id
				from.frame, // flyIcon
				state->flyLayer->mapFromGlobal(
					from.globalStartGeometry), // flyFrom
				st::settingsReactionsFlyUp, // flyUp
				{}, // scaleOutDuration
				{}, // scaleOutTarget
				1., // miniCopyMultiplier
				kCenterSizeMultiplier, // centerSizeMultiplier
				true, // flyKeepSize
			},
