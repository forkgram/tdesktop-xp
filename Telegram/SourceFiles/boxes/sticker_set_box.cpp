		...
		const auto fillSetCreatorFooter = [&] {
			...
			return Filler([=, show = _show, set = _set](
				...
				const auto deleteEveryone = [=] {
					...
					args.text = tr::lng_stickers_delete_pack_sure(tr::now);
				...
				args.text = tr::lng_stickers_context_delete_pack_everyone(
					tr::now);
				args.handler = deleteEveryone;
				args.icon = &st::menuIconDeleteAttention;
				args.isAttention = true;
				addSub(std::move(args));
				sub->addAction(
...
-> Ui::MessageSendingAnimationFrom {
	...
	return {
		...
		_session->data().nextLocalMessageId(), // localId
		mapToGlobal( // globalStartGeometry
	...
	if (setType() == Data::StickersType::Emoji) {
		...
		if (!amSetCreator()) {
			...
			const auto addAction = Ui::Menu::CreateAddActionCallback(
				_menu.get());
			// XP walk: designated -> named-local (C7555; MenuCallback::Args sparse).
			auto args = Ui::Menu::MenuCallback::Args();
			args.text = tr::lng_emoji_context_delete(tr::now);
			args.handler = [index, this, show = _show] {
		...
		ChatHelpers::TabbedPanelDescriptor{
			...
			object_ptr<Selector>( // ownedSelector
				...
				ChatHelpers::TabbedSelectorDescriptor{
					...
					Selector::Mode::StickersOnly, // mode
					{}, // customTextColor
					{}, // features
					_setId, // excludeStickerSetId
				}),
		});
