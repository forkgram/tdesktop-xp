...
const auto kGiftAnimations = std::array<GiftAnimationConfig, 7>{{ {
	DecayingEndRotation({ 4.1, -8.2 }, 0.98, { 1, -2 }, 2200), // rotation
	2200, // duration
	600, // revealTime
	1, // targetFaceIndex
	180, // targetFaceRotation
}, {
	DecayingRotation({ 3.2, -5.9 }, 0.97), // rotation
	1200, // duration
	0, // revealTime
	4, // targetFaceIndex
	180, // targetFaceRotation
}, {
	DecayingEndRotation({ 6.2, 7.8 }, 0.98, { 2, 1 }, 2200), // rotation
	2200, // duration
	1000, // revealTime
	1, // targetFaceIndex
	0, // targetFaceRotation
}, {
	DecayingRotation({ 7.0, 4.5 }, 0.97), // rotation
	1200, // duration
	200, // revealTime
	5, // targetFaceIndex
	0, // targetFaceRotation
}, {
	DecayingEndRotation({ -6.5, 5.0 }, 0.98, { 0, 1 }, 2200), // rotation
	2200, // duration
	800, // revealTime
	1, // targetFaceIndex
	0, // targetFaceRotation
}, {
	DecayingRotation({ -2.5, 3.5 }, 0.98), // rotation
	1200, // duration
	200, // revealTime
	2, // targetFaceIndex
	180, // targetFaceRotation
}, {
	DecayingEndRotation({ -4.4, -8.2 }, 0.98, { 0, -1.5 }, 2200), // rotation
	2200, // duration
	600, // revealTime
	3, // targetFaceIndex
	0, // targetFaceRotation
...
std::unique_ptr<CraftFailAnimation> SetupFailureAnimation(
	...
	result->lottie = Lottie::MakeIcon({
		u"craft_failed"_q, // name
		QString(), // path
		QByteArray(), // json
		nullptr, // color
		st::craftFailLottieSize, // sizeOverride
	});
	return result;
}
...
[[nodiscard]] QRect PrepareCraftFrame(
	...
	return success->widget->prepareCraftFrame(success->frame, {
		shared->forgeRect.size(), // initial
		shared->forgeSides[faceIndex].bg, // initialBg
...
void PaintLoadingAnimation(
	...
	const auto adjustedState = RadialState{
		radial.shown * loadingShown, // shown
		radial.arcFrom, // arcFrom
		radial.arcLength, // arcLength
	};
...
void SetupCraftProgressTitle(
	...
	auto owned = Lottie::MakeIcon({
		u"craft_progress"_q, // name
		QString(), // path
		QByteArray(), // json
		nullptr, // color
		iconSize, // sizeOverride
		0, // frame
		true, // limitFps
	});
	...
	if (startRequest) {
		...
		startRequest([=](CraftResult result) {
			if (!weak) {
				...
				box->uiShow()->show(MakeInformBox(ConfirmBoxArgs{
					v::get<CraftResultError>(result).type, // text
				}));
				box->closeBox();
				return;
			...
			if (const auto result = state->craftResult->get()) {
				...
				>(CraftDoneAnimation{
					MakeUniqueGiftCover(
						...
						{ {}, std::move(numberText) }), // args: pretitle@0, numberText@1
				});
				const auto success = state->successAnimation.get();
