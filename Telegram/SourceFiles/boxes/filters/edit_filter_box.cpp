...
void EditFilterBox(
	...
	// XP walk: designated -> named-local (C7555; State non-contiguous, skips links/hasLinks).
	auto initial = State();
	initial.rules = filter;
	initial.chatlist = filter.chatlist();
	initial.creating = filter.title().empty();
	initial.title = filter.titleText();
	initial.staticTitle = filter.staticTitle();
	const auto state = box->lifetime().make_state<State>(std::move(initial));
	...
	state->staticTitle.value() | rpl::start_with_next([=](bool value) {
		...
		name->setCustomTextContext([=](Fn<void()> repaint) {
			...
			result.customEmojiLoopLimit = value ? -1 : 0;
...

	{
		...
		// XP walk: designated -> named-local (C7555; MarkedTextContext skips .type).
		auto tagContext = Core::MarkedTextContext();
		tagContext.session = session;
		tagContext.customEmojiRepaint = [] {};
		tag->context.textContext = tagContext;
