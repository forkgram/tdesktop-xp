...
void ShowProxyQrBox(std::shared_ptr<Ui::Show> show, const QString &link) {
	show->showBox(Box([=](not_null<Ui::GenericBox*> box) {
		...
		const auto copyCallback = [=, image = ProxyQrForShare(link)] {
			...
			toast.text = { tr::lng_group_invite_qr_copied(tr::now) }; // text
...
void ProxiesBox::setupTopButton() {
	...
	top->setClickedCallback([=] {
		*menu = base::make_unique_q<Ui::PopupMenu>(
		...
		// XP walk: designated init -> named-local (C7555; MenuCallback::Args sparse).
		auto args = Ui::Menu::MenuCallback::Args();
		args.text = tr::lng_proxy_add_from_clipboard(tr::now);
		args.handler = [=] { AddProxyFromClipboard(_controller, uiShow()); };
		args.icon = &st::menuIconImportTheme;
		addAction(std::move(args));
		if (!_rows.empty()) {
			auto deleteArgs = Ui::Menu::MenuCallback::Args();
			deleteArgs.text = tr::lng_group_invite_context_delete_all(tr::now);
			deleteArgs.handler = [=] { _controller->deleteItems(); };
...
void ProxiesBoxController::shareItems() {
	...
	auto toast = Ui::Toast::Config();
	toast.text = { tr::lng_proxy_edit_share_list_toast(tr::now) }; // text
	toast.iconLottie = u"toast/copy"_q; // iconLottie
	toast.iconLottieSize = st::toastLottieIconSize; // iconLottieSize
	_show->showToast(std::move(toast));
}
