...
void BackgroundBox::chooseFromFile() {
	...
	const auto callback = [=](const FileDialog::OpenResult &result) {
		...
		auto image = Images::Read({
			result.paths.isEmpty() ? QString() : result.paths.front(), // path
	...
	if (chosenDefaultForPeer(paper)) {
		if (!hasDefaultForPeer()) {
			...
			_controller->show(Ui::MakeConfirmBox({
				tr::lng_background_sure_reset_default(), // text
				reset, // confirmed
				v::null, // cancelled
				tr::lng_background_reset_default(), // confirmText
			}));
		} else {
			closeBox();
		}
...
void BackgroundBox::removePaper(const Data::WallPaper &paper) {
	...
	_controller->show(Ui::MakeConfirmBox({
		tr::lng_background_sure_delete(), // text
		remove, // confirmed
		v::null, // cancelled
