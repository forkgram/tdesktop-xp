...
[[nodiscard]] std::shared_ptr<EditorState> PrepareEditorState(
	...
	return std::make_shared<EditorState>(EditorState{
		std::make_shared<Image>(std::move(canvas)), // canvas
		Editor::PhotoModifications{
			{}, // angle
			{}, // flipped
			QRect(0, 0, side, side), // crop
			Editor::EditorData::CropType::Rect, // cropType
			Editor::EditorData::CropMode::Hint, // cropMode
			Editor::RoundedCornersLevel::Large, // cornersLevel
...
void ShowPhotoEditor(
	...
	auto editor = base::make_unique_q<Editor::PhotoEditor>(
		...
		Editor::EditorData{
			{}, // about
			{}, // confirm
			QSize(side, side), // exactSize
			Editor::EditorData::CropType::RoundedRect, // cropType
			Editor::EditorData::CropMode::Mask, // cropMode
...
void CreateMediaBox(
	...
	auto pickerDescriptor = ChatHelpers::EmojiPickerOverlayDescriptor{
		(isEmoji
			...
			: tr::lng_stickers_create_emoji_about(tr::now)), // aboutText
		{}, // recent
		kMaxEmojis, // maxSelected
		true, // allowExpand
		std::move(args.emoji), // initialSelected
	};
