...
base::options::toggle ModerateCommonGroups({
	kModerateCommonGroups, // id
	"Ban users from several groups at once.", // name
});
...
ModerateOptions CalculateModerateOptions(const HistoryItemsList &items) {
	...
	auto result = ModerateOptions{ // XP walk: designated -> positional (C7555)
		false, // reportSpam
		true, // deleteAllMessages
		false, // deleteAllReactions
		true, // banOrRestrict
	};
...
ModerateOptions CalculateModerateOptions(const ModerateReactionEntry &reaction) {
	auto result = ModerateOptions{ // XP walk: designated -> positional (C7555)
		false, // reportSpam
		false, // deleteAllMessages
		false, // deleteAllReactions
		false, // banOrRestrict
		{ reaction.participant }, // participants
	};
...
void CreateModerateMessagesBox(
	...
	if (showMessagesCheckbox || showReactionsCheckbox) {
		...
		if (useSingleDeleteOptions) {
			...
			deleteMessagesController = box->lifetime().make_state<Controller>(
				Controller::Data{ // XP walk: designated -> positional (C7555)
					deleteMessagesCounts->value(), // messagesCounts
					Participants{ participant }, // participants
					checkedParticipants, // checked
				});
			deleteReactionsController = box->lifetime().make_state<Controller>(
				Controller::Data{ // XP walk: designated -> positional (C7555)
					nullptr, // messagesCounts
					Participants{ participant }, // participants
					checkedParticipants, // checked
				});
	...
	const auto makeTitleLoadingDescriptor = [] {
		return Lottie::IconDescriptor{ // XP walk: designated -> positional (C7555)
	...
	const auto makeMessageTitleData = [=](
		...
		auto result = MessageTitleData{ // XP walk: designated -> positional (C7555)
			baseMessagesCount, // count
			true, // resolved
		};
	...
	auto titleContext = Core::TextContext({ session }); // XP walk: designated -> positional (C7555)
	...
	if (banOrRestrict) {
		...
		if (ownedWrap) {
			...
			auto [checkboxes, getRestrictions, changes, highlightWidget] = CreateEditRestrictions(
				...
				prepareFlags,
				disabledMessages,
				{ peer->isForum(), true }); // XP walk: designated -> positional (RestrictionsSetOptions{isForum, isUserSpecific})
