...
// XP walk: take theirs - AddEmojiToggleToField was moved upstream to
// ui/controls/emoji_button_factory.{h,cpp} (Ui::AddEmojiToggleToField). That
// header is already #included here and the callers below use the Ui:: form,
// so this local anonymous-namespace copy is removed to avoid a duplicate.
...
void ShowMediaUploadingToast() {
	...
	config.title = tr::lng_polls_media_uploading_toast_title(tr::now);
	config.text = tr::lng_polls_media_uploading_toast(tr::now, tr::marked);
	config.iconLottie = u"uploading"_q;
	config.iconLottieSize = st::pollToastUploadingIconSize;
...
object_ptr<Ui::RpWidget> CreatePollBox::setupContent() {
		...
		state->prepareQueue->addTask(std::make_unique<PreparePoll>(
			...
			[=](std::shared_ptr<FilePrepareResult> prepared) {
				...
				state->uploads.emplace(uploadId, UploadContext{
					...
					prepared->filename, // filename
					prepared->filemime, // filemime
					std::move(attributes), // attributes
					false, // forceFile
				});
				media->uploadDataId = prepared->id;
				_controller->session().uploader().upload(
	...
	const auto showMediaMenu = [=](
		...
		if (HistoryView::ShowPollMediaPreview(_controller, media, {
			...
			[=] { chooseDocument(media); }, // chooseDocument
			[=] {
	...
	const auto showWhoVoted = (!(_disabled & PollData::Flag::PublicVotes))
		? AddPollToggleButton(
			...
			{
				&st::pollBoxFilledPollViewIcon, // icon
				Settings::IconType::Rounded, // type
				&st::settingsIconBg4, // background
			},
	...
	const auto multiple = AddPollToggleButton(
		...
		{
			&st::pollBoxFilledPollMultipleIcon, // icon
			Settings::IconType::Rounded, // type
			&st::settingsIconBg3, // background
		},
	...
	const auto addOptions = (!(_disabled & PollData::Flag::OpenAnswers))
		? AddPollToggleButton(
			...
			{
				&st::pollBoxFilledPollAddIcon, // icon
				Settings::IconType::Rounded, // type
				&st::settingsIconBg4, // background
			},
	...
	const auto revoting = AddPollToggleButton(
		...
		{
			&st::pollBoxFilledPollRevoteIcon, // icon
			Settings::IconType::Rounded, // type
			&st::settingsIconBg6, // background
		},
	...
	const auto shuffle = AddPollToggleButton(
		...
		{
			&st::pollBoxFilledPollShuffleIcon, // icon
			Settings::IconType::Rounded, // type
			&st::settingsIconBg8, // background
		},
	...
	const auto quiz = AddPollToggleButton(
		...
		{
			&st::pollBoxFilledPollCorrectIcon, // icon
			Settings::IconType::Rounded, // type
			&st::settingsIconBg2, // background
		},
	...
	const auto restrictToSubscribers = isBroadcastChannel
		? AddPollToggleButton(
			...
			{
				&st::pollBoxFilledPollSubscribersIcon, // icon
				Settings::IconType::Rounded, // type
				&st::settingsIconBg5, // background
			},
	...
	const auto limitByCountry = isBroadcastChannel
		? AddPollToggleButton(
			...
			{
				&st::pollBoxFilledPollCountryIcon, // icon
				Settings::IconType::Rounded, // type
				&st::settingsIconBg4, // background
			},
			rpl::single(false),
	...
	const auto duration = AddPollToggleButton(
		...
		{
			&st::pollBoxFilledPollDeadlineIcon, // icon
			Settings::IconType::Rounded, // type
			&st::settingsIconBg1, // background
		},
		rpl::single(false),
		st::detailedSettingsButtonStyle);
			...
			[=] {
				...
				show->show(Box([=](not_null<Ui::GenericBox*> box) {
					Ui::ChooseDateTimeBox(box, {
						tr::lng_polls_create_deadline_title(), // title
						tr::lng_polls_create_deadline_button(), // submit
						[=](TimeId time) {
							state->closeDate = time;
							state->closePeriod = 0;
							box->closeBox();
						}, // done
						[=] { return base::unixtime::now() + 60; }, // min
						current, // time
						[=] {
