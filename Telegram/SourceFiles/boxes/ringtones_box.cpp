...
void RingtonesBox(
	...
	const auto state = container->lifetime().make_state<State>(State{
		{}, // creator
		std::make_shared<Ui::RadiobuttonGroup>(), // group
		{}, // medias
		selected, // chosen
	});
	...
	session->api().ringtones().uploadDones(
		...
		state->chosen = Data::NotifySound{ {}, {}, id };
		rebuild();
	}, container->lifetime());
	...
	box->addButton(tr::lng_settings_save(), [=] {
		...
		auto sound = (value == kDefaultValue)
			? Data::NotifySound()
			: (value == kNoSoundValue)
			// XP walk: NotifySound designated -> positional (id@2, none@3);
			// took theirs' presavedVolume save block.
			? Data::NotifySound{ {}, {}, {}, true }
			: Data::NotifySound{ {}, {}, state->medias[value]->owner()->id };
