...
[[nodiscard]] QImage PrepareScaledNonPattern(
	...
	return Images::Prepare(image, QSize(takeWidth, takeHeight) * ratio, { {}, Images::Option::TransparentBackground | blur, { size, size } });
}
...
-> OverridenStyle {
	...
	auto result = OverridenStyle{
		// XP walk: designated -> positional (C7555); v4.12.0 changed the
		// subtitle style settingsSubsectionTitle -> defaultSubsectionTitle.
		st::defaultBox, // box
		toggle, // toggle
		st::defaultContinuousSlider, // slider
		st::defaultSubsectionTitle, // subtitle
	};
