...
void ShareBox::submit(Api::SendOptions options) {
	...
	const auto checkPaid = [=] {
		...
		if (!waiting.empty()) {
			...
			// SendPaymentDetails{messages,stars}; PaidConfirmStyles{label,checkbox}.
			ShowSendPaidConfirm(sessionShow, paid, SendPaymentDetails{
				...
				(scheduleBoxSt // label
					...
					: nullptr),
				_descriptor.st.checkbox, // checkbox
			});
			return false;
		}
		return true;
...
ChatHelpers::ForwardedMessagePhraseArgs CreateForwardedMessagePhraseArgs(
	...
	return {
		// XP walk: designated -> positional (C7555). ForwardedMessagePhraseArgs:
		// toCount, singleMessage, to1, to2.
		result.size(), // toCount
		(msgIds.size() <= 1), // singleMessage
		(toCount > 0) ? result.front()->peer().get() : nullptr, // to1
		(toCount > 1) ? result[1]->peer().get() : nullptr, // to2
	};
}
...
void ShowForwardedMessageToast(
	...
	base::call_delayed(st::boxDuration, session, [=] {
		...
		config.text = phrase;
		config.filter = ChatHelpers::ForwardedToSavedMessagesFilter(session);
		config.iconLottie = icon;
...
void FastShareMessage(
	...
	show->show(Box<ShareBox>(ShareBox::Descriptor{
		...
		ShareBox::DefaultForwardCountMessages( // countMessagesCallback
			history,
			msgIds),
		ShareBox::DefaultForwardCallback( // submitCallback
			show,
			history,
			msgIds),
		...
		{}, // videoTimestamp
		{ // forwardOptions
			ItemsForwardSendersCount(items),
			ItemsForwardCaptionsCount(items),
		...
		[=] {
			...
			if (!phrase.empty()) {
				...
				config.filter = ChatHelpers::ForwardedToSavedMessagesFilter(
					&show->session());
				config.iconLottie = ChatHelpers::ForwardedMessagePhraseIcon(
					donePhraseArgs);
				config.iconLottieSize = st::toastLottieIconSize;
				show->showToast(std::move(config));
			}
