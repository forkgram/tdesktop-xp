...
struct BidSliderValues {
	...
	// XP walk: defaulted operator== (C++20) -> manual.
	friend inline bool operator==(
			const BidSliderValues &a,
			const BidSliderValues &b) {
		return (a.min == b.min)
			&& (a.explicitlyAllowed == b.explicitlyAllowed)
			&& (a.max == b.max);
	}
};
...
object_ptr<RpWidget> MakeAuctionInfoBlocks(
	...
	// XP walk: designated -> positional (C7555); StarSelectInfoBlock{title,subtext,click}.
	return MakeStarSelectInfoBlocks(box, {
		{
			std::move(bidTitle), // title
			tr::lng_auction_bid_minimal(
				lt_count,
				std::move(minimal)), // subtext
			setMinimal, // click
		},
		{
...
base::weak_qptr<BoxContent> ChooseAndShowAuctionBox(
	...
	if (showBidBox) {
		...
		box = window->show(Box([=](not_null<GenericBox*> box) {
			...
			ConfirmBox(box, std::move(confirmArgs));
		}));
	} else if (showInfoBox) {
		box = window->show(Box(
			AuctionInfoBox,
...
Fn<void()> ActiveAuctionsCallback(
	...
	const auto singleFrom = [](const Data::GiftAuctionState &state) {
		// XP walk: designated -> positional (C7555). Single: giftId0,document1,round2,total3,bid4,position5,winning6,ends7 (all contiguous).
		return Single{
			state.gift->id, // giftId
