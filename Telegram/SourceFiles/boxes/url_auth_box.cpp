	...
	const auto sendRequest = [=](Result result) {
		if (!result.auth) {
			...
			currentSession->api().request(MTPmessages_AcceptUrlAuth(
				...
				config.title = tr::lng_url_auth_phone_toast_good_title(tr::now);
				config.text = ((requestPhone && !sharePhone)
					...
					: tr::lng_url_auth_phone_toast_good)(
						...
						tr::link(domain),
						tr::marked);
				config.duration = crl::time(4000);
				show->showToast(std::move(config));
			}).fail([=] {
				auto config = Ui::Toast::Config();
				config.title = tr::lng_url_auth_phone_toast_bad_title(tr::now);
				config.text = tr::lng_url_auth_phone_toast_bad(
					tr::now,
					lt_domain,
					tr::link(domain),
					tr::marked);
	...
	*box = show->show(Box([=](not_null<Ui::GenericBox*> box) {
		const auto callback = [=](Result result) {
			...
			box->uiShow()->show(Box([=](not_null<Ui::GenericBox*> box) {
				...
				args.text = tr::lng_url_auth_phone_sure_text(
					lt_domain,
					rpl::single(tr::bold(capitalized(domain))),
					lt_phone,
					PhoneValue(currentSession->user()),
					tr::rich);
				args.confirmed = confirm(true);
				args.cancelled = confirm(false);
				args.confirmText = tr::lng_allow_bot();
				args.cancelText = tr::lng_url_auth_phone_sure_deny();
