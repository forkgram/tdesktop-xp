...
void BoostBox(
	...
	// XP walk: take theirs (already C++17). HEAD's State{ data.boost.mine } was
	// stale vs the merged State { rpl::variable<BoostCounters> data; ... }.
...
void BoostBoxAlready(not_null<GenericBox*> box, bool group) {
	ConfirmBox(box, {
		...
		// cancelStyle, labelStyle, labelFilter, labelPadding, title, inform.
		(group
			? tr::lng_boost_error_already_text_group
			: tr::lng_boost_error_already_text)(Text::RichLangValue), // text
		{}, // confirmed
		{}, // cancelled
		{}, // confirmText
		{}, // cancelText
...
void GiftedNoBoostsBox(not_null<GenericBox*> box, bool group) {
	InformBox(box, {
		...
		(group
			...
			: tr::lng_boost_error_gifted_text)(Text::RichLangValue), // text
		{}, // confirmed
		{}, // cancelled
		{}, // confirmText
		{}, // cancelText
...
void PremiumForBoostsBox(
	...
	ConfirmBox(box, {
		...
		(group
			...
			: tr::lng_boost_error_premium_text)(Text::RichLangValue), // text
		buyPremium, // confirmed
		{}, // cancelled
...
void AskBoostBox(
	...
	// XP walk: take theirs - removed HEAD's unused State (dead code in AskBoostBox).
	FillBoostLimit(
		BoxShowFinishes(box),
		box->verticalLayout(),
...
void FillBoostLimit(
	...
	auto bubbleRowState = rpl::duplicate(
		...
		return Premium::BubbleRowState{
			// XP walk: designated -> positional (C7555); HEAD used stale `index`.
			// BubbleRowState: counter, ratio, animateFromZero, dynamic.
			counters.boosts, // counter
			ratio(counters), // ratio
			(counters.level != previous.level), // animateFromZero
			true, // dynamic
		};
	});
	...
	auto limitState = std::move(
		...
		return Premium::LimitRowState{
			// XP walk: designated -> positional (C7555).
			// LimitRowState: ratio, animateFromZero, dynamic.
			state.ratio, // ratio
			state.animateFromZero, // animateFromZero
	...
	Premium::AddLimitRow(
		...
		Premium::LimitRowLabels{
			// XP walk: designated -> positional (C7555); take theirs' rpl producers.
			// LimitRowLabels: leftLabel, leftCount, rightLabel, rightCount (no dynamic).
