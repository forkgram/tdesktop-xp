	...
	raw->paintOn([=](QPainter &p) {
		...
		const auto ttop = etop + esize;
		{
			...
			for (auto i = 0; i != 7; ++i, left += width + border) {
				...
				auto paintContext = Text::PaintContext();
				paintContext.position = { x + (w - mul.maxWidth()) / 2, y + ttop };
				mul.draw(p, paintContext);
			}
		}
	});
...
void EmojiGameStakeBox(
	...
	const auto sixText = helper.image({
		MakeEmojiFrame(6, st::emojiSize), // image
		{}, // margin
		false, // textColor
	});
