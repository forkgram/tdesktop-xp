...
bool RoundVideoRecorder::Private::initAudio() {
	...
	_swrContext = MakeSwresamplePointer(
		// XP walk: old-API MakeSwresamplePointer takes uint64_t layouts by
		// value (not AVChannelLayout*); keep ours, drop theirs pointer form.
...
RoundVideoResult RoundVideoRecorder::Private::finish() {
	...
	// Take theirs _result.content (WriteBytesWrap now wraps a .content).
