...
void UserpicButton::choosePhotoLocally() {
	...
	const auto editorData = [=](ChosenType type) {
		...
		const auto phrase = (type == ChosenType::Suggest)
			...
			: nullptr;
		return Editor::EditorData{ // XP walk: designated -> positional (C7555)
			(phrase
				? (*phrase)(
					tr::now,
					lt_user,
		...
		base::call_delayed(
			...
			crl::guard(this, [=] {
				PrepareProfilePhotoFromFile(
					this,
					_window,
					editorData(type), // XP walk: take theirs helper (editorData @L281)
					callback(type));
			}));
	};
	const auto user = _peer ? _peer->asUser() : nullptr;
