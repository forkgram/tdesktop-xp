...
[[nodiscard]] Fn<QImage()> CustomEmojiBadgeFactory(
	...
	return [=] {
		...
		pcontext.position = { padding.left(), padding.top() };
		string.draw(p, pcontext);
...
void FillRatingLimit(
	...
	auto bubbleRowState = rpl::duplicate(
		...
		// XP walk: designated -> positional (C7555); BubbleRowState
		// counter@0, ratio@1, animateFromZero@2, dynamic@3.
		return Premium::BubbleRowState{
			counters.stars,
			ratio(counters),
			(counters.level != previous.level),
			true,
		};
	});
	...
	auto limitState = std::move(
		...
		// XP walk: designated -> positional (C7555); LimitRowState
		// ratio@0, animateFromZero@1, dynamic@2 (v6.0.2 negative-aware).
		return Premium::LimitRowState{
			negative ? 0.5 : state.ratio,
	...
	Premium::AddLimitRow(
		...
		// XP walk: designated -> positional (C7555); LimitRowLabels non-contiguous
		// (leftLabel@0, leftCount@1, rightLabel@2, rightCount@3, activeLineBg@4); v6.0.2 negative-aware.
		Premium::LimitRowLabels{
			std::move(left), // leftLabel@0
			{}, // leftCount@1
			std::move(right), // rightLabel@2
			{}, // rightCount@3
			[=] { return negative // activeLineBg@4
...
void StarsRating::paint(QPainter &p) {
	...
	// (position, then availableWidth, skipping outerWidth).
	auto context = Ui::Text::PaintContext();
	context.position = QPoint(x, 0) + _shape->position;
	context.availableWidth = _collapsedText.maxWidth();
	_collapsedText.draw(p, context);
}
