...
void FillRatingLimit(
	...
	auto limitState = std::move(
		...
		// ratio@0, animateFromZero@1, dynamic@2.
	...
	Premium::AddLimitRow(
		...
		// (leftLabel@0, leftCount@1, rightLabel@2, rightCount@3, activeLineBg@4).
		Premium::LimitRowLabels{
			std::move(left), // leftLabel@0
			{}, // leftCount@1
			std::move(right), // rightLabel@2
			{}, // rightCount@3
			[=] { return st::windowBgActive->b; }, // activeLineBg@4
		},
		std::move(limitState),
		limitLinePadding);
}
...
void StarsRating::paint(QPainter &p) {
	...
	context.position = inner.topLeft() + QPoint(skip, 0);
