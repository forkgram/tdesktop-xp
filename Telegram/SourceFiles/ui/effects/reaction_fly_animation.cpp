...
ReactionFlyAnimationArgs ReactionFlyAnimationArgs::translated(QPoint point) const {
	return {
		// XP walk: designated -> positional (C7555). ReactionFlyAnimationArgs:
		// id0, flyIcon1, flyFrom2, flyUp3, scaleOutDuration4, scaleOutTarget5,
		// miniCopyMultiplier6 (default 1., NOT {}), centerSizeMultiplier7,
		// flyKeepSize8, effectOnly9, forceFirstFrame10.
		id, // id
		flyIcon, // flyIcon
		flyFrom.translated(point), // flyFrom
		flyUp, // flyUp
		scaleOutDuration, // scaleOutDuration
		scaleOutTarget, // scaleOutTarget
		miniCopyMultiplier, // miniCopyMultiplier
...
void ReactionFlyAnimation::paintCenterFrame(
	...
	if (_center) {
		...
		_custom->paint(p, {
			colored, // textColor
			{ _customSize, _customSize }, // size
			now, // now
			(scaled ? (size.width() / float64(_customSize)) : 1.), // scale
			QPoint(
...
ReactionFlyCenter ReactionFlyAnimation::takeCenter() {
	_valid = false;
	return {
		std::move(_custom), // custom
		std::move(_center), // icon
		(_scaleOutDuration > 0) ? _scaleOutTarget : 1., // scale
		_centerSizeMultiplier, // centerSizeMultiplier
