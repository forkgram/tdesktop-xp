...
[[nodiscard]] CacheBackgroundResult CacheBackgroundByRequest(
	...
	if (request.background.isPattern
		...
		return {
			std::move(result).convertToFormat( // image
				QImage::Format_ARGB32_Premultiplied),
			// XP walk: designated -> positional (C7555); gap-fill x@3,y@4.
			gradient, // gradient
			request.area, // area
			{}, // x
			{}, // y
			...
			request.background.waitingForNegativePattern() // waitingForNegativePattern
		};
	} else {
...
style::colorizer ChatTheme::bubblesAccentColorizer(
	...
	return {
		...
		{}, // lightnessMin
		255, // lightnessMax
		color(_palette->msgFileOutBg()->c), // was
		color(accent), // now
	};
}
...
void ChatTheme::setBubblesBackground(QImage image) {
	...
	_bubblesBackground = CacheBackground({
		{ // background
			{}, // key
			_bubblesBackgroundPrepared, // prepared
		},
		(_bubblesBackground.area.isEmpty() // area
...
-> CacheBackgroundRequest {
	...
	return {
		...
		area, // area
		addRotation, // gradientRotationAdd
	};
}
...
-> CacheBackgroundRequest {
	...
	return {
		{ // background
			...
			{}, // prepared
			{}, // preparedForTiled
			_bubblesBackgroundPrepared, // gradientForFill
		},
		area, // area
	};
}
...
BackgroundImageFields ReadBackgroundImage(
	...
	auto read = Images::Read({ // XP walk: designated -> positional (C7555)
		path, // path
		content, // content
		findGiftSymbols ? "GiftPatterns"_q : QByteArray(), // svgCutOutId
		QSize(kMaxSize, kMaxSize), // maxSize
		gzipSvg, // gzipSvg
	});
	auto result = BackgroundImageFields{ // XP walk: designated -> positional (C7555)
		std::move(read.image), // image
	};
...
ChatThemeBackground PrepareBackgroundImage(
	...
	return ChatThemeBackground{
		data.key, // key
		prepared, // prepared
		PrepareImageForTiled(prepared), // preparedForTiled
		std::move(gradientForFill), // gradientForFill
		(!prepared.isNull() // colorForFill
			? imageMonoColor
			: (data.colors.size() > 1 || data.colors.empty())
			? std::nullopt
			: std::make_optional(data.colors.front())),
		// XP walk: designated -> positional (C7555); +giftSymbols@6,giftSymbolFrame@7,giftId@8.
		data.colors, // colors
		std::move(read.giftSymbols), // giftSymbols
		data.giftSymbolFrame, // giftSymbolFrame
		data.giftId, // giftId
		data.patternOpacity, // patternOpacity
		data.generateGradient ? data.gradientRotation : 0, // gradientRotation
		data.isPattern, // isPattern
	};
}
...
[[nodiscard]] QImage PrepareGiftSymbol(
	...
	auto p = QPainter(&result);
	// XP walk: designated -> positional (C7555). v6.1.2 dropped shift/position; only textColor@0.
	emoji->paint(p, {
		QColor(0, 0, 0), // textColor
	});
	return result;
}
