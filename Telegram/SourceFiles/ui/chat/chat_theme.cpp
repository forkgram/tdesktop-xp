...
[[nodiscard]] CacheBackgroundResult CacheBackgroundByRequest(
	...
	if (request.background.isPattern
		...
		return {
		std::move(result).convertToFormat(
				QImage::Format_ARGB32_Premultiplied),
		gradient,
		request.area,
		{}, // x (skipped, default 0)
...
style::colorizer ChatTheme::bubblesAccentColorizer(
	...
	return {
		255, // hueThreshold
		0, // lightnessMin (default)
		255, // lightnessMax (default, NOT {} which would be 0)
...
void ChatTheme::setBubblesBackground(QImage image) {
	...
	_bubblesBackground = CacheBackground({
		{
			_bubblesBackgroundPrepared,
		},
		(_bubblesBackground.area.isEmpty()
...
ChatPaintContext ChatTheme::preparePaintContext(
	...
	return {
		...
		nullptr, // reactionEffects (inserted as field 2 in v3.4.4)
		viewport,
		clip,
		{},
		false, // outbg (inserted as field 6 of ChatPaintContext in v3.0.2)
		crl::now(),
	};
}
...
-> CacheBackgroundRequest {
	...
	return {
		{
			{}, // prepared (skipped)
			{}, // preparedForTiled (skipped)
			_bubblesBackgroundPrepared, // gradientForFill (field 3)
		},
		area,
	};
}
...
ChatThemeBackground PrepareBackgroundImage(
	...
	return ChatThemeBackground{
		prepared,
		PrepareImageForTiled(prepared),
		std::move(gradientForFill),
		(!prepared.isNull()
			? imageMonoColor
		...
		data.colors,
		data.patternOpacity,
		data.generateGradient ? data.gradientRotation : 0,
		data.isPattern,
	};
}
