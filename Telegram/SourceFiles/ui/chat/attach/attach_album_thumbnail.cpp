...
, _repaintRect(std::move(repaintRect)) {
	...
	_photo = PixmapFromImage(Images::Prepare(
		...
		{
			nullptr, // colored
			Option::RoundLarge, // options
			{ imageWidth, imageHeight }, // outer
		}));
	...
	_fileThumb = PixmapFromImage(Images::Prepare(
		...
		{
			nullptr, // colored
			Option::RoundSmall, // options
			{ layoutSt.thumbSize, layoutSt.thumbSize }, // outer
		}));
...
void AlbumThumbnail::moveToLayout(const GroupMediaLayout &layout) {
	...
	_albumImage = PixmapFromImage(Prepare(
		...
		{
			nullptr, // colored
			RoundOptions(ImageRoundRadius::Large, _albumCorners), // options
			{ width, height }, // outer
		}));
...
void AlbumThumbnail::paintFile(
	...
	if (!_caption.isEmpty()) {
		...
		// XP walk: designated -> named-local (C7555; Text::PaintContext large/churns).
		auto context = Text::PaintContext();
		context.position = { left, captionTop };
		context.outerWidth = outerWidth;
		context.availableWidth = _captionAvailableWidth;
		context.align = style::al_left;
		context.elisionLines = 1;
		context.elisionBreakEverywhere = true;
		_caption.draw(p, context);
	} else {
		_lastRectOfCaption = {};
	}
