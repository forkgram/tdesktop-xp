...
[[nodiscard]] std::optional<NativeMessage> ParseNativeMessage(
	...
	if (externalShell) {
		...
		return NativeMessage{
			...
			// NativeMessage: source, origin, command, arguments.
			source, // source
			(source == NativeMessageSource::ExternalWebApp)
				? origin.toString()
				: QString(), // origin
	...
	return NativeMessage{
		...
		OriginFromUrl(sourceUrl), // origin
		command, // command
		arguments, // arguments
	};
}
...
, _widget(std::make_unique<SeparatePanel>(Ui::SeparatePanelArgs{
	...
	// parent, anchorGeometry, transientParent, onAllSpaces, animationsPaused, menuSt.
	_externalPanelParent.get(), // parent
	{}, // anchorGeometry
	{}, // transientParent
	{}, // onAllSpaces
	{}, // animationsPaused
...
bool Panel::showWebview(Args &&args, const Webview::ThemeParams &params) {
	...
	const auto dispatch = SharedPanelMenuDispatchArgs{
		...
		[=](uint32 id, DownloadsAction type) { // download
	...
	_widget->setMenuAllowed([=](
		...
		// XP walk: designated -> positional (C7555). SharedPanelMenuBuildArgs:
		// downloads, hasSettings, buttons.
		const auto items = BuildSharedPanelMenuItems({
			&downloads, // downloads
			_webview // hasSettings
...
Panel::ExternalShellAnchor Panel::externalShellAnchor() const {
	...
	auto result = ExternalShellAnchor{
		...
		std::move(popupAnchor.geometry), // anchorGeometry
...
void Panel::openPopup(const QJsonObject &args) {
	...
	const auto weak = base::make_weak(this);
	// XP walk: designated -> positional (C7555). PopupArgs: parent, anchorGeometry,
	// transientParent, title, text, value, buttons, ignoreFloodCheck.
	showPopup({
		webviewWindowForPopup(), // parent
		{}, // anchorGeometry
		{}, // transientParent
		args["title"].toString(), // title
		message, // text
		{}, // value
		std::move(buttons), // buttons
...
void Panel::openScanQrPopup(const QJsonObject &args) {
	// XP walk: designated -> positional (C7555). PopupArgs: parent, anchorGeometry,
	// transientParent, title, text, value, buttons, ignoreFloodCheck. Button: id, text, type.
	showPopup({
		webviewWindowForPopup(), // parent
		{}, // anchorGeometry
		{}, // transientParent
...
void Panel::openShareStory(const QJsonObject &args) {
	...
	showPopup({
		...
		{}, // transientParent
		{}, // title
		tr::lng_bot_no_share_story(tr::now), // text
		{}, // value
		{ {
			"ok", // id
			tr::lng_box_ok(tr::now), // text
...
void Panel::requestWriteAccess() {
	...
	_delegate->botCheckWriteAccess([=](bool allowed) {
		...
		showPopup({
			...
			{}, // anchorGeometry
			{}, // transientParent
			integration->phraseBotAllowWriteTitle(), // title
			integration->phraseBotAllowWrite(), // text
			{}, // value
			{ // buttons
				{
					"allow", // id
					integration->phraseBotAllowWriteConfirm(), // text
				},
				{ "cancel", {}, Button::Type::Cancel }, // id, text, type
			},
		}, [=](Webview::PopupResult result) {
			if (!weak) {
				return;
...
void Panel::requestPhone() {
	...
	showPopup({
		...
		{}, // anchorGeometry
		{}, // transientParent
		integration->phraseBotSharePhoneTitle(), // title
		integration->phraseBotSharePhone(), // text
		{}, // value
		{ // buttons
			{
				"share",
...
void Panel::closeWithConfirmation() {
	...
	showPopup({
		...
		{}, // anchorGeometry
		{}, // transientParent
		integration->phrasePanelCloseWarning(), // title
		integration->phrasePanelCloseUnsaved(), // text
		{}, // value
		{ // buttons
			{
				"close",
				integration->phrasePanelCloseAnyway(),
				Button::Type::Destructive,
			},
			{ "cancel", {}, Button::Type::Cancel },
		},
		true, // ignoreFloodCheck
	}, [=](Webview::PopupResult result) {
		if (!weak) {
			return;
