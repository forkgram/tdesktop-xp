...
bool Panel::createWebview(const Webview::ThemeParams &params) {
	...
	_webview = std::make_unique<WebviewWithLifetime>(
		container,
		// XP walk: designated -> positional (C7555). WindowConfig: opaqueBg, storageId.
...
void Panel::closeWithConfirmation() {
	...
	const auto result = Webview::ShowBlockingPopup({
		widget ? widget->window() : nullptr,
		integration->phrasePanelCloseWarning(),
		integration->phrasePanelCloseUnsaved(),
		{},
		{
			{
				"close",
...
void Panel::processButtonMessage(
	...
	button->updateArgs({
		// XP walk: designated -> positional (C7555). Args: isActive, isVisible,
		// isProgressVisible, text.
