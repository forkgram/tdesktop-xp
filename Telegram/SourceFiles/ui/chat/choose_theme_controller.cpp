...
void ChooseThemeController::fill(
	...
	_dark.value(
		...
		_entries.push_back({
			...
			{}, // theme
			{}, // media
			GenerateEmptyPreview(), // preview
			_disabledEmoji, // emoji
			QRect(QPoint(x, margin.top()), single), // geometry
			(_chosen.current() == kDisableElement()), // chosen
		});
		...
		for (const auto &theme : themes) {
			...
			_entries.push_back({
				...
				{}, // preview
				emoji, // emoji
				QRect(QPoint(x, skip), single), // geometry
				isChosen, // chosen
			});
