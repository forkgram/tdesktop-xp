...
[[nodiscard]] Preview GeneratePreview(
	...
	return { // XP walk: designated -> positional (C7555)
		Images::Round(std::move(result), ImageRoundRadius::Large), // preview
		userpic, // userpic
	};
}
...
void ChooseThemeController::initButtons() {
	...
	const auto confirmTakeGiftTheme = crl::guard(apply, [=](
		...
		_controller->show(Box([=](not_null<Ui::GenericBox*> box) {
			...
			Ui::ConfirmBox(box, { // XP walk: designated -> positional (C7555); gap-fill cancelled@2.
				tr::lng_chat_theme_gift_replace(
					lt_name,
...
void ChooseThemeController::paintEntry(QPainter &p, const Entry &entry) {
	...
	if (const auto emoji = entry.emoji) {
		...
		custom->paint(p, { // XP walk: designated -> positional (C7555)
			st::windowFg->c, // textColor
			{}, // size
			{}, // now
			{}, // scale
			{ emojiLeft + customSkip, emojiTop + customSkip }, // position
		});
	}
...
void ChooseThemeController::fill(
	...
	rpl::combine(
		...
		auto x = margin.left();
		// XP walk: designated -> named-local (C7555; Entry large + move-only custom@6).
		auto entry = Entry();
		entry.emoji = _disabledEmoji;
		entry.preview = GenerateEmptyPreview();
		entry.geometry = QRect(QPoint(x, margin.top()), single);
		entry.chosen = (_chosen.current() == kDisableElement());
		...
		const auto push = [&](
			...
			entry.token = token;
			entry.key = key;
			entry.gift = theme.unique;
			entry.takenUserpic = (themeUser
				? Ui::MakeUserpicThumbnail(themeUser, true)
				: nullptr);
			entry.custom = (theme.unique
				? manager->create(
					theme.unique->model.document,
					[=] { _inner->update(); },
					Data::CustomEmojiSizeTag::Large)
				: nullptr);
			entry.emoji = (theme.emoticon.isEmpty()
				? nullptr
