...
base::options::option<QString> OptionTranslateUrlTemplate({
	"translate-url-template", // id
	"Translate URL template", // name
	"Template URL for custom translation provider." // description
...
TranslateProviderRequest PrepareTranslateProviderRequest(
	...
	auto result = TranslateProviderRequest{
		uint64(peer->id.value), // peerId
		IsServerMsgId(msgId) ? msgId.bare : 0, // msgId
		std::move(text), // text
	};
