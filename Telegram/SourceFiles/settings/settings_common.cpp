...
base::options::toggle OptionMonoSettingsIcons({ kOptionMonoSettingsIcons, "Mono settings and menu icons", "Use a single color for settings and main menu icons." });
...
LottieIcon CreateLottieIcon(
	...
	return { std::move(object), std::move(animate) };
}
...
void FillMenu(
	...
	if (type == Chat::Id()) {
		...
		addAction({ tr::lng_settings_logout(tr::now), [=] { window->showLogoutConfirmation(); }, &st::menuIconLeaveAttention, {}, {}, true });
	}
}
