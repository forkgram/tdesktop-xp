...
void BuildSecuritySection(
...

	{
		...
		builder.addButton({
			u"security/login_email"_q, // id
			tr::lng_settings_cloud_login_email_section_title(), // title
			&st::settingsButtonRightLabelSpoiler, // st
			{ &st::menuIconRecoveryEmail }, // icon
			{}, // container
			std::move(loginEmailLabel), // label
			{}, // toggled
			[=] {
				UrlClickHandler::Open(u"tg://settings/login_email"_q);
			}, // onClick
			{ u"email"_q, u"login"_q }, // keywords
			{}, // highlight
			std::move(loginEmailShown), // shown
		});
	}
