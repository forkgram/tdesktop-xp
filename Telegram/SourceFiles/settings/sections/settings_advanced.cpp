...
void BuildScreenReaderSection(SectionBuilder &builder) {
	...
	builder.addSkip();
	builder.addSubsectionTitle({
		u"advanced/screen_reader"_q, // id
		tr::lng_screen_reader_settings_title(), // title
		{ u"screen reader"_q, u"accessibility"_q, u"voiceover"_q }, // keywords
	});

	const auto toggle = builder.addButton({
		u"advanced/screen_reader_disable"_q, // id
		tr::lng_screen_reader_settings_disable(), // title
		&st::settingsButtonNoIcon, // st
		{}, // icon
		nullptr, // container
		{}, // label
		rpl::single(disabled), // toggled
		nullptr, // onClick
		{ u"screen reader"_q, u"accessibility"_q }, // keywords
	});
