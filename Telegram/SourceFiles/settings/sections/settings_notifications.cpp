...
void BuildNotifyTypeSection(SectionBuilder &builder) {
	...
	builder.add([controller, showOther](const WidgetContext &ctx) {
		...
		if (ctx.highlights) {
			...
			ctx.highlights->push_back({
				...
				{ reactions.get(), { {}, HighlightShape::Rect, 0, nullptr, 0.4, false, true } },
			});
		}
		return SectionBuilder::WidgetToAdd{};
	}, [] {
	...
	builder.add(nullptr, [] {
		return SearchEntry{
			u"notifications/reactions"_q, // id
			tr::lng_notification_reactions(tr::now), // title
			{ u"reactions"_q }, // keywords
			{}, // section
