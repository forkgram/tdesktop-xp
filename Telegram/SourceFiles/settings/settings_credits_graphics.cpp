...
void AddViewMediaHandler(
	...
	invoiceData.amount = uint64(e.credits.abs().whole());
	...
	const auto fill = [=](Data::CreditTopupOptions options) {
		...
		for (auto i = 0; i < options.size(); i++) {
			...
			inner->paintRequest(
				...
				p.setPen(st.textFg);
				// XP walk: designated -> named-local (C7555); v5.4.0 sets .elisionLines
				// (field 19), past the fullWidthSelection=true default, so a positional
				// init would hit the default trap. outerWidth stays default 0.
				auto context = Ui::Text::PaintContext();
				context.position = QPoint(textLeft, 0);
				context.availableWidth = inner->width() - textLeft;
...
not_null<Ui::RpWidget*> AddBalanceWidget(
	...
	balance->paintRequest(
		...
		// XP walk: designated -> named-local (PaintContext .availableWidth
		// non-contiguous). v5.16.0: star icon is now embedded in the count text,
		// so the separate star image draw is gone.
...
CreditsEntryBoxStyleOverrides DarkCreditsEntryBoxStyle() {
	...
	return {
		// XP walk: designated -> positional (C7555). +theme@7 (v6.1.0), +craft@9 (v6.5.0), +offer@18 (v6.3.x).
		...
		&st::darkGiftTheme, // theme (v6.1.0)
		&st::darkGiftTransfer, // transfer
		&st::darkGiftCraft, // craft (v6.5.0)
...
void GenericCreditsEntryCover(
	...
	if (uniqueGift) {
		...
		// pretitle, numberText, subtitle, subtitleClick, subtitleLinkColored,
		// subtitleOutlined, resalePrice, resaleClick.
...
void GenericCreditsEntryBody(
	...
	if (e.bareGiftReleasedById && !e.uniqueGift) {
		...
		amount->paintRequest(
			...
			text->draw(p, Ui::Text::PaintContext{ // XP walk: designated -> positional
				QPoint(x, y), // position
				amount->width(), // outerWidth
				amount->width(), // availableWidth
			});
			if (minorText) {
				minorText->draw(p, Ui::Text::PaintContext{ // XP walk: designated -> positional
					QPoint(
						x + text->maxWidth(),
						y + st::lineWidth * 2), // position
					amount->width(), // outerWidth
					amount->width(), // availableWidth
				});
			}
...
void GiftedCreditsBox(
	...
	// XP walk: designated -> named-local (C7555); named local avoids int64->uint64
	// narrowing. v5.4.0 passes the entry as e (3rd arg) with a default
	// SubscriptionEntry (4th arg) instead of the old (nullptr, entry) order.
	...
	entry.credits = CreditsAmount(count);
	...
	entry.in = received;
	entry.gift = true;
	Settings::ReceiptCreditsBox(box, controller, entry, {});
}
...
void CreditsPrizeBox(
	...
	entry.date = base::unixtime::parse(date);
	entry.credits = CreditsAmount(data.count);
...
void GlobalStarGiftBox(
	...
	auto entry = Data::CreditsHistoryEntry();
	entry.credits = CreditsAmount(data.stars);
	entry.bareGiftStickerId = data.document->id;
	entry.bareGiftOwnerId = ownerId;
	entry.bareGiftHostId = data.unique ? data.unique->hostId.value : 0; // XP walk: NEW v6.2.0
	entry.bareGiftResaleRecipientId = ((resale.recipientId != selfId)
		? resale.recipientId.value
		: 0); // XP walk: v6.0.0 resale param (was resaleRecipientId)
	...
	entry.stargift = true;
	entry.giftResaleForceTon = resale.forceTon; // XP walk: v6.0.0 new (TON currency)
...
Data::CreditsHistoryEntry SavedStarGiftEntry(
	...
	entry.bareGiftOwnerId = ownerId.value;
	entry.bareGiftHostId = hostId.value; // XP walk: NEW v6.2.0
...
void ShowStarGiftViewBox(
	...
	const auto nextToUpgradeShow = upgradeNext
		? [=] { ShowSavedStarGiftBox(
			...
			*upgradeNext); }
		: Fn<void()>();
	// XP walk: designated -> named-local (C7555; CreditsHistoryEntry large; took theirs).
	auto entry = Data::CreditsHistoryEntry();
	entry.id = data.slug;
	entry.description = data.message;
	entry.date = base::unixtime::parse(item->date());
	entry.credits = CreditsAmount(data.count);
	entry.bareMsgId = uint64(data.realGiftMsgId
	...
	entry.bareGiftOwnerId = ownerId.value;
	entry.bareGiftHostId = hostId.value; // XP walk: NEW v6.2.0
	entry.bareGiftReleasedById = (data.stargiftReleasedBy
...
void SmallBalanceBox(
	...
	const auto content = [&]() -> Ui::Premium::TopBarAbstract* {
		// XP walk: designated -> named-local (C7555). v5.4.0 semantics: title
		// filters needed>0 and about branches on the SmallBalanceSource variant
		// (old creditsNeeded/bot no longer exist). Named-local avoids the
		// optimizeMinistars=true pass-through trap under gradientStops.
		...
		descriptor.about = (v::is<SmallBalanceSubscription>(source)
		...
		return box->setPinnedToTopContent(object_ptr<Ui::Premium::TopBar>(
			box,
			// XP walk: take theirs (dark ? videoStreamStarsCover); descriptor stays
			// named-local (built above; C7555).
			(dark
