	...
	reset->entity()->addClickHandler([=] {
		...
		if (!date || *sent) {
			...
			*sent = true;
			...
			Ui::show(Ui::MakeConfirmBox({ tr::lng_cloud_password_reset_cancel_sure(), cancel, {}, tr::lng_box_yes(), tr::lng_box_no() }));
		}
	});
...
object_ptr<Ui::BoxContent> CloudPasswordAppOutdatedBox() {
	...
	return Ui::MakeConfirmBox({ tr::lng_passport_app_out_of_date(), callback, {}, tr::lng_menu_update() });
}
