...
void Premium::setupContent() {
	...
	// XP walk: v4.13.0 moved the inline premium-summary/icon building into
	// the AddSummaryPremium() helper (defined below in this file).
...
void AddSummaryPremium(
	...
	for (auto i = 0; i < int(icons.size()); i++) {
		...
		AddButtonIcon(
			...
			stDefault,
			// XP walk: designated -> positional (C7555)
			{ icons[i], IconType::Rounded, {}, brush }); // icon, type, background, backgroundBrush
	}
