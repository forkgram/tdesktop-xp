	...
	button->addClickHandler([=] {
		controller->show(Box([=](not_null<Ui::GenericBox*> box) {
			const auto save = [=](int index) {
				...
				controller->show(Ui::MakeConfirmBox({ tr::lng_settings_need_restart(), confirmed, {}, tr::lng_settings_restart_now() }));
			};
			SingleChoiceBox(box, {
				tr::lng_settings_angle_backend(),
				options,
...
void SetupOpenGL(
	...
	button->toggledValue(
		...
		controller->show(Ui::MakeConfirmBox({ tr::lng_settings_need_restart(), confirmed, cancelled, tr::lng_settings_restart_now() }));
	}, container->lifetime());
}
