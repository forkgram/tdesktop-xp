...
void SetupPremium(
	...
	const auto button = AddButtonWithIcon(
		...
		st::settingsButton,
		{ &st::menuIconShop }); // XP walk: designated -> positional (C7555): icon
	button->addClickHandler([=] {
	...
	const auto setScale = [=](int scale, const auto &repeatSetScale) -> void {
		...
		*inSetScale = true;
		...
		if (cEvalScale(scale) != cEvalScale(cConfigScale())) {
			...
			window->show(Ui::MakeConfirmBox({ tr::lng_settings_need_restart(), confirmed, cancelled, tr::lng_settings_restart_now() }));
...
void SetupHelp(
	...
	button->addClickHandler([=] {
		...
		auto box = Ui::MakeConfirmBox({
			// XP walk: designated -> positional (C7555). ConfirmBoxArgs: text, confirmed, cancelled,
			// confirmText, cancelText, confirmStyle, cancelStyle, labelStyle, labelFilter,
			// labelPadding, title, inform, strictCancel.
			tr::lng_settings_ask_sure(), // text
			sure, // confirmed
			[=](Fn<void()> close) { // cancelled
				OpenFaq(controller);
				close();
			},
			tr::lng_settings_ask_ok(), // confirmText
			tr::lng_settings_faq_button(), // cancelText
			{}, // confirmStyle
			{}, // cancelStyle
			{}, // labelStyle
			{}, // labelFilter
