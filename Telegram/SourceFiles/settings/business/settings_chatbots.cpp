			...
			*requestId = session->api().request(MTPcontacts_ResolveUsername(
				...
				if (const auto user = peer->asUser()) {
					if (user->isBot()) {
						...
						consumer.put_next(BotState{ // XP walk: designated -> positional (C7555)
							user, // bot
							LookupState::Ready, // state
						});
						return;
				...
				consumer.put_next(BotState{ {}, LookupState::Ready }); // XP walk: designated -> positional (C7555); {}=bot, state
			}).fail([=] {
				cache->emplace(extracted, nullptr);
				consumer.put_next(BotState{ {}, LookupState::Ready }); // XP walk: designated -> positional (C7555); {}=bot, state
			}).send();
