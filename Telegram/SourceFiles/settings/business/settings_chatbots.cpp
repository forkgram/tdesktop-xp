...
[[nodiscard]] ParsedUsernameQuery ParseUsernameQuery(QString text) {
	...
	return { // XP walk: designated -> positional (C7555)
		text, // value
		!text.isEmpty()
			&& validateExpression.match(text).hasMatch(), // valid
	};
}
...
[[nodiscard]] BotSearchState SingleBotState(UserData *bot) {
	auto result = BotSearchState{ // XP walk: designated -> positional (C7555)
		{}, // bots
		bot ? LookupState::Ready : LookupState::Empty, // state
	};
	if (bot) {
...
[[nodiscard]] BotSearchState MakeReadySearchState(
	...
	return { // XP walk: designated -> positional (C7555)
		std::move(bots), // bots
		unsupported ? LookupState::Unsupported : LookupState::Ready, // state
		request.exactUnsupported, // exactUnsupported
	};
}
...
[[nodiscard]] rpl::producer<BotSearchState> LookupBots(
	...
	return std::move(
		...
		return [=](auto consumer) {
			...
			consumer.put_next(BotSearchState{ // XP walk: designated -> positional (C7555)
				{}, // bots
				LookupState::Loading, // state
			});
...
void Chatbots::checkBeforeClose(Fn<void()> close) {
	...
	box.text = tr::lng_chatbots_leave_without_added_text();
	box.confirmed = crl::guard(this, [=](Fn<void()> closeBox) {
		closeBox();
		close();
	});
	box.confirmText = tr::lng_box_leave();
	box.cancelText = tr::lng_cancel();
	box.title = tr::lng_chatbots_leave_without_added_title();
	controller()->show(Ui::MakeConfirmBox(std::move(box)));
}
...
void Chatbots::setupContent() {
	...
	const auto addBot = [=](UserData *resolved) {
		...
		config.text = { tr::lng_chatbots_added_success(
			tr::now,
			lt_bot,
			resolved->name()) };
		config.icon = &st::toastCheckIcon;
	...
	AddBusinessRecipientsSelector(_detailsWrap, { // XP walk: designated -> positional (C7555)
...
void Chatbots::refreshDetails() {
	...
	std::move(
		...
		const auto warn = [&](tr::phrase<lngtag_bot> text) {
			// XP walk: designated -> named-local (C7555; ConfirmBoxArgs non-contiguous).
			auto box = Ui::ConfirmBoxArgs();
			box.text = text(tr::now, lt_bot, Ui::Text::Bold(bot->name()), Ui::Text::RichLangValue);
			box.title = tr::lng_chatbots_warning_title();
			controller()->show(Ui::MakeInformBox(std::move(box)));
		};
