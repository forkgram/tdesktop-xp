...
void ShortcutMessages::fillTopBarMenu(
	...
	const auto confirmDeleteShortcut = [=] {
		...
		if (slice.fullCount == 0) {
			...
			_controller->show(Ui::MakeConfirmBox({ // XP walk: designated -> positional (C7555)
				{ tr::lng_replies_delete_sure() }, // text
				confirmed, // confirmed
				{}, // cancelled
...
void ShortcutMessages::setupComposeControls() {
	...
	const auto state = Dialogs::EntryState{ // XP walk: designated -> positional (C7555)
		Dialogs::Key{ _history }, // key
	...
	auto writeRestriction = rpl::combine(
		...
		return (count >= limit)
			? Controls::WriteRestriction{ // XP walk: designated -> positional (C7555)
				tr::lng_business_limit_reached( // text
					tr::now,
					lt_count,
					limit),
				{}, // button
...
base::weak_qptr<Ui::RpWidget> ShortcutMessages::createPinnedToBottom(
	...
	_composeControls = std::make_unique<ComposeControls>(
		...
		ComposeControlsDescriptor{ // XP walk: designated -> positional (C7555)
			&st::repliesComposeControls, // stOverride
