...
void Greeting::setupContent(
	...
	_enabled.value() | rpl::filter(_1) | rpl::start_with_next([=] {
		if (!_canHave.current()) {
			...
			config.text = { tr::lng_greeting_limit_reached(tr::now) };
...
void Greeting::save() {
	...
	session->data().businessInfo().saveGreetingSettings(
		_enabled.current() ? Data::GreetingSettings{ // XP walk: designated -> positional (C7555)
			_recipients.current(), // recipients
			_noActivityDays.current(), // noActivityDays
			LookupShortcutId(session, u"hello"_q), // shortcutId
