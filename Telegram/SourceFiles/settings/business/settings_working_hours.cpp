...
void WorkingHours::setupContent(
	...
	const auto state = content->lifetime().make_state<State>(State{ // XP walk: designated -> positional (C7555)
		info->timezonesValue(), // timezones
	});
	...
	AddDividerTextWithLottie(content, { // XP walk: designated -> positional (C7555)
		...
		tr::lng_hours_about(Ui::Text::WithEntities), // about
