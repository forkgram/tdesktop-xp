...
void AddAwayScheduleSelector(
	...
	const auto chooseDate = [=](
		...
		*box = controller->show(Box(ChooseDateTimeBox, ChooseDateTimeBoxArgs{ // XP walk: designated -> positional (C7555)
...
void AwayMessage::setupContent(
	...
	if (BadCustomInterval(initialSchedule.customInterval)) {
		...
		initialSchedule.customInterval = WorkingInterval{ // XP walk: designated -> positional (C7555)
			now, // start
			now + 24 * 60 * 60, // end
		};
	}
	...
	AddDividerTextWithLottie(content, { // XP walk: designated -> positional (C7555)
		...
		tr::lng_away_about(Ui::Text::WithEntities), // about
	...
	Ui::AddSkip(inner);
	AddAwayScheduleSelector(inner, { // XP walk: designated -> positional (C7555)
...
void AwayMessage::save() {
	...
	session->data().businessInfo().saveAwaySettings(
		_enabled.current() ? Data::AwaySettings{ // XP walk: designated -> positional (C7555)
			_recipients.current(), // recipients
			_schedule.current(), // schedule
			LookupShortcutId(session, u"away"_q), // shortcutId
			_offlineOnly.current(), // offlineOnly
		} : Data::AwaySettings(),
		fail);
}
