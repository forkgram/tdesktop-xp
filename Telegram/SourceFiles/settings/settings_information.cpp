...
[[nodiscard]] object_ptr<Ui::SettingsButton> MakeAccountButton(
	...
	raw->clicks(
		...
		auto logoutCallback = [=] {
			...
			window->show(
				Ui::MakeConfirmBox({ tr::lng_sure_logout(), crl::guard(session, callback), {}, tr::lng_settings_logout(), {}, &st::attentionBoxButton }),
				Ui::LayerOption::CloseOther);
		};
		addAction({ tr::lng_settings_logout(tr::now), std::move(logoutCallback), &st::menuIconLeaveAttention, {}, {}, true });
		state->menu->popup(QCursor::pos());
	}, raw->lifetime());
...
AccountsEvents SetupAccounts(
	...
	const auto list = container->lifetime().make_state<AccountsList>(
		container,
		controller);
	return { list->currentAccountActivations() };
}
