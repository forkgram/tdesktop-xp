	...
	const auto icon = CreateInteractiveLottieIcon(
		content,
		{
			...
			currentStepValidate
				...
				: u"cloud_password/password_input"_q, // name
			{}, // path
			{}, // json
			{}, // color
			Size(st::settingsCloudPasswordIconSize), // sizeOverride
		},
	...
	if (isCheck) {
		...
		auto recoverCallback = [=] {
			...
			if (state->hasRecovery) {
				...
				controller()->show(Ui::MakeConfirmBox({ tr::lng_cloud_password_reset_no_email(), callback, {}, tr::lng_cloud_password_reset_ok(), tr::lng_cancel(), &st::attentionBoxButton }));
			}
		};
...
void Input::setupRecoverButton(
	...
	button->setClickedCallback([=] {
		...
		if (suggest == Status::SuggestAction::Recover) {
			...
			controller()->show(Ui::MakeConfirmBox({ tr::lng_cloud_password_reset_cancel_sure(), cancel, {}, tr::lng_box_yes(), tr::lng_box_no() }));
