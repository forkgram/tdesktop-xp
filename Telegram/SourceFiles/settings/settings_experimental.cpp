	...
	button->events(
		...
		*menu = base::make_unique_q<Ui::PopupMenu>(
		...
		(*menu)->addAction(u"Copy deep link"_q, [=] {
			...
			// XP walk: designated -> named-local (C7555). Toast::Config has a
			// move-only `content` member, so positional init is impossible.
...
void AddOption(
	...
	if (restarter) {
		restarter->setCallback([=] {
			window->show(Ui::MakeConfirmBox({ tr::lng_settings_need_restart(), [] { Core::Restart(); }, {}, tr::lng_settings_restart_now(), tr::lng_settings_restart_later() }));
		});
	}
