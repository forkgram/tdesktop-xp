...
void ItemBase::updateZoom(float64 zoom) {
	...
	const auto maxSide = std::max(
		...
		_imageSize.height());
	_sizeLimits = {
		std::max(int(maxSide * kMinSizeRatio), 1), // min
		std::max(int(maxSide * kMaxSizeRatio), 1), // max
	};
	_horizontalSize = std::clamp(
		_horizontalSize,
...
ItemBase::Data ItemBase::generateData() const {
	return {
		(st::photoEditorItemHandleSize / _scaledHandleSize), // initialZoom
		_lastZ, // zPtr
		int(_horizontalSize), // size
		int(scenePos().x()), // x
		int(scenePos().y()), // y
		flipped(), // flipped
