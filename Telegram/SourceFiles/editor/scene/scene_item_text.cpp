...
LayoutMetrics ComputeMetrics(
	...
	return { // XP walk: designated -> positional (C7555)
		std::max(int(std::ceil(maxWidth)), kMinContentWidth), // contentWidth
		int(std::ceil(totalHeight)), // contentHeight
		padding, // padding
...
QPainterPath BuildConnectedBackground(
	...
	for (auto i = 0; i < layout.lineCount(); ++i) {
		...
		rects.push_back({ // XP walk: designated -> positional (C7555)
			centerX - hw, // left
			padding + float64(line.y()) - linePadV, // top
			centerX + hw, // right
			padding + float64(line.y() + line.height()) + linePadV, // bottom
		});
	}
...
void ItemText::save(SaveState state) {
	...
	saved = { // XP walk: designated -> positional (C7555)
		_text, // text
		_color, // color
		_fontSize, // fontSize
