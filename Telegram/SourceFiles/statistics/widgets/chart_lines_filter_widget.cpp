...
void ChartLinesFilterWidget::FlatCheckbox::paintEvent(QPaintEvent *e) {
	...
	p.setPen(textColor);
	// XP walk: designated init -> named-local (C7555). +elision (v6.8.0).
	auto textContext = Ui::Text::PaintContext();
	textContext.position = QPoint(textX, textY);
	textContext.availableWidth = textWidth;
	textContext.elisionLines = constrained ? 1 : 0;
