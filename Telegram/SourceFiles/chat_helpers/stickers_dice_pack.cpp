...
// XP walk: took v5.4.0 and dropped OUR DicePack::generateLocal member.
// It is dead: tryGenerateLocalZero() now builds via the local lambda over
// ChatHelpers::GenerateLocalTgsSticker, and the header no longer declares it.
	...
	_resolveGameOptionsRequestId = _session->api().request(
		...
		if (const auto onstack = base::take(_resolveGameOptionsCallback)) {
			onstack(result.match([&](
				...
				return Data::DiceGameOptions{
					data.vgame_hash().v, // seedHash
					int64(data.vprev_stake().v), // previousSteakNanoTon
					rewards, // milliRewards
					jackpot, // jackpotMilliReward
					data.vcurrent_streak().v, // currentStreak
					data.vplays_left().value_or_empty(), // playsLeft
				};
			}));
		}
	}).fail([=] {
