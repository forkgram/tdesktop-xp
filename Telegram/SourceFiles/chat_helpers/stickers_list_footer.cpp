...
void StickersListFooter::paintExpanding(
	...
	const auto context = ExpandingContext{
		clip.translated(-shift), // clip
		clip.height() / float64(height()), // progress
		int(std::ceil(radius)), // radius
		true, // expanding
	};
	paint(p, context);
	p.translate(-shift);
...
void StickersListFooter::enumerateIcons(
	...
	for (auto i = 0, count = int(_icons.size()); i != count; ++i) {
		...
		const auto result = callback({
			i, // index
			left, // left
			shifted, // adjustedLeft
			int(base::SafeRound(width)), // width
			visible, // visible
		});
		if (!result) {
			break;
...
auto StickersListFooter::iconInfo(int index) const -> IconInfo {
	if (index < 0) {
		...
		return {
			-1, // index
			-_singleWidth - _iconsLeft, // left
			-_singleWidth - _iconsLeft - iconsX, // adjustedLeft
			_singleWidth, // width
			false, // visible
		};
	}
	...
	if (icon.sticker) {
		...
		if (icon.custom) {
			...
			const auto frame = icon.webm->current(
				{ /*frame*/ { icon.pixw, icon.pixh }, /*outer*/ {}, /*factor*/ {}, /*radius*/ {}, /*corners*/ RectPart::AllCorners, /*colored*/ QColor(0, 0, 0, 0), /*keepAlpha*/ true },
				paused ? 0 : now);
			if (icon.savedFrame.isNull()) {
				icon.savedFrame = frame;
				icon.savedFrame.setDevicePixelRatio(
