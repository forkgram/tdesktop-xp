...
void EmojiColorPicker::handleMouseRelease(QPoint globalPos) {
	...
	if (_selected >= 0 && (pressed < 0 || _selected == pressed)) {
		_chosen.fire_copy({ _variants[_selected] }); // emoji
	}
	_ignoreShow = true;
	hideAnimated();
}
		...
		for (const auto &one : set.list) {
			...
			if (checkCustom(one.emoji, id)) {
				_searchResults.push_back({
					{}, // collectible
					one.custom, // custom
					{ RecentEmojiDocument{ id, test } }, // id
				});
			}
		}
	}
...
bool EmojiListWidget::addSearchShortcut(not_null<Data::StickersSet*> set) {
		...
		if (const auto sticker = document->sticker()) {
			list.push_back({
				...
				resolveCustomEmoji(
					...
					true),
				document, // document
				Ui::Emoji::Find(sticker->alt), // emoji
			});
			break;
		}
	}
	...
	_searchShortcutSets.push_back({ // XP walk: designated -> positional (C7555)
		set->id, // id
		set, // set
		set->lookupThumbnailDocument(), // thumbnailDocument
		set->title, // title
		std::move(list), // list
		{}, // ripple
		false, // painted
...
void EmojiListWidget::fillSelectedSearchShortcut() {
	...
	const auto installed = !!(set->flags & Data::StickersSetFlag::Installed);
	_searchSets.push_back({ // XP walk: designated -> positional (C7555)
		set->id, // id
		set, // set
		set->lookupThumbnailDocument(), // thumbnailDocument
		tr::lng_custom_emoji_count(
			tr::now,
			lt_count,
			set->count), // title
		std::move(list), // list
...
object_ptr<TabbedSelector::InnerFooter> EmojiListWidget::createFooter() {
	...
	auto result = object_ptr<StickersListFooter>(FooterDescriptor{
		...
		this, // parent
		&st(), // st
		{ // features (ComposeFeatures{ .stickersSettings = false }, rest default)
			false, // likes
			true, // sendAs
			true, // ttlInfo
			true, // botCommandSend
		...
		(_mode == Mode::BackgroundEmoji), // forceFirstFrame
	});
	_footer = result;
...
void EmojiListWidget::fillRecent() {
	...
	for (const auto &one : list) {
		...
		_recent.push_back({
			{}, // collectible
			resolveCustomRecent(one.id), // custom
			one.id, // id
		});
		if (document) {
...
void EmojiListWidget::fillRecentFrom(
	...
	for (const auto &id : list) {
		if (!id && _mode == Mode::EmojiStatus) {
			...
			// XP walk: designated -> positional (C7555)
			_recent.push_back({ {}, {}, { Ui::Emoji::Find(star) } }); // collectible, custom, id
		} else if (!id
			&& (_mode == Mode::BackgroundEmoji
				|| _mode == Mode::ChannelStatus)) {
			...
			_recent.push_back({
				{}, // collectible
				resolveCustomRecent(fakeId), // custom
				{ Ui::Emoji::Find(no) }, // id
			});
			...
			_recent.push_back({
				...
				resolveCustomRecent(id), // custom
				{ RecentEmojiDocument{ id.documentId, test } }, // id
			});
...
void EmojiListWidget::fillRecentMenu(
	...
	if (custom) {
		id.data = RecentEmojiDocument{
			custom.document->id, // id
			custom.document->session().isTestMode(), // test
		};
	}
	...
	const auto resetRecent = [=] {
		...
		showBoxPreventHide(Ui::MakeConfirmBox({
			tr::lng_emoji_reset_recent_sure(), // text
			crl::guard(this, sure), // confirmed
			{}, // cancelled
			tr::lng_emoji_reset_recent_button(tr::now), // confirmText
			{}, // cancelText
			{}, // confirmStyle
			{}, // cancelStyle
			&st().boxLabel, // labelStyle
			}));
	};
	addAction({
...
void EmojiListWidget::validateEmojiPaintContext(
	...
	auto value = Ui::Text::CustomEmojiPaintContext{
		...
		QSize(_customSingleSize, _customSingleSize), // size
		crl::now(), // now
		context.progress, // scale
		{}, // position
		On(powerSavingFlag()) || paused(), // paused
		context.expanding, // scaled
		{ false, (_mode == Mode::BackgroundEmoji) }, // internal: colorized, forceFirstFrame
	};
...
EmojiChosen EmojiListWidget::lookupChosen(
	...
	return {
		emoji, // emoji
		{ // messageSendingFrom
			Ui::MessageSendingAnimationFrom::Type::Emoji, // type
			{}, // localId
			mapToGlobal(icon), // globalStartGeometry
		},
	};
}
...
FileChosen EmojiListWidget::lookupChosen(
	...
	return {
		...
		{ // messageSendingFrom
			Ui::MessageSendingAnimationFrom::Type::Emoji, // type
			{}, // localId
			over ? mapToGlobal(emoji) : QRect(), // globalStartGeometry
			over ? Ui::GrabWidgetToImage(this, emoji) : QImage(), // frame
		},
		custom.collectible, // collectible
	};
}
...
void EmojiListWidget::removeMegagroupSet(bool locally) {
	...
	showBoxPreventHide(Ui::MakeConfirmBox({
		// XP walk: designated -> positional (C7555). ConfirmBoxArgs: text, confirmed,
		// cancelled, confirmText, cancelText, confirmStyle, cancelStyle, labelStyle.
		tr::lng_emoji_remove_group_set(), // text
	...
	const auto push = [&](uint64 setId, bool installed) {
		...
		for (const auto document : list) {
			...
			if (_restrictedCustomList.contains(id.documentId)) {
				...
				set.push_back({
					// XP walk: designated -> positional (C7555). CustomOne:
					// collectible0, custom1, document2, emoji3.
					{}, // collectible
					resolveCustomEmoji(id, document, setId), // custom
					document, // document
					Ui::Emoji::Find(sticker->alt), // emoji
				});
		...
		_custom.push_back({
			setId, // id
			it->second.get(), // set
			it->second->lookupThumbnailDocument(), // thumbnailDocument
			...
			{}, // expanded
			canRemove, // canRemove
			premium && premiumMayBeBought, // premiumRequired
		});
	};
...
not_null<Ui::Text::CustomEmoji*> EmojiListWidget::resolveCustomEmoji(
	...
	return _customEmoji.emplace(
		id,
		CustomEmojiInstance{ std::move(instance), setId } // emoji, setId
...
not_null<Ui::Text::CustomEmoji*> EmojiListWidget::resolveCustomRecent(
	...
	return _customEmoji.emplace(
		id,
		CustomEmojiInstance{
			std::move(custom), // emoji
			RecentEmojiSectionSetId(), // setId
			true, // recentOnly
		}
	).first->second.emoji.get();
}
...
void EmojiListWidget::refreshEmojiStatusCollectibles() {
	...
	for (const auto &status : list) {
		...
		set.push_back({
			...
			resolveCustomEmoji(status, document, setId), // custom
			document, // document
			sticker ? Ui::Emoji::Find(sticker->alt) : nullptr, // emoji
		});
	}
	if (set.empty()) {
		return;
	...
	_custom.push_back({
		setId, // id
		collectibles, // set
		nullptr, // thumbnailDocument
		collectibles->title, // title
		std::move(set), // list
		{}, // ripple
		{}, // painted
		{}, // expanded
		false, // canRemove
		!session().premium(), // premiumRequired
	});
}
...
void EmojiListWidget::updateSelected() {
	...
	if (p.y() >= info.top && p.y() < info.rowsTop) {
		...
		if (sx >= 0 && sx < _columnCount * _singleSize.width()) {
			...
			if (index < info.count) {
				newSelected = OverEmoji{ section, index }; // section, index
			}
		}
	}
	setSelected(newSelected);
