	...
	case Notification::Reinit: {
		enumerateIcons([&](const IconInfo &info) {
			...
			if (icon.setId != setId || !icon.webm) {
				...
				icon.webm->start({ { icon.pixw, icon.pixh }, {}, {}, {}, {}, true });
			}
...
void StickersListWidget::fillContextMenu(
	...
	if (auto sticker = std::get_if<OverSticker>(&selected)) {
		...
		const auto send = [=](Api::SendOptions options) {
			_chosen.fire({ document, options, options.scheduled
...
Ui::MessageSendingAnimationFrom StickersListWidget::messageSentAnimationInfo(
	...
	return { Ui::MessageSendingAnimationFrom::Type::Sticker, session().data().nextLocalMessageId(), mapToGlobal(
	...
	if (!v::is_null(pressed) && pressed == _selected) {
		if (auto sticker = std::get_if<OverSticker>(&pressed)) {
			...
			if (e->modifiers() & Qt::ControlModifier) {
				...
				_chosen.fire({ document, {}, messageSentAnimationInfo(
						sticker->section,
						sticker->index,
						document) });
			}
...
void StickersListWidget::removeMegagroupSet(bool locally) {
	...
	controller()->show(Ui::MakeConfirmBox({ tr::lng_stickers_remove_group_set(), crl::guard(this, [this, group = _megagroupSet] {
...
void StickersListWidget::removeSet(uint64 setId) {
	...
	const auto text = tr::lng_stickers_remove_pack(
		...
		lt_sticker_pack,
		set->title);
	controller()->show(Ui::MakeConfirmBox({ text, crl::guard(this, [=](Fn<void()> &&close) {
			close();
			const auto &sets = session().data().stickers().sets();
			...
			_removingSetId = 0;
			_checkForHide.fire({});
		}), tr::lng_stickers_remove_pack_confirm() }), Ui::LayerOption::KeepOther);
}
