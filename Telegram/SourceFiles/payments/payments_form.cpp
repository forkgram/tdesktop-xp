...
[[nodiscard]] Ui::Address ParseAddress(const MTPPostAddress &address) {
	return address.match([](const MTPDpostAddress &data) {
		return Ui::Address{
			qs(data.vstreet_line1()),
			qs(data.vstreet_line2()),
			qs(data.vcity()),
			qs(data.vstate()),
			qs(data.vcountry_iso2()),
			qs(data.vpost_code()),
		};
	});
}
	...
	return ranges::views::all(
		...
		return price.match([&](const MTPDlabeledPrice &data) {
			return Ui::LabeledPrice{
				qs(data.vlabel()),
				ParsePriceAmount(data.vamount().v),
			};
		});
	}) | ranges::to_vector;
}
...
void Form::processReceipt(const MTPDpayments_paymentReceipt &data) {
	...
	_paymentMethod.savedCredentials = { { "(used)", qs(data.vcredentials_title()) } };
	_paymentMethod.savedCredentialsIndex = 0;
	fillPaymentMethodInformation();
	_updates.fire(FormReady{});
}
...
void Form::processReceipt(const MTPDpayments_paymentReceiptStars &data) {
	...
	const auto receiptData = CreditsReceiptData{
		...
		CreditsAmount(data.vtotal_amount().v), // credits (@5 now CreditsAmount)
		data.vdate().v, // date (@6)
	};
	// XP walk: designated -> positional (C7555)
...
void Form::processInvoice(const MTPDinvoice &data) {
	...
	// XP walk: range-v3's transform|to_vector chain fails on v141_xp -> plain loop;
	// Invoice's designated init skipped tipsSelected/receipt/provider -> {} gaps.
	auto suggestedTips = std::vector<int64>();
	suggestedTips.reserve(suggested.size());
	for (const auto &amount : suggested) {
		suggestedTips.push_back(ParsePriceAmount(amount.v));
	}
	_invoice = Ui::Invoice{
		std::move(_invoice.cover),

		ParsePrices(data.vprices()),
		ranges::views::all(
			suggested
		...
		ParsePriceAmount(data.vmax_tip_amount().value_or_empty()),
		{}, // tipsSelected
		qs(data.vcurrency()),
		{}, // receipt

		data.is_name_requested(),
		data.is_phone_requested(),
		data.is_email_requested(),
		data.is_shipping_address_requested(),
		data.is_recurring(),
		data.is_flexible(),
		data.is_test(),
		{}, // provider
...
void Form::processDetails(const MTPDpayments_paymentForm &data) {
	...
	_details = FormDetails{
		data.vform_id().v,
		qs(data.vurl()),
		qs(data.vnative_provider().value_or_empty()),
		{}, // termsBotUsername
		std::move(nativeParamsJson),
		uint64(data.vbot_id().v),
		uint64(data.vprovider_id().v),
		data.is_can_save_credentials(),
		data.is_password_missing(),
	};
...
void Form::processSavedInformation(const MTPDpaymentRequestedInfo &data) {
	...
	_savedInformation = _information = Ui::RequestedInformation{
		defaultPhone(),
		defaultCountry(),
		{}, // save (skipped by upstream designated init)
		qs(data.vname().value_or_empty()),
		qs(data.vphone().value_or_empty()),
		qs(data.vemail().value_or_empty()),
		address ? ParseAddress(*address) : Ui::Address(),
	};
}
...
void Form::refreshSavedPaymentMethodDetails() {
	...
	if (entered) {
		_paymentMethod.ui.savedMethods.push_back({ {}, entered.title });
	}
	for (const auto &item : list) {
		_paymentMethod.ui.savedMethods.push_back({ item.id, item.title });
	}
...
void Form::fillStripeNativeMethod(QJsonObject object) {
	...
	_paymentMethod.ui.native = Ui::NativeMethodDetails{
		true,
		value(u"need_country").toBool(),
		value(u"need_zip").toBool(),
		value(u"need_cardholder_name").toBool(),
	};
}
...
void Form::submit() {
	...
	if (index < list.size() && password.isEmpty()) {
		...
		_updates.fire(BotTrustRequired{
			_session->data().user(_details.botId),
			_session->data().user(_details.providerId),
		});
		return;
	}
