...
not_null<RpWidget*> EditInformation::setupContent() {
	...
	if (_invoice.isShippingAddressRequested) {
		_street1 = add({
			{},
			tr::lng_payments_address_street1(),
			_information.shippingAddress.address1,
			RangeLengthValidator(1, kMaxStreetSize),
		});
		_street2 = add({
			{},
			tr::lng_payments_address_street2(),
			_information.shippingAddress.address2,
			MaxLengthValidator(kMaxStreetSize),
		});
		_city = add({
			{},
			tr::lng_payments_address_city(),
			_information.shippingAddress.city,
			RangeLengthValidator(kMinCitySize, kMaxCitySize),
		});
		_state = add({
			{},
			tr::lng_payments_address_state(),
			_information.shippingAddress.state,
		});
		_country = add({
			FieldType::Country,
			tr::lng_payments_address_country(),
			_information.shippingAddress.countryIso2,
			RequiredFinishedValidator(),
			showBox,
			{},
			{},
			_information.defaultCountry,
		});
		_postcode = add({
			{},
			tr::lng_payments_address_postcode(),
			_information.shippingAddress.postcode,
			RangeLengthValidator(1, kMaxPostcodeSize),
		});
	}
	if (_invoice.isNameRequested) {
		_name = add({
			...
			_information.name,
			RangeLengthValidator(1, kMaxNameSize),
		});
	}
	if (_invoice.isEmailRequested) {
		_email = add({
			...
			_information.email,
			RangeLengthValidator(1, kMaxEmailSize),
		});
	}
	if (_invoice.isPhoneRequested) {
		_phone = add({
			...
			_information.phone,
			RangeLengthValidator(1, kMaxPhoneSize),
			{},
			{},
			_information.defaultPhone,
		});
	}
...
RequestedInformation EditInformation::collect() const {
	return {
		...
		_phone ? _phone->value() : QString(),
		_email ? _email->value() : QString(),
		{
			_street1 ? _street1->value() : QString(),
			_street2 ? _street2->value() : QString(),
			_city ? _city->value() : QString(),
			_state ? _state->value() : QString(),
			_country ? _country->value() : QString(),
			_postcode ? _postcode->value() : QString(),
		},
	};
}
