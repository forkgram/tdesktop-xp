...
[[nodiscard]] SimpleFieldState CleanMoneyState(
	...
	if (decimalPosition < 0) {
		...
		if (state.value.size() > maxLength) {
			state = {
				state.value.mid(0, maxLength),
				std::min(state.position, maxLength),
			};
		}
	}
	...
	state = {
		state.value.mid(skip),
		std::max(state.position - skip, 0),
	};
	if (decimalPosition >= 0) {
		...
		if (decimalPosition > digitsLimit) {
			state = {
				(state.value.mid(0, digitsLimit)
					+ state.value.mid(decimalPosition)),
				(state.position > digitsLimit
					? std::max(
		...
		state = {
			state.value.mid(0, digitsLimit),
			std::min(state.position, digitsLimit),
		};
	}
	return state;
}
...
[[nodiscard]] MaskedInputField *CreateMoneyField(
	...
	const auto state = wrap->lifetime().make_state<State>(State{
		LookupCurrencyRule(config.currency),
		st::paymentsMoneyField,
	});
