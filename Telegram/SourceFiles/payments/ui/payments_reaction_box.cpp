...
void SelectShownPeer(
	...
	for (const auto &entry : mine) {
		...
		const auto updateUserpic = [=] {
			...
			// XP walk: designated -> named-local (C7555; WhoReactedEntryData skips
			// date@1, customEntityData@3).
			auto entryData = Ui::WhoReactedEntryData();
			entryData.text = entry.name;
			entryData.type = ((id == selected)
				? Ui::WhoReactedType::RefRecipientNow
				: Ui::WhoReactedType::RefRecipient);
			entryData.userpic = actions->at(index).userpic->image(size);
			entryData.callback = [=] { callback(id); };
			actions->at(index).action->setData(std::move(entryData));
		};
	...
	rpl::combine(
		...
		if (list.empty()) {
			...
			for (const auto &entry : list) {
				// XP walk: designated -> positional (C7555). Key = TopReactorKey: photo, count, name.
				const auto key = Key{
					entry.photo, // photo
					entry.count, // count
					entry.name, // name
...
StarSelectDiscreter StarSelectDiscreterForMax(int max) {
	...
	// XP walk: designated -> positional (C7555). StarSelectDiscreter: ratioToValue, valueToRatio.
	return {
...
void PaidReactionSlider(
	...
	const auto update = [=](int count) {
		if (activeFgOverride) {
			...
			// XP walk: designated -> named-local (C7555; ColorOverrides skips
			// activeFg@0, inactiveFg@5).
			auto overrides = MediaSlider::ColorOverrides();
			overrides.activeBg = color;
			overrides.activeBorder = color;
			overrides.seekFg = st::groupCallMembersFg->c;
			overrides.seekBorder = color;
			overrides.inactiveBorder = Qt::transparent;
			slider->setColorOverrides(std::move(overrides));
		}
	};
...
not_null<Premium::BubbleWidget*> AddStarSelectBubble(
	...
	auto bubbleRowState = rpl::duplicate(value) | rpl::map([=](int value) {
		...
		// XP walk: designated -> positional (C7555). BubbleRowState: counter,
		// ratio (animateFromZero@2/dynamic@3 keep in-class defaults false).
