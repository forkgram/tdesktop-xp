...
void TryAddingPaidReaction(
	...
	Settings::MaybeRequestBalanceIncrease(
		show,
		count,
		// XP walk: designated -> positional (C7555). SmallBalanceVideoStream: streamerId.
		Settings::SmallBalanceVideoStream{ call->peer()->id },
		done);
}
...
void ShowPaidReactionDetails(
	...
	const auto add = [&](const Data::MessageReactionsTopPaid &entry) {
		...
		top.push_back({
			...
			peer ? uint64(peer->id.value) : 0, // barePeerId
			int(entry.count), // count
			peer ? open : Fn<void()>(), // click
			(entry.my == 1), // my
		});
	};
	...
	const auto myAdd = [&](PeerData *peer) {
		...
		// XP walk: designated -> named-local (C7555; skips .top field 2).
	...
	state->selectBox = show->show(Ui::MakePaidReactionBox({
		// XP walk: designated -> positional (C7555). PaidReactionBoxArgs grew in
		// v6.3.0: min@0/explicitlyAllowed@1 (leading) + colorings@8 (mid) gap-filled
		// to defaults; not_null session@5 rules out a named-local.
		0, // min
		0, // explicitlyAllowed
		chosen, // chosen
		max, // max
