	...
	for (const auto &item : draft.items) {
		...
		if (genClientSideMessage) {
			...
			history->addNewLocalMessage({
				...
				// +markup@11/suggest@12 (v5.16.0); forwarded msgs carry no effect.
				newId.msg, // id
				flags, // flags
				...
				{}, // groupedId
				{}, // effectId (forwarded messages carry no effect)
				{}, // markup
...
void ApiWrap::editMedia(
	...
	_fileLoader->addTask(std::make_unique<FileLoadTask>(FileLoadTask::Args{
		...
		(file.videoCover // videoCover
			? std::make_unique<FileLoadTask>(FileLoadTask::Args{
				&session(), // session
				file.videoCover->path, // filepath
				file.videoCover->content, // content
				std::move(file.videoCover->information), // information
				nullptr, // videoCover
...
void ApiWrap::sendMessage(
	...
	while (TextUtilities::CutPart(sending, left, messageLengthLimit)
		...
		lastMessage = history->addNewLocalMessage({
			// XP walk: designated -> positional (C7555). +scheduleRepeatPeriod@5 (v6.3.0);
			// starsPaid@7, effectId@11, markup@12, suggest@13.
			newId.msg, // id
			flags, // flags
			NewMessageFromId(action), // from
			action.replyTo, // replyTo
