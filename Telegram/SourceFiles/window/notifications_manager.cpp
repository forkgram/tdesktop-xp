	...
	if (const auto session = system()->findSession(id.contextId.sessionId)) {
		...
		if (!options.draft.text.isEmpty()) {
			...
			auto draft = std::make_unique<Data::Draft>(
				...
				FullReplyTo{
					// XP walk: designated -> positional (messageId, quote, storyId, topicRootId, monoforumPeerId).
					replyToId, // messageId
					{}, // quote
					{}, // storyId
...
void NativeManager::doShowNotification(NotificationFields &&fields) {
		...
		if (const auto markup = item->inlineReplyMarkup()) {
			...
			for (auto row = 0; row != rowsCount; ++row) {
				...
				for (auto col = 0; col != colsCount; ++col) {
					...
					if (button.type == ButtonType::Callback) {
						actions.push_back({
							u"callbackButton:%1:%2"_q.arg(row).arg(col), // id
							button.text, // text
						});
					}
				}
	...
	// XP walk: designated inits (C++20, C7555) -> positional; all NotificationInfo
	// fields in declared order (peer, topicRootId, monoforumPeerId, itemId, title,
	// subtitle, message, sound, options, actions).
	doShowNativeNotification({
		item->history()->peer, // peer
		item->topicRootId(), // topicRootId
		(item->history()->amMonoforumAdmin()
			? item->sublistPeerId()
			: PeerId()), // monoforumPeerId (v5.15.0)
		item->id, // itemId
