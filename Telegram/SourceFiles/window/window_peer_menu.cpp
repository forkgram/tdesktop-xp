...
void Filler::addToggleMuteSubmenu(bool addSeparator) {
	...
	if (addSeparator) {
		_addAction(PeerMenuCallback::Args{ {}, {}, {}, {}, {}, {}, {}, {}, {}, true }); // isSeparator@9 (v6.4.2 dropped triggerFilter@7 -> tail shifted; was @10) (v5.16: +make@5)
	}
}
...
void Filler::addToggleFolder() {
	...
	// text, handler, icon, separatorSt, fillSubmenu, ... ({} = separatorSt).
...
void ShowDisableSharingBox(
	...
	controller->show(Box([=](not_null<Ui::GenericBox*> box) {
		...
		auto icon = Settings::CreateLottieIcon(
			box,
			{
				// XP walk: designated -> positional (C7555). IconDescriptor order.
				u"stop"_q, // name
				{}, // path
				{}, // json
				{}, // color
	...
	_addAction(disabledNow
		...
		if (controller->showFrozenError()) {
			...
			if (willBeRequest) {
				...
				args.text = tr::lng_enable_sharing_request_text(
					tr::now,
					lt_name,
				...
				args.confirmText = tr::lng_enable_sharing_request_button();
				args.title = tr::lng_enable_sharing_request_title();
...
void Filler::addViewAsMessages() {
	...
	_addAction({
		...
		std::move(make), // make
		{}, // submenuSt
		parentHideRequests->events() | to_instant, // hideRequests
	});
}
...
void Filler::fillArchiveActions() {
	...
	if (_request.section == Section::ChatsList) {
		...
		}
		{
			...
			_addAction(text, [=] {
				if (!inmenu) {
					// XP walk: Toast::Config st/duration sit past the move-only
					// `content` field; positional init cannot map. Named local.
					auto config = Ui::Toast::Config();
					config.text = {
	...
	_addAction({ {}, {}, {}, {}, {}, {}, {}, {}, {}, true }); // isSeparator@9 (v6.4.2 dropped triggerFilter@7 -> tail shifted; was @10) (10 gaps; make@5 added v5.16)
...
void PeerMenuDeleteTopicWithConfirmation(
	...
	controller->show(Box([=](not_null<Ui::GenericBox*> box) {
		...
		// XP walk: designated init -> named local (C7555). ConfirmBoxArgs.
		auto args = Ui::ConfirmBoxArgs();
		args.text = tr::lng_forum_topic_delete_sure(tr::now);
		args.confirmed = callback;
		args.confirmText = tr::lng_box_delete();
		args.confirmStyle = &st::attentionBoxButton;
		args.labelPadding = st::boxRowPadding;
		Ui::ConfirmBox(box, std::move(args));
	}));
}
	...
	auto callback = [=](not_null<Data::Thread*> thread) {
		...
		navigation->parentController()->show(
			Ui::MakeConfirmBox({ tr::lng_forward_share_contact(
					tr::now,
					lt_recipient,
					recipient), // text (XP walk: ConfirmBoxArgs designated -> positional)
				[state](Fn<void()> &&close) { // confirmed
					state->share({});
					close();
				},
				v::null, // cancelled
	...
	*weak = navigation->parentController()->show(
		Box<PeerListBox>(
			std::make_unique<ChooseRecipientBoxController>(
				ChooseRecipientArgs{ // XP walk: designated -> positional (C7555)
...
void PeerMenuTodoWantsPremium(TodoWantsPremium type) {
	...
	// content member -> positional aggregate init impossible).
	...
	toast.filter = filter;
	toast.duration = kToastDuration;
	window->uiShow()->showToast(std::move(toast));
}
...
object_ptr<Ui::BoxContent> PrepareChooseRecipientBox(
			...
			if (shown) {
				...
				state->submit = [=](Api::SendOptions options) {
					...
					if (!waiting.empty()) {
						...
						// XP walk: designated -> positional. SendPaymentDetails: messages, stars.
	...
	// XP walk: take theirs' refactor (Prepare builds Box; controller @L1975)
...
base::weak_qptr<Ui::BoxContent> ShowForwardMessagesBox(
	...
	const auto state = [&] {
		...
		boxRaw->setForwardOptions({ // XP walk: designated -> positional (C7555)
			sendersCount, // sendersCount
			captionsCount, // captionsCount
			(draft.options != Data::ForwardOptions::PreserveInfo), // dropNames
			(draft.options
				== Data::ForwardOptions::NoNamesAndCaptions), // dropCaptions
		});
...
base::weak_qptr<Ui::BoxContent> ShowForwardMessagesBox(
	...
	return ShowForwardMessagesBox(
		navigation,
		Data::ForwardDraft{ std::move(items) },
		std::move(successCallback));
}
...
base::weak_qptr<Ui::BoxContent> ShowShareGameBox(
	...
	*weak = navigation->parentController()->show(Box<PeerListBox>(
		std::make_unique<ChooseRecipientBoxController>(ChooseRecipientArgs{ // XP walk: designated -> positional (C7555)
			&navigation->session(), // session
			std::move(chosen), // callback
...
void PeerMenuAddChannelMembers(
	...
	api->chatParticipants().requestForAdd(channel, crl::guard(navigation, [=](
		...
		const auto &[availableCount, list] = Api::ChatParticipants::Parse(
			channel,
			data);
		auto already = std::vector<UserData*>();
		for (const auto &p : list) {
			const auto user = p.isUser()
				? channel->owner().userLoaded(p.userId())
				: nullptr;
			if (user) {
				already.push_back(user);
			}
		}
...
void ToggleMessagePinned(
	...
	if (pin) {
		...
		navigation->parentController()->show(
			Ui::MakeConfirmBox({ tr::lng_pinned_unpin_sure(), callback, {}, tr::lng_pinned_unpin() }),
			Ui::LayerOption::CloseOther);
	}
}
...
void HidePinnedBar(
	...
	navigation->parentController()->show(
		Ui::MakeConfirmBox({ tr::lng_pinned_hide_all_sure(), callback, {}, tr::lng_pinned_hide_all_hide() }),
		Ui::LayerOption::CloseOther);
}
...
void UnpinAllMessages(
	...
	navigation->parentController()->show(
		Ui::MakeConfirmBox({ tr::lng_pinned_unpin_all_sure(), callback, {}, tr::lng_pinned_unpin() }),
		Ui::LayerOption::CloseOther);
}
...
void ToggleHistoryArchived(
	...
	const auto callback = [=] {
		...
		// switched Ui::Toast::Show(...) to show->showToast(...). Theirs adds
		// iconLottie + iconLottieSize.
		auto config = Ui::Toast::Config();
		config.text = { archived
			? tr::lng_archived_added(tr::now)
			: tr::lng_archived_removed(tr::now) };
		config.iconLottie = archived
...
void AddSeparatorAndShiftUp(const PeerMenuCallback &addAction) {
	// XP walk: designated -> positional (C7555)
	addAction({ {}, {}, {}, &st::popupMenuExpandedSeparator.menu.separator });
			...
			[=] {
				...
				if (!phrase.empty()) {
					...
					toast.filter = ChatHelpers::ForwardedToSavedMessagesFilter(
						session);
					toast.iconLottie = ChatHelpers::ForwardedMessagePhraseIcon({
						1, // toCount
						false, // singleMessage
						session->user(), // to1
					});
