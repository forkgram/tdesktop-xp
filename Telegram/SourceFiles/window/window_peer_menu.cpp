...
void PeerMenuDeleteContact(not_null<UserData*> user) {
	...
	Ui::show(Ui::MakeConfirmBox({ text, deleteSure, {}, tr::lng_box_delete() }));
}
	...
	auto callback = [=](not_null<PeerData*> peer) {
		...
		Ui::show(
			Ui::MakeConfirmBox({ tr::lng_forward_share_contact(
					tr::now,
					lt_recipient,
					recipient), [peer, user, navigation] {
					const auto history = peer->owner().history(peer);
					navigation->showPeerHistory(
						history,
...
void ToggleMessagePinned(
	...
	if (pin) {
		...
		Ui::show(Ui::MakeConfirmBox({ tr::lng_pinned_unpin_sure(), callback, {}, tr::lng_pinned_unpin() }));
	}
}
...
void HidePinnedBar(
	...
	Ui::show(Ui::MakeConfirmBox({ tr::lng_pinned_hide_all_sure(), callback, {}, tr::lng_pinned_hide_all_hide() }));
}
...
void UnpinAllMessages(
	...
	Ui::show(Ui::MakeConfirmBox({ tr::lng_pinned_unpin_all_sure(), callback, {}, tr::lng_pinned_unpin() }));
}
