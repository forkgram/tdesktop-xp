	...
	if (isMuted) {
		const auto text = tr::lng_context_unmute(tr::now)
			...
			+ Ui::FormatMuteForTiny(peer->notifyMuteUntil().value_or(0)
				- base::unixtime::now());
		addAction(text, [=] {
			peer->owner().notifySettings().update(peer, { true });
		}, &st::menuIconUnmute);
	} else {
		const auto show = std::make_shared<Window::Show>(controller);
		addAction(PeerMenuCallback::Args{ tr::lng_context_mute(tr::now), nullptr, peer->owner().notifySettings().sound(peer).none
				? &st::menuIconSilent
	...
	auto callback = [=](not_null<PeerData*> peer) {
		...
		navigation->parentController()->show(
			Ui::MakeConfirmBox({ tr::lng_forward_share_contact(
					tr::now,
					lt_recipient,
					recipient), [peer, user, navigation](Fn<void()> &&close) {
					const auto history = peer->owner().history(peer);
					navigation->showPeerHistory(
						history,
