...
BackgroundRects ComputeBackgroundRects(QSize fillSize, QSize imageSize) {
	if (uint64(imageSize.width()) * fillSize.height() > uint64(imageSize.height()) * fillSize.width()) {
		...
		return {
			QRect((imageSize.width() - takewidth) / 2, 0, takewidth, imageSize.height()),
			QRect(int((fillSize.width() - takewidth * pxsize) / 2.), 0, qCeil(takewidth * pxsize), fillSize.height()),
		};
	} else {
		...
		return {
			QRect(0, (imageSize.height() - takeheight) / 2, imageSize.width(), takeheight),
			QRect(0, int((fillSize.height() - takeheight * pxsize) / 2.), fillSize.width(), qCeil(takeheight * pxsize)),
		};
	}
}
