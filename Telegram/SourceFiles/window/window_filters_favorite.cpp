...
base::options::option<QString> FavoriteLink({
	kOptionFolderFavoriteLink, // id
	"Favorite folder button", // name
	"Favorite button at the bottom of the folders strip " // description
...
void FolderFavoriteButton::paintEvent(QPaintEvent *e) {
	...
	if (mode != Mode::IconsOnly) {
		...
		// churns fields between versions, so set them by name.
		auto context = Ui::Text::PaintContext();
		context.position = { _st.textSkip, _st.textTop };
		context.availableWidth = width() - 2 * _st.textSkip;
		context.align = style::al_top;
		context.elisionLines = kMaxLabelLines;
		_label.draw(p, context);
	}
}
...
void FolderFavoriteButton::setLink(const QString &link) {
	...
	case Category::ChatFolder:
		apply({
			Ui::MakeIconThumbnail(st::foldersCustom), // thumbnail
			tr::lng_filters_title(tr::now), // label
		});
		break;
	case Category::Settings: {
		const auto content = SettingsContent(parsed.value);
		apply({
			Ui::MakeIconThumbnail(*content.icon), // thumbnail
			content.label, // label
		});
		break;
	}
...
void FolderFavoriteButton::resolveInvite(const QString &hash) {
	...
	const auto done = [=](const MTPChatInvite &result) {
		...
		result.match([&](const MTPDchatInvite &data) {
			...
			apply({
				std::move(thumbnail), // thumbnail
				qs(data.vtitle()), // label
			});
...
void FolderFavoriteButton::resolveStickerSet(const QString &shortName) {
	...
	_requestId = _api.request(MTPmessages_GetStickerSet(
		Data::InputStickerSet(
			...
			StickerSetIdentifier{ {}, {}, shortName }), // id, accessHash, shortName
		MTP_int(0)
		...
		result.match([&](const MTPDmessages_stickerSet &data) {
			...
			apply({
				Ui::MakeDocumentThumbnail( // thumbnail
					document,
...
void FolderFavoriteButton::showGeneric() {
	apply({
		Ui::MakeIconThumbnail(st::foldersFavorite), // thumbnail
		tr::lng_info_link_label(tr::now), // label
	});
}
...
void FolderFavoriteButton::openLink() {
	...
	UrlClickHandler::Open(
		...
		// itemId0, elementDelegate1, sessionWindow2.
		{}, // itemId
		{}, // elementDelegate
		base::make_weak(_controller.get()), // sessionWindow
		}));
}
...
void FolderFavoriteButton::showMenu() {
	...
	addAction({
		...
		// text0, handler1, icon2, separatorSt3, fillSubmenu4, make5,
		// submenuSt6, hideRequests7, addTopShift8, isSeparator9, isAttention10.
		tr::lng_filters_context_remove(tr::now), // text
		[=] { FavoriteLink.set(QString()); }, // handler
