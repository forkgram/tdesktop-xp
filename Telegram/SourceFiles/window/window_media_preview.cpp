...
void MediaPreviewWidget::paintEvent(QPaintEvent *e) {
	...
	const auto frame = (_lottie && _lottie->ready())
		? _lottie->frameInfo({
			...
			dimensions * factor, // box
			((_document && _document->emojiUsesTextColor())
				? st::windowFg->c
...
QPixmap MediaPreviewWidget::currentImage() const {
	const auto blur = Images::PrepareArgs{ {}, Images::Option::Blur };
	if (_document) {
		...
		if (sticker && !webm) {
			...
			if (gif && gif->started()) {
				const auto paused = _controller->isGifPausedAtLeastFor(
					Window::GifPauseReason::MediaPreview);
				// XP walk: designated -> named-local (C7555); FrameRequest
				// non-contiguous (frame@0, keepAlpha@6) + corners@4 default-trap.
				auto request = Media::Clip::FrameRequest();
				request.frame = currentDimensions();
				request.keepAlpha = webm;
				return QPixmap::fromImage(
					gif->current(request, paused ? 0 : crl::now()),
					Qt::ColorOnly);
			}
...
void MediaPreviewWidget::startGifAnimation(
	...
	gif->start({ currentDimensions(), {}, {}, {}, {}, _gifWithAlpha });
}
