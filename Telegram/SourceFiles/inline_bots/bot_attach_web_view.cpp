...
[[nodiscard]] std::optional<AttachWebViewBot> ParseAttachBot(
	...
	auto result = bot.match([&](const MTPDattachMenuBot &data) {
		...
		return good
			? AttachWebViewBot{ user, ResolveIcon(session, data), {}, qs(data.vshort_name()), data.is_inactive() } : std::optional<AttachWebViewBot>();
	});
	...
	_addToMenuId = _session->api().request(MTPmessages_GetAttachMenuBot(
		...
		const auto open = [=] {
			...
			request(
				...
				contextPeer,
				bot,
				{ {}, startCommand }); // WebViewButton: text gap, startCommand
			return true;
		};
