...
void FillDisclaimerBox(
	...
	Ui::ConfirmBox(box, {
		tr::lng_mini_apps_disclaimer_text(
			tr::now,
			Ui::Text::RichLangValue), // text
		// XP walk: designated -> positional (C7555); take theirs' cancelled
		callback, // confirmed
		[=](Fn<void()> close) { done(false); close(); }, // cancelled
		...
		{}, // labelStyle
		{}, // labelFilter
		QMargins(padding.left(), 0, padding.right(), 0), // labelPadding
		tr::lng_mini_apps_disclaimer_title(), // title
		{}, // inform
		{}, // strictCancel
	});
...
void WebViewInstance::requestButton() {
	...
	_requestId = _session->api().request(MTPmessages_RequestWebView(
		...
		const auto &data = result.data();
		show({
			// XP walk: designated -> positional (C7555): url, title, queryId, fullscreen.
			qs(data.vurl()), // url
			{}, // title (default)
...
void WebViewInstance::show(ShowArgs &&args) {
	...
	_panelUrl = args.url;
	// XP walk: designated -> positional (C7555). BotWebView::Args order: url,
	// storageId, title, titleBadge, bottom, delegate, menuButtons, fullscreen,
	// allowClipboardRead, downloadsProgress.
	_panel = Ui::BotWebView::Show({
		args.url, // url
		_session->local().resolveStorageIdBots(), // storageId
		std::move(title), // title
		std::move(titleBadge), // titleBadge
		...
		downloads->progress(_bot), // downloadsProgress
	});
		...
		result.match([&](const MTPDkeyboardButtonRequestPeer &data) {
			...
			data.vpeer_type().match([&](
				...
				ShowCreateManagedBotBox({
					...
					bot, // manager
					qs(
						createData.vsuggested_name().value_or_empty()), // suggestedName
					qs(
						createData.vsuggested_username()
							.value_or_empty()), // suggestedUsername
					false, // viaDeeplink
					[=](not_null<UserData*> createdBot) {
						sendPeers({ createdBot });
						auto informArgs = Ui::ConfirmBoxArgs();
						informArgs.text = tr::lng_managed_bot_created_text(
							tr::now,
							lt_parent_name,
							bot->name());
						informArgs.title = tr::lng_managed_bot_created_title(
							tr::now,
							lt_name,
							createdBot->name());
	...
	for (const auto &bot : bots->attachBots()) {
		...
		const auto callback = [=] {
			...
			bots->open({
				bot.user, // bot
				{}, // parentShow
				{ // context: controller, dialogsEntryState, action
					controller, // controller
					{}, // dialogsEntryState
					actionFactory(), // action
				},
				{}, // button
				InlineBots::WebViewSourceAttachMenu(), // source
			});
		};
