...
void Video::prepareThumbnail(QSize size) const {
	...
	if (_thumb.size() != size * style::DevicePixelRatio()) {
		...
		_thumb = thumb->pixNoCache(
			...
			{ {}, Images::Option::TransparentBackground, size });
	}
}
...
void Contact::prepareThumbnail(int width, int height) const {
	...
	_thumb = Image(base::duplicate(*thumb)).pixNoCache(
		...
		{ {}, Images::Option::TransparentBackground, { width, height } });
}
...
void Thumbnail::validateThumbnail(
	...
	_thumb = image->pixNoCache(
		frame * style::DevicePixelRatio(),
		{
			nullptr, // colored
			(Images::Option::TransparentBackground
				| (good ? Images::Option() : Images::Option::Blur)), // options
			size, // outer
		});
	_thumbGood = good;
}
	...
	if (const auto photo = getResultPhoto()) {
		...
		if (_thumb.isNull()) {
			...
			_thumb = Image(base::duplicate(*thumb)).pixNoCache(
				...
				{
					nullptr, // colored
					Images::Option::TransparentBackground, // options
					size, // outer
				});
		}
	}
...
void Game::paint(Painter &p, const QRect &clip, const PaintContext *context) const {
	...
	if (animatedThumb) {
		...
		if (animating) {
			const auto pixmap = _gif->current({ _frameSize, { st::inlineThumbSize, st::inlineThumbSize } }, context->paused ? 0 : context->ms);
			if (_thumb.isNull()) {
				_thumb = pixmap;
				_thumbGood = true;
			}
		...
		if (_gif) {
			if (_gif->state() == State::Error) {
				...
				if (_gif->width() * _gif->height() > kMaxInlineArea) {
					...
					_gif->start({ _frameSize, { st::inlineThumbSize, st::inlineThumbSize } });
				}
			} else if (_gif->autoPausedGif() && !context()->inlineItemVisible(this)) {
				unloadHeavyPart();
			}
