...
void Reactions::setLottie(ImageSet &set) {
	...
	set.icon = Lottie::MakeIcon({ {}, set.media->owner()->filepath(true), set.media->bytes(), {}, QSize(size, size), -1 });
	set.media = nullptr;
}
	...
	return entry.match([&](const MTPDavailableReaction &data) {
		...
		return known
			? std::make_optional(Reaction{
				emoji,
				qs(data.vtitle()),
				_owner->processDocument(data.vstatic_icon()),
	...
	for (const auto &reaction : recent) {
		reaction.match([&](const MTPDmessagePeerReaction &data) {
			...
			if (_list.contains(emoji)) {
				parsed[emoji].push_back(RecentReaction{ owner.peer(peerFromMTP(data.vpeer_id())), data.is_unread(), data.is_big() });
			}
		});
	}
