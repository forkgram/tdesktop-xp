...
[[nodiscard]] std::vector<MyTagInfo> ListFromMTP(
	...
	for (const auto &reaction : list) {
		...
		if (id.empty()) {
			...
			result.push_back({
				// XP walk: designated -> positional (C7555). MyTagInfo: id, title, count.
				id, // id
...
std::optional<Reaction> Reactions::parse(const MTPAvailableReaction &entry) {
	...
	return std::make_optional(Reaction{
		...
		//activateAnimation / activateEffects (unused)
...
void Reactions::send(not_null<HistoryItem*> item, bool addToRecent) {
	...
	for (const auto &reaction : chosen) {
		// XP walk: v5.4.0 filters out paid reactions before sending; keep
		// OURS' manual build (range-v3 0.12 fails on | transform | to<QVector>).
		if (!reaction.paid()) {
	...
	i->second = api.request(MTPmessages_SendReaction(
		...
		MTP_int(id.msg),
		// XP walk: paid filter applied in the manual loop above (range-v3
		// 0.12 fails on theirs' | filter | transform | to<QVector>).
		...
		if (paid->sending > 0) {
			finishPaidSending({
				...
				MaybeShownPeer(
					...
					paid->sendingShownPeer), // shownPeer
			}, false);
		}
	}
}
...
PaidReactionSend MessageReactions::startPaidSending() {
	...
	return {
		...
		true, // valid
		MaybeShownPeer(_paid->sendingPrivacySet, _paid->sendingShownPeer), // shownPeer
	};
}
