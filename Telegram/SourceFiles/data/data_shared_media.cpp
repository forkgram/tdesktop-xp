...
rpl::producer<SparseIdsMergedSlice> SharedScheduledMediaViewer(
	...
	return rpl::single(rpl::empty) | rpl::then(
		...
		// XP walk: range-v3 0.12 + v141_xp can't convert this filter view to a
		// vector (basic_iterator const-cursor C2665) -- use a plain loop.
		auto items = std::vector<HistoryItem*>();
		for (const auto &fullId : list.ids) {
			const auto item = session->data().message(fullId);
			if (item && IsItemGoodForType(item, key.type)) {
