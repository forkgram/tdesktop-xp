...
CloudTheme CloudTheme::Parse(
	...
	return {
		...
		parseSettings ? paper() : std::nullopt,
		parseSettings ? accentColor() : std::nullopt,
		(parseSettings
			? outgoingAccentColor()
			: std::nullopt),
		(parseSettings
	...
	for (const auto &theme : list) {
		theme.match([&](const MTPDchatTheme &data) {
			_chatThemes.push_back({
				qs(data.vemoticon()),
				CloudTheme::Parse(_session, data.vtheme(), true),
				CloudTheme::Parse(_session, data.vdark_theme(), true),
			});
		});
	}
}
