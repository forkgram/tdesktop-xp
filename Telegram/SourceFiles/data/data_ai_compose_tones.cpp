...
AiComposeTone AiComposeTones::parseTone(
	...
	return tone.match([&](const MTPDaiComposeTone &data) {
		auto result = AiComposeTone{
			...
			data.vemoji_id().value_or_empty(), // emojiId
			qs(data.vprompt().value_or_empty()), // prompt
			data.vinstalls_count().value_or_empty(), // installsCount
			data.vauthor_id()
				? UserId(data.vauthor_id()->v)
				: UserId(0), // authorId
			data.is_creator(), // creator
		};
		if (const auto example = data.vexample_english()) {
			example->match([&](const MTPDaiComposeToneExample &d) {
				result.firstExample = AiComposeToneExample{
					Api::ParseTextWithEntities(_session, d.vfrom()), // from
					Api::ParseTextWithEntities(_session, d.vto()), // to
				};
			});
		}
		return result;
		...
		result.emojiId = data.vemoji_id().v;
		result.isDefault = true;
		result.defaultType = qs(data.vtone());
		return result;
	});
}
	...
	_session->api().request(MTPaicompose_GetToneExample(
		...
		result.match([&](const MTPDaiComposeToneExample &data) {
			if (done) {
					...
					Api::ParseTextWithEntities(_session, data.vfrom()), // from
					Api::ParseTextWithEntities(_session, data.vto()), // to
				});
			}
		});
	}).fail([=](const MTP::Error &error) {
