...
void DownloadManager::addLoading(DownloadObject object) {
	...
	data.downloading.push_back({ object, computeNextStartDate(), path, {}, size, (!shownExists
...
void DownloadManager::check(
	...
	if (!path.isEmpty()) {
		...
		_loadingProgress = DownloadProgress{ _loadingProgress.current().ready + readyChange, _loadingProgress.current().total + totalChange };
	}
}
	...
	for (const auto &id : ids) {
		if (const auto item = MessageByGlobalId(id)) {
			...
			if (k != end(data.downloaded)) {
				...
				descriptor.files.emplace(k->path, DocumentDescriptor{ id.sessionUniqueId, document ? document->id : DocumentId(), id.itemId });
				_loaded.remove(item);
...
void DownloadManager::deleteAll() {
	...
	for (auto &[session, data] : _sessions) {
		...
		for (auto &id : base::take(data.downloaded)) {
			...
			descriptor.files.emplace(id.path, DocumentDescriptor{ sessionUniqueId, document ? document->id : DocumentId(), id.itemId });
			if (document) {
				_generatedDocuments.remove(document);
			}
...
void DownloadManager::resolveRequestsFinished(
	...
	for (; data.resolveSentTotal > 0; --data.resolveSentTotal) {
		...
		if (i->download.type == DownloadType::Document
			...
			i->object = std::make_unique<DownloadObject>(DownloadObject{ item, document, photo });
			_loaded.emplace(item);
		}
		_loadedAdded.fire(&*i);
	}
...
void DownloadManager::generateEntry(
	...
	id.object = std::make_unique<DownloadObject>(DownloadObject{ generateFakeItem(document), document });
...
rpl::producer<Ui::DownloadBarProgress> MakeDownloadBarProgress() {
	return Core::App().downloadManager().loadingProgressValue(
		...
		return Ui::DownloadBarProgress{ progress.ready, progress.total };
	});
}
	...
	return [](auto consumer) {
		...
		const auto resolveThumbnailRecursive = [=](auto &&self) -> bool {
			...
			if (const auto image = state->media->thumbnail()) {
				...
				state->thumbnail = Images::Prepare(embed->original(), 0, { {}, Images::Option::Blur });
			}
