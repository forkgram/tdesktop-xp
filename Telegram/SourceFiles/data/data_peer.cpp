...
// XP walk: C7589 - theirs' defaulted friend operator== is C++20; keep OURS'
// manual free operators. operator== now also compares paidEnabled (new in
// v5.4.0) to match theirs' all-member defaulted ==. Take theirs' 3-arg Parse.
bool operator<(
...
AllowedReactions Parse(
	...
	return value.match([&](const MTPDchatReactionsNone &) {
		// XP walk: designated -> positional (C7555). Order: some, maxCount,
		// type, paidEnabled. some={} and type=Some are real defaults.
		return AllowedReactions{
			{}, // some
			maxCount,
			AllowedReactionsType::Some, // type (real default)
			paidEnabled,
		};
	}, [&](const MTPDchatReactionsAll &data) {
		return AllowedReactions{
			// XP walk: designated -> positional (C7555). Order: some,
			// maxCount, type, paidEnabled. Take theirs' fields; some={}.
			{}, // some
			maxCount,
			(data.is_allow_custom()
				? AllowedReactionsType::All
				: AllowedReactionsType::Default), // type
			paidEnabled,
		};
...
void PeerData::setUserpic(
	...
	// XP walk: designated -> positional (C7555); location is field 1
