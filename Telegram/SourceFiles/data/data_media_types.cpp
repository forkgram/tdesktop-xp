...
bool UpdateExtendedMedia(
	...
	return extended.match([&](const MTPDmessageExtendedMediaPreview &data) {
		...
		if (const auto &w = data.vw()) {
			...
			// XP walk: take v5.2.0 photo-size logic; keep != nullptr because our
			// frozen tl::conditional has no .has_value().
			Assert(h != nullptr);
			size = QSize(w->v, h->v);
...
ItemPreview MediaWebPage::toPreview(ToPreviewOptions options) const {
	...
	if (pageTypeWithPreview || !_page->collage.items.empty()) {
		...
		// XP walk: designated -> positional (C7555). ItemPreview{ text, images }.
		if (auto found = FindCachedPreview(
				options.existing,
				_page,
				radius,
...
: Media(parent)
, _invoice{
	// XP walk: designated -> positional (C7555). Invoice field order:
	// receiptMsgId, amount, currency, title, description, extendedMedia,
	// photo, isPaidMedia, isTest. v5.2.0 dropped extendedPreview, made
	// extendedMedia a vector (populated below), added isPaidMedia.
	data.receiptMsgId, // receiptMsgId
	data.amount, // amount
	data.currency, // currency
	data.title, // title
	data.description, // description
	{}, // extendedMedia (empty; cloned in below)
	data.photo, // photo
	data.isPaidMedia, // isPaidMedia
	data.isTest, // isTest
} {
...
bool MediaDice::updateSentMedia(const MTPMessageMedia &media) {
	...
	if (const auto outcome = data.vgame_outcome()) {
		...
		_outcome = Data::DiceGameOutcome{
			int64(data.vton_amount().v), // nanoTon
			int64(data.vstake_ton_amount().v), // stakeNanoTon
			data.vseed().v, // seed
		};
	} else {
		_outcome = {};
		...
		if (!game || !sendAllowed) {
			...
			pack->resolveGameOptions([=](
				...
				if (!options || !window) {
					...
					// XP walk: designated -> positional (C7555); EmojiGameStakeArgs
					// (not default-constructible: session) fields in order.
					showToast(Ui::MakeEmojiGameStakeToast(window->uiShow(), {
						&window->session(), // session
						options.previousSteakNanoTon, // currentStake
						options.milliRewards, // milliRewards
						options.jackpotMilliReward, // jackpotMilliReward
						sendWithStake, // submit
					}));
				}
			});
		}
...
MediaGiftBox::MediaGiftBox(
	...
	GiftType type,
	int64 count) // XP walk: take theirs (int64 count param)
// XP walk: designated -> named-local via IIFE (C7555). Data::GiftCode is
// non-contiguous (count@15, type@16, many fields between) so a positional
// brace-init is unsafe; this delegating ctor sets count + type only.
