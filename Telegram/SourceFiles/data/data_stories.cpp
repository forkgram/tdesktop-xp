...
[[nodiscard]] StoryAlbum FromTL(
	...
	return {
		// XP walk: designated -> positional (C7555). StoryAlbum: id, title, iconPhoto, iconVideo.
		data.valbum_id().v, // id
...
StoriesSourceInfo StoriesSource::info() const {
	return {
		peer->id, // id
		ids.empty() ? 0 : ids.back().date, // last
		uint32(std::min(int(ids.size()), kMaxSegmentsCount)), // count
		uint32(std::min(unreadCount(), kMaxSegmentsCount)), // unreadCount
		(peer->isUser() && peer->asUser()->isPremium()) ? 1U : 0, // premium
		hasVideoStream ? 1U : 0, // hasVideoStream (XP walk: v6.3.0 video-stream)
	};
}

int StoriesSource::unreadCount() const {
	const auto i = ids.lower_bound(StoryIdDates{ readTill + 1 }); // id
...
void Stories::apply(const MTPStoriesStealthMode &stealthMode) {
	...
	_stealthMode = StealthMode{
		data.vactive_until_date().value_or_empty(), // enabledTill
		data.vcooldown_until_date().value_or_empty(), // cooldownTill
	};
}
...
void Stories::sendViewsSliceRequest() {
	...
	_viewsRequestId = api->request(MTPstories_GetStoryViewsList(
		...
		const auto fullId = FullStoryId{
			_owner->session().userPeerId(), // peer
			_viewsStoryId, // story
		};
		if (const auto story = lookup(fullId)) {
...
std::optional<Stories::PeerSourceState> Stories::peerSourceState(
	...
	if (_readTillReceived || (i != end(_readTill))) {
		return PeerSourceState{
			...
			live, // hasVideoStream (XP walk: v6.3.0 video-stream)
		};
	}
	requestReadTills();
	_pendingPeerRecentState[peer] = RecentState{
		maxId, // maxId (XP walk: designated -> positional, C7555)
		live, // hasVideoStream
	};
	return std::nullopt;
}
