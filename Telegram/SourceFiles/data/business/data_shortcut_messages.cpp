...
void ShortcutMessages::updateShortcuts(const QVector<MTPQuickReply> &list) {
	...
	for (auto &[id, shortcut] : _shortcuts.list) {
		...
		if (foundId) {
			...
			changes.push_back({ id, foundId }); // XP walk: designated -> positional (C7555): oldId, newId
		} else {
...
Shortcut ShortcutMessages::parseShortcut(const MTPQuickReply &reply) const {
	...
	return Shortcut{
		...
		BusinessShortcutId(data.vshortcut_id().v), // id
		data.vcount().v, // count
		qs(data.vshortcut()), // name
		localMessageId(data.vtop_message().v), // topMessageId
	};
}
...
BusinessShortcutId ShortcutMessages::emplaceShortcut(QString name) {
	...
	_shortcuts.list.emplace(result, Shortcut{ result, {}, name }); // XP walk: designated -> positional (C7555): id, {}count, name
	return result;
}
