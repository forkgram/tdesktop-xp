...
[[nodiscard]] AwaySettings FromMTP(
	...
	auto result = AwaySettings{
		// XP walk: designated -> positional (C7555)
		FromMTP(owner, data.vrecipients()), // recipients
		{}, // schedule (type == AwayScheduleType::Never)
		data.vshortcut_id().v, // shortcutId
		data.is_offline_only(), // offlineOnly
	};
...
[[nodiscard]] GreetingSettings FromMTP(
	...
	return GreetingSettings{
		// XP walk: designated -> positional (C7555)
		FromMTP(owner, data.vrecipients()), // recipients
		data.vno_activity_days().v, // noActivityDays
		data.vshortcut_id().v, // shortcutId
	};
}
