...
void Panel::startScheduledNow() {
	...
	if (!date) {
		...
		auto owned = ConfirmBox({ (_call->peer()->isBroadcast()
				? tr::lng_group_call_start_now_sure_channel
				: tr::lng_group_call_start_now_sure)(), done, {}, tr::lng_group_call_start_now() });
		*box = owned.data();
...
void Panel::chooseShareScreenSource() {
	...
	auto box = ConfirmBox({ text, done, {}, tr::lng_continue() });
	*shared = box.data();
	showBox(std::move(box));
}
