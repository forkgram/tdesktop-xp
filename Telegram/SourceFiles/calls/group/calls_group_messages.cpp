...
[[nodiscard]] StarsTop ParseStarsTop(
	...
	auto result = StarsTop();
	result.total = int(data.vtotal_stars().v);
	result.topDonors.reserve(list.size());
	for (const auto &entry : list) {
		const auto &fields = entry.data();
		// XP walk: designated -> positional (C7555). StarsTopDonor: peer, stars, my.
		result.topDonors.push_back({
			(fields.vpeer_id() // peer
				? owner->peer(peerFromMTP(*fields.vpeer_id())).get()
				: nullptr),
			int(fields.vstars().v), // stars
			fields.is_my(), // my
		});
	}
	return result;
...
Messages::~Messages() {
	if (_paid.sending > 0) {
		// XP walk: designated -> positional (C7555). PaidReactionSend: count, valid, shownPeer.
		finishPaidSending({
			int(_paid.sending), // count
			true, // valid
			_paid.sendingShownPeer, // shownPeer
		}, false);
	}
}
...
void Messages::send(TextWithTags text, int stars) {
	...
	if (skip) {
		...
		// XP walk: designated -> positional (C7555). Message: id, date, pinFinishDate,
		// peer, text, stars, failed, admin, mine; not_null peer -> no named-local.
		_messages.push_back({
			localId, // id
			0, // date
			0, // pinFinishDate
			...
			stars, // stars
			false, // failed
			(from == _call->peer()) || (creator && from->isSelf()), // admin
			true, // mine
		});
	}
	if (!_call->conference()) {
...
void Messages::sent(uint64 randomId, MsgId realId) {
	...
	// XP walk: designated -> positional (C7555). MessageIdUpdate: localId, realId.
	_idUpdates.fire({ localId, realId });
}
...
void Messages::received(
	...
	if (skip) {
		...
		// loaded here yet.
		// XP walk: designated -> positional (C7555). Message: id, date,
		// pinFinishDate, peer, text, stars, failed, admin, mine; not_null peer
		// makes Message non-default-constructible (no named-local). failed@6=false.
		_messages.push_back({
			id, // id
			date, // date
			PinFinishDate(author, date, stars), // pinFinishDate
			author, // peer
			std::move(text), // text
			stars, // stars
			false, // failed
...
void Messages::reactionsPaidSend() {
	...
	if (skip) {
		...
		// XP walk: designated -> positional (C7555). Message: id, date, pinFinishDate,
		// peer, text, stars, failed, admin, mine; not_null peer -> no named-local.
		// No text on a paid-reaction message -> text@4 = {}.
		_messages.push_back({
			localId, // id
			0, // date
			0, // pinFinishDate
...
Messages::PaidLocalState Messages::starsLocalState() const {
	...
	return { total, my }; // XP walk: designated -> positional (C7555). PaidLocalState.
}
