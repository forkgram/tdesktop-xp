...
void SettingsBox(
	...
	if (rtmp) {
		...
		const auto revokeSure = [=] {
			...
			state->requestId = session->api().request(
				...
				auto data = result.match([&](
					...
					return StartRtmpProcess::Data{ qs(data.vurl()), qs(data.vkey()) };
				});
		...
		const auto revoke = [=] {
			...
			box->getDelegate()->show(Ui::MakeConfirmBox({ tr::lng_group_call_rtmp_revoke_sure(), [=](Fn<void()> &&close) {
					revokeSure();
					close();
				}, {}, tr::lng_group_invite_context_revoke(), {}, {}, {}, &st::groupCallBoxLabel }));
		};
