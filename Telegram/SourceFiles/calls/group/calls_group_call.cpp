...
[[nodiscard]] JoinClientFields ParseJoinResponse(const QByteArray &json) {
	...
	if (document.object().value("stream").toBool()) {
		return JoinBroadcastStream{ document.object().value("rtmp").toBool(), document.object().value("rtmp_stream_url").toString(), document.object().value("rtmp_stream_key").toString() };
	}
		...
		crl::on_main(weak, [=, payload = std::move(payload)] {
			...
			_api.request(MTPphone_JoinGroupCall(
				...
				Ui::ShowMultilineToast({
					nullptr, // parentOverride (field 1)
...
bool GroupCall::tryCreateController() {
	...
	tgcalls::GroupInstanceDescriptor descriptor = {
		nullptr, // threads (1, StaticThreads excluded under DISABLE_WEBRTC)
		tgcalls::GroupConfig{
		},
		[=](tgcalls::GroupNetworkState networkState) {
			crl::on_main(weak, [=] { setInstanceConnected(networkState); });
		},
		[=](const tgcalls::GroupLevelsUpdate &data) {
		...
		nullptr, // onAudioFrame (5)
		_audioInputId.toStdString(),
		_audioOutputId.toStdString(),
		true, // useDummyChannel (8)
		false, // disableIncomingChannels (9)
		...
		[=, call = base::make_weak(this)]( // requestVideoBroadcastPart (15)
				int64_t time,
				int64_t period,
				int32_t channel,
		...
		32, // outgoingAudioBitrateKbit (16)
		false, // disableOutgoingAudioProcessing (17)
		tgcalls::VideoContentType::Generic, // videoContentType (18)
		settings.groupCallNoiseSuppression(), // initialEnableNoiseSuppression (19)
		{}, // videoCodecPreferences (20)
		[=, call = base::make_weak(this)]( // requestMediaChannelDescriptions (21)
		...
		100, // minOutgoingVideoBitrateKbit (22)
	};
...
bool GroupCall::tryCreateScreencast() {
	...
	const auto weak = base::make_weak(&_screenInstanceGuard);
	tgcalls::GroupInstanceDescriptor descriptor = {
		nullptr, // threads (1, StaticThreads excluded)
		tgcalls::GroupConfig{
		},
		[=](tgcalls::GroupNetworkState networkState) {
			crl::on_main(weak, [=] {
		...
		nullptr, // audioLevelsUpdated (4)
		nullptr, // onAudioFrame (5)
		{}, // initialInputDeviceId (6)
		{}, // initialOutputDeviceId (7)
		true, // useDummyChannel (8)
		false, // disableIncomingChannels (9)
		nullptr, // createAudioDeviceModule (10)
		nullptr, // videoCapture (11, _screenCapture excluded)
		nullptr, // getVideoSource (12)
		nullptr, // requestCurrentTime (13)
		nullptr, // requestAudioBroadcastPart (14)
		nullptr, // requestVideoBroadcastPart (15)
		32, // outgoingAudioBitrateKbit (16)
		false, // disableOutgoingAudioProcessing (17)
		tgcalls::VideoContentType::Screencast, // videoContentType (18)
		false, // initialEnableNoiseSuppression (19)
		{}, // videoCodecPreferences (20)
		nullptr, // requestMediaChannelDescriptions (21)
		100, // minOutgoingVideoBitrateKbit (22)
	};
...
void GroupCall::updateRequestedVideoChannels() {
	...
	for (const auto &[endpoint, video] : _activeVideoTracks) {
		...
		channels.push_back({
			participant->ssrc,
			endpointId,
			(params->camera.endpointId == endpointId
