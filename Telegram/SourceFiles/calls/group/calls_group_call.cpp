...
[[nodiscard]] JoinClientFields ParseJoinResponse(const QByteArray &json) {
	...
	if (document.object().value("stream").toBool()) {
		return JoinBroadcastStream{
			document.object().value("rtmp").toBool(),
			{
				document.object().value("rtmp_stream_url").toString(),
				document.object().value("rtmp_stream_key").toString(),
			},
		};
	}
...
: GroupCall(delegate, Group::JoinInfo{ // XP walk: designated -> positional (C7555).
	info.call ? info.call->peer() : info.show->session().user(), // peer
	info.show ? info.show->session().user() : info.call->peer(), // joinAs (v6.3.0)
}, info, info.call
	? info.call->input()
	...
	_screenState.value(
		...
		if (nowActive) {
			if (emitShareScreenError()) {
				...
				// XP walk: VideoCaptureInterface::Create + StaticThreads are
				// WebRTC-only -- screen capture is unavailable on the XP build.
...
bool GroupCall::tryCreateController() {
	...
	// XP walk: designated -> named-local (C7555). tgcalls GroupInstanceDescriptor
	// grew middle fields (statsLogPath, signalBarsUpdated, ssrcActivityUpdated,
	// createWrappedAudioDeviceModule, disableAudioInput, ios_enableSystemMute,
	// e2eEncryptDecrypt, isConference) -> positional no longer maps. WebRTC is
	// disabled on XP: threads / createAudioDeviceModule / videoCapture stay null.
	auto descriptor = tgcalls::GroupInstanceDescriptor();
	descriptor.threads = nullptr; // StaticThreads excluded under DISABLE_WEBRTC
	descriptor.networkStateUpdated = [=](tgcalls::GroupNetworkState networkState) {
		crl::on_main(weak, [=] { setInstanceConnected(networkState); });
	};
	descriptor.audioLevelsUpdated = [=](const tgcalls::GroupLevelsUpdate &data) {
	...
	descriptor.initialInputDeviceId = captureDeviceIdInitial.value.toStdString();
	descriptor.initialOutputDeviceId = playbackDeviceIdInitial.value.toStdString();
	descriptor.createAudioDeviceModule = nullptr; // WebRTC disabled on XP
	descriptor.videoCapture = nullptr; // WebRTC disabled on XP
	descriptor.requestCurrentTime = [=, call = base::make_weak(this)](
	...
	descriptor.requestAudioBroadcastPart = [=, call = base::make_weak(this)](
			int64_t time,
			int64_t period,
	...
	descriptor.requestVideoBroadcastPart = [=, call = base::make_weak(this)](
			int64_t time,
			int64_t period,
			int32_t channel,
	...
	descriptor.initialEnableNoiseSuppression
		= settings.groupCallNoiseSuppression();
	descriptor.videoCodecPreferences = lookupVideoCodecPreferences();
	descriptor.requestMediaChannelDescriptions = [=, call = base::make_weak(this)](
	...
	descriptor.e2eEncryptDecrypt = (_e2eEncryptDecrypt
...
bool GroupCall::tryCreateScreencast() {
	...
	const auto weak = base::make_weak(&_screenInstanceGuard);
	// XP walk: designated -> named-local (C7555); see main descriptor above.
	// WebRTC disabled on XP: threads / createAudioDeviceModule / videoCapture null.
	auto descriptor = tgcalls::GroupInstanceDescriptor();
	descriptor.threads = nullptr; // StaticThreads excluded under DISABLE_WEBRTC
	descriptor.config.need_log = Logs::DebugEnabled();
	descriptor.networkStateUpdated = [=](tgcalls::GroupNetworkState networkState) {
		crl::on_main(weak, [=] {
	...
	descriptor.createAudioDeviceModule = nullptr; // WebRTC disabled on XP
	descriptor.videoCapture = nullptr; // _screenCapture excluded (WebRTC disabled)
	descriptor.videoContentType = tgcalls::VideoContentType::Screencast;
	descriptor.videoCodecPreferences = lookupVideoCodecPreferences();
	descriptor.e2eEncryptDecrypt = (_e2eEncryptDecrypt
		? _e2eEncryptDecrypt->callback()
		: nullptr);
...
void GroupCall::broadcastPartStart(std::shared_ptr<LoadPartTask> task) {
	...
	const auto requestId = _api.request(MTPupload_GetFile(
		...
		result.match([&](const MTPDupload_file &data) {
			...
			finish({
				time,
				TimestampFromMsgId(response.outerMsgId),
				Status::Success,
				std::move(bytes),
			});
...
bool GroupCall::mediaChannelDescriptionsFill(
	...
	for (const auto ssrc : task->ssrcs()) {
		...
		if (const auto byAudio = real->participantPeerByAudioSsrc(ssrc)) {
			add(Channel{
				// XP walk: designated -> positional (C7555). MediaChannelDescription:
				// type, audioSsrc, userId, videoInformation.
				Channel::Type::Audio, // type
				ssrc, // audioSsrc
				int64_t(peerToUser(byAudio->id).bare), // userId
			});
		} else if (!resolved) {
			_unresolvedSsrcs.emplace(ssrc);
...
void GroupCall::updateRequestedVideoChannels() {
	...
	for (const auto &[endpoint, video] : _activeVideoTracks) {
		...
		channels.push_back({
			// XP walk: designated -> positional (C7555). VideoChannelDescription:
			// audioSsrc, userId, endpointId, ssrcGroups, minQuality, maxQuality.
			participant->ssrc, // audioSsrc
			int64_t(peerToUser(participant->peer->id).bare), // userId
			endpointId, // endpointId
			(params->camera.endpointId == endpointId // ssrcGroups
