...
void ScheduleGroupCallBox(
	...
	auto descriptor = Ui::ChooseDateTimeBox(box, {
		...
		send,
		min,
		base::unixtime::serialize(schedule),
		max,
...
void ChooseJoinAsProcess::processList(
	...
	if (!_request->changingJoinAsFrom && (onlyByMe || byAlreadyUsed)) {
		...
		auto box = Ui::MakeConfirmBox({ confirmation, crl::guard(guard, [=] { finish(info); }), {}, (creating
				? tr::lng_create_group_create()
				: tr::lng_group_call_join()), {}, {}, {}, {}, filter });
		box->boxClosing(
		) | rpl::start_with_next([=] {
			_request = nullptr;
		}, _request->lifetime);
