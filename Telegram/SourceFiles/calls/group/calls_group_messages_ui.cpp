...
void ShowDeleteMessageConfirmation(
	...
	show->show(Box([=](not_null<Ui::GenericBox*> box) {
		...
		const auto confirmed = [=](Fn<void()> close) {
			// XP walk: designated -> positional (C7555). MessageDeleteRequest:
			// id, deleteAllFrom, ban, reportSpam.
			callback(MessageDeleteRequest{
				id, // id
				state->all.current() ? from.get() : nullptr, // deleteAllFrom
				state->ban.current() ? from.get() : nullptr, // ban
				state->report.current(), // reportSpam
			});
			close();
		};
		// XP walk: designated -> named-local (C7555; ConfirmBoxArgs sparse/large).
		auto confirmArgs = Ui::ConfirmBoxArgs();
		confirmArgs.text = tr::lng_selected_delete_sure_this();
		confirmArgs.confirmed = confirmed;
		confirmArgs.confirmText = tr::lng_box_delete();
		confirmArgs.labelStyle = &st::groupCallBoxLabel;
...
, _crownHelper(Core::TextContext({ &_show->session() }))
...
void MessagesUi::appendMessage(const Message &data) {
	...
	// XP walk: designated -> positional-minimal + field assign (C7555).
	// MessageView: id@0, sendingId@1, from@2 (not_null -> non-default-
	// constructible, so no named-local); construct the leading not_null, set rest.
	auto &entry = _views.emplace_back(MessageView{ id, 0, peer });
	entry.original = data.text;
	entry.date = data.date;
	entry.stars = data.stars;
	entry.place = donorPlace(peer);
	entry.top = top;
	entry.sending = !data.date;
	entry.admin = data.admin && _streamMode;
	entry.mine = data.mine;
	const auto repaint = [=] {
		repaintMessage(id);
	};
...
void MessagesUi::appendPinned(const Message &data, TimeId now) {
	...
	// XP walk: designated -> positional-minimal + field assign (C7555).
	// PinnedView: id@0, from@1 (not_null -> no named-local); set the rest by field.
	auto &entry = *_pinnedViews.insert(j, PinnedView{ data.id, peer });
	entry.duration = (data.pinFinishDate - data.date) * crl::time(1000);
	entry.end = finishes;
	entry.stars = data.stars;
	entry.place = donorPlace(peer);
	entry.left = left;
	...
	if (place > 0) {
		...
		if (i == _crownEmojiDataCache.end()) {
			i = _crownEmojiDataCache.emplace(
				place,
				// XP walk: designated -> positional (C7555). ImageEmoji: image, margin.
				_crownHelper.imageData(Ui::Text::ImageEmoji{
					CrownMask(place), // image
					st::paidReactCrownMargin, // margin
				})).first;
		}
	...
	_messages->paintRequest() | rpl::on_next([=](QRect clip) {
		...
		for (auto &entry : _views) {
			...
			if (!entry.name.isEmpty()) {
				...
				liveContext.position = { liveLeft, y + padding.top() };
				_liveBadge.draw(p, liveContext);
				...
				const auto adminLeft = x
					...
					- padding.right()
					- _adminBadge.maxWidth();
				auto adminContext = Ui::Text::PaintContext();
				adminContext.position = { adminLeft, y + padding.top() };
				_adminBadge.draw(p, adminContext);
				p.setOpacity(1.);
			}
			...
			textContext.position = { x + textLeft, y + entry.textTop };
			textContext.availableWidth = entry.width - textLeft - textRight;
			...
			if (!entry.price.isEmpty()) {
				...
				priceContext.position = { priceLeft, priceTop };
				priceContext.availableWidth = entry.price.maxWidth();
				entry.price.draw(p, priceContext);
			}
	...
	_pinned->paintRequest() | rpl::on_next([=](QRect clip) {
		...
		for (auto &entry : _pinnedViews) {
			...
			const auto position = QPoint(
				x + userpicPadding.left(),
				y + userpicPadding.top());
			// XP walk: designated -> positional (C7555). Userpic paint args:
			// position, size, shape.
			entry.from->paintUserpic(p, entry.view, {
				position, // position
				userpicSize, // size
			...
			// XP walk: designated -> named-local (Ui::Text::PaintContext).
			auto textContext = Ui::Text::PaintContext();
			textContext.position = { x + leftSkip, y + padding.top() };
