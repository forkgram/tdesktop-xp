...
MembersRowDelegate::IconState MembersRow::computeIconState(
	...
	return {
		// XP walk: designated -> positional (C7555). IconState: speaking, active,
		// muted, mutedByMe, raisedHand, invited, calling, style.
		speaking, // speaking
		active, // active
		muted, // muted
		_mutedByMe, // mutedByMe
		(_state == State::RaisedHand), // raisedHand
		(_state == State::Invited), // invited
		(_state == State::Calling), // calling
		style, // style
	};
}
