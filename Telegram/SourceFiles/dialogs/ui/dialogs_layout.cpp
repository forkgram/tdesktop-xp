...
base::options::toggle DialogsMuteIcon({
	kOptionDialogsMuteIcon, // id
	"Mute icon in dialogs", // name
	"Show a small mute icon next to the chat name " // description
		"for muted chats.",
});
...
void PaintRow(
	...
	const auto paintPeerBadge = [&](int rowNameWidth) {
		const auto badgeWidth = rowBadge.drawGetWidth(p, {
			...
			rowNameWidth, // nameWidth -- XP walk: v5.10.3 uses the paintPeerBadge param
