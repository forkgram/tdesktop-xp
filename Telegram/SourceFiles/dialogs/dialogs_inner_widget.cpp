...
void InnerWidget::paintEvent(QPaintEvent *e) {
	...
	auto context = Ui::PaintContext{
		...
		// rightButton0, chatsFilterTags1, quickActionContext2, st3, topicJumpCache4,
		// folder5, forum6, community7, currentBg8, filter9, topicsExpanded10, now11,
		// searchLowerText12, width13, active14, selected15, topicJumpSelected16,
		// paused17, search18, narrow19, displayUnreadInfo20, insideCommunity21.
		{}, // rightButton
		{}, // chatsFilterTags
		{}, // quickActionContext
		_st, // st
		...
		_openedFolder, // folder
		_openedForum, // forum
		communityModeShown() ? _openedCommunity : nullptr, // community
		currentBg(), // currentBg
		_filterId, // filter
		{}, // topicsExpanded
		ms, // now
		...
		{}, // displayUnreadInfo
		communityModeShown(), // insideCommunity
	};
			...
			if (from < _peerSearchResults.size()) {
				...
				for (; from < to; ++from) {
					...
					paintPeerSearchResult(p, result.get(), {
						// XP walk: designated -> positional (C7555); PaintContext
						// rightButton@0, chatsFilterTags@1, quickActionContext@2 inserted.
						(result->sponsored // rightButton
							? &result->sponsored->button
							: nullptr),
						{}, // chatsFilterTags
						{}, // quickActionContext
...
void InnerWidget::paintPeerSearchResult(
	...
	const auto badgeWidth = result->badge.drawGetWidth(p, {
		...
		result->name.maxWidth(), // nameWidth
		...
		(context.active
			...
			: &st::windowSubTextFg), // direct (XP walk: new field)
		(context.active
