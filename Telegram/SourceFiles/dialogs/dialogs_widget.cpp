...
, _mainMenu({
	...
	object_ptr<MenuUnderButton>(_searchControls), // under
})
...
, _singleMessageSearch(&controller->session()) {
	...
	_cancelSearch->setClickedCallback([=] {
		// XP walk: designated -> positional (C7555). CancelSearchOptions:
		// forceFullCancel@0 (false), jumpBackToSearchedChat@1 (true).
...
void Widget::setupSwipeBack() {
		...
		if (_inner) {
			...
			if (!isRightToLeft) {
				if (const auto key = _inner->calcSwipeKey(top);
					...
					// keepRatioWithinRange@4 gap-filled).
					return Ui::Controls::SwipeHandlerFinishData{
	...
	Ui::Controls::SetupSwipeHandler({
		_innerList, // widget (v6.7.0: was _inner)
		_scroll.data(), // scroll
...
void Widget::toggleFiltersMenu(bool enabled) {
	...
	if (!enabled == !_chatFilters) {
		...
		shadow->show();
		inner->sizeValue() | rpl::on_next([=](const QSize &s) {
			raw->resize(s);
			shadow->setGeometry(
				0,
...
void Widget::escape() {
	// XP walk: cancelSearch designated -> positional (C7555). CancelSearchOptions:
	// forceFullCancel@0 (false), jumpBackToSearchedChat@1 (true).
	if (!cancelSearch({ false, true })) {
...
bool Widget::search(bool inCache, SearchRequestDelay delay) {
	...
	if (trimmed.isEmpty() && !fromPeer && inTags.empty()) {
		...
		if (_searchWithPostsPreview) {
			searchApplyEmpty(
				// XP walk: designated -> positional (C7555):
				// SearchRequestType{ migrated, posts, start, peer }.
				SearchRequestType{ false, true, true, false },
				&_postsProcess);
		}
		_peerSearch.clear();
		_api.request(base::take(_topicSearchRequest)).cancel();
		...
		if (inPeer) {
			...
			_historiesRequest = histories.sendRequest(history, type, [=](
					Fn<void()> finish) {
				const auto type = SearchRequestType{ // XP walk: designated -> positional (C7555)
					false, // migrated
					false, // posts
					true, // start
					true, // peer
