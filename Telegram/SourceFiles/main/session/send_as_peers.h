...
struct SendAsPeer {
	...
	friend inline bool operator==(const SendAsPeer &a, const SendAsPeer &b) {
		return (a.peer == b.peer) && (a.premiumRequired == b.premiumRequired);
	}
	friend inline bool operator!=(const SendAsPeer &a, const SendAsPeer &b) {
		return !(a == b);
	}
	friend inline bool operator<(const SendAsPeer &a, const SendAsPeer &b) {
		return std::tie(a.peer, a.premiumRequired)
			< std::tie(b.peer, b.premiumRequired);
	}
};
