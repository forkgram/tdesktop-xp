...
, _saveSettingsTimer([=] { saveSettings(); }) {
	...
	changes().peerFlagsValue(
		...
		auto view = Ui::PeerUserpicView{ {}, _selfUserpicView };
...
QString Session::createInternalLink(const QString &query) const {
	return createInternalLink(TextWithEntities{ query }).text;
}

QString Session::createInternalLinkFull(const QString &query) const {
	return createInternalLinkFull(TextWithEntities{ query }).text;
}
