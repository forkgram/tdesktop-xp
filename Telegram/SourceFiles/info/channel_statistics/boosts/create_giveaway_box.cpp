...
void AddPremiumTopBarWithDefaultTitleBar(
	...
	const auto titlePaintContext = Ui::Text::PaintContext{
		...
		// PaintContext: position, outerWidth, availableWidth, ...
		st::boxTitlePosition, // position
		(st::boxWideWidth - hPadding), // outerWidth
		(st::boxWideWidth - hPadding), // availableWidth
	};
	...
	// XP walk: designated -> named-local (C7555); TopBarDescriptor sets
	// non-contiguous fields (logo@1, gradientStops@6 left default).
	auto topBarDescriptor = Ui::Premium::TopBarDescriptor();
	topBarDescriptor.clickContextOther = nullptr;
	topBarDescriptor.title = tr::lng_giveaway_new_title();
	topBarDescriptor.about = std::move(subtitleText);
	topBarDescriptor.light = true;
	topBarDescriptor.optimizeMinistars = false;
	const auto bar = Ui::CreateChild<Ui::Premium::TopBar>(
		box.get(),
		st::startGiveawayCover,
		std::move(topBarDescriptor));
		...
		button->setClickedCallback([=] {
			box->uiShow()->showBox(Box([=](not_null<Ui::GenericBox*> b) {
				Ui::ChooseDateTimeBox(b, {
					// XP walk: designated -> positional (C7555)
					tr::lng_giveaway_date_select(), // title
					tr::lng_settings_save(), // submit
					[=](TimeId time) {
						state->dateValue = time;
						b->closeBox();
					}, // done
					QDateTime::currentSecsSinceEpoch, // min
					state->dateValue.current(), // time
					[=] {
		...
		button->setClickedCallback([=] {
			...
			if (isSpecific) {
				...
				invoice.purpose = Payments::InvoicePremiumGiftCodeGiveaway{
					// XP walk: designated -> positional (C7555)
					peer->asChannel(), // boostPeer
					ranges::views::all(
						state->selectedToSubscribe
					) | ranges::views::transform([](
							const not_null<PeerData*> p) {
						return not_null{ p->asChannel() };
					}) | ranges::to_vector, // additionalChannels
					state->countriesValue.current(), // countries
					state->additionalPrize.current(), // additionalPrize
					state->dateValue.current(), // untilDate
					(membersGroup->current() /*XP walk: v4.15.1*/
						== GiveawayType::OnlyNewMembers), // onlyNewSubscribers
					state->showWinners.current(), // showWinners
				};
			}
			...
			if (prepaid) {
				...
				show->show(Ui::MakeConfirmBox({
					// XP walk: designated -> positional (C7555).
					// ConfirmBoxArgs: text, confirmed, cancelled.
					tr::lng_giveaway_start_sure(tr::now), // text
					startPrepaid, // confirmed
					cancel, // cancelled
				}));
			} else {
