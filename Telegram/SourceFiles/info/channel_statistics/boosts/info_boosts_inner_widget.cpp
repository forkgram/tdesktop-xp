...
void InnerWidget::fill() {
...

	{
		...
		Ui::FillBoostLimit(
			fakeShowed->events(),
			dividerContent.data(),
			// XP walk: designated -> positional (C7555). v4.11.6 wraps in rpl::single
			// (FillBoostLimit takes rpl::producer<BoostCounters>) and overview field
			// isBoosted -> mine (now int). Order: level,boosts,thisLevel,nextLevel,mine.
