...
std::unique_ptr<PeerListRow> AwardMembersListController::createRow(
	...
	auto type = Type{
		...
		{}, // rank
		{}, // removeText
		_chatStyle.get(), // chatStyle
		&_pillCircleCache, // circleCache
	};
...
void MyChannelsListController::rowClicked(not_null<PeerListRow*> row) {
	...
	if (checked && channel && channel->username().isEmpty()) {
		_show->showBox(Box(Ui::ConfirmBox, Ui::ConfirmBoxArgs{
			...
			tr::lng_giveaway_channels_confirm_about(), // text
			[=](Fn<void()> close) {
				delegate()->peerListSetRowChecked(row, checked);
				close();
			}, // confirmed
			{}, // cancelled
			tr::lng_filters_recommended_add(), // confirmText
			{}, // cancelText
			{}, // confirmStyle
			{}, // cancelStyle
			{}, // labelStyle
			{}, // labelFilter
			{}, // labelPadding
			tr::lng_giveaway_channels_confirm_title(), // title
		}));
	} else {
		delegate()->peerListSetRowChecked(row, checked);
	}
