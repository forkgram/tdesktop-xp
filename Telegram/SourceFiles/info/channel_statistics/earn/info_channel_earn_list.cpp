...
[[nodiscard]] TextWithEntities EmojiCurrency(
	...
	auto emoji = TextWithEntities{
		(QString(QChar(0xD83D)) + QChar(0xDC8E)), // XP walk: designated -> positional
	};
...
void InnerWidget::fill() {
		...
		label->setLink(1, std::make_shared<LambdaClickHandler>([=] {
			_show->showBox(Box([=](not_null<Ui::GenericBox*> box) {
				...
				const auto bigCurrencyIcon = emojiHelper.paletteDependent({
					[=] { // XP walk: designated init -> positional (C7555, PaletteDependentEmoji)
	...
	if (data.availableBalance) {
		...
		Api::HandleWithdrawalButton(
			{ _peer }, // XP walk: designated->positional (RewardReceiver.currencyReceiver@0); v5.14.2 channel->_peer
			button,
			_controller->uiShow());
...
void AddEmojiToMajor(
	...
	std::move(
		...
		auto icon = helper.paletteDependent({ [=] { // XP walk: designated init -> positional (C7555, PaletteDependentEmoji)
			return Ui::Earn::IconCurrencyColored(
				st.style.font,
				!isIn
				? st::currencyFg->c
