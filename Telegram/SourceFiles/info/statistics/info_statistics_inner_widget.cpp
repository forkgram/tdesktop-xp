...
void FillOverview(
	...
	const auto topLeftLabel = (isChannelStoryStats && isChannel)
		...
		: isMessage
		// XP walk: designated -> positional (C7555); took theirs (messageOrStory).
		? addPrimary({ float64(messageOrStory.views) })
	...
	const auto topRightLabel = (isChannelStoryStats && isChannel)
		...
		: isMessage
		// XP walk: designated -> positional (C7555); took theirs (messageOrStory).
		? addPrimary({ float64(messageOrStory.publicForwards) })
	...
	const auto bottomLeftLabel = (isChannelStoryStats && isChannel)
		...
		: isMessage
		// XP walk: designated -> positional (C7555); took theirs (reactions).
		? addPrimary({ float64(messageOrStory.reactions) })
		: addPrimary(supergroup.viewerCount);
	const auto bottomRightLabel = (isChannelStoryStats && isChannel)
		? addPrimary(!hasPostReactions
			? Value{ -1 } // XP walk: designated -> positional (C7555); StatisticalValue.value
			: channel.meanStoryReactionCount)
		: isChannel
		? addPrimary(channel.meanStoryViewCount)
		: isMessage
		// XP walk: designated -> positional (C7555); took theirs (privateForwards).
		? addPrimary({ float64(messageOrStory.privateForwards) })
	...
	_showFinished.events(
		...
		if (!_contextId && !_storyId) {
			...
			api->request([=](const Data::StoryStatistics &data) {
				// XP walk: designated -> positional (C7555). AnyStatistics:
				// channel, supergroup, message, story. Took theirs.
				_state.stats = Data::AnyStatistics{
					{}, // channel
					{}, // supergroup
					_contextId ? data : Data::StoryStatistics(), // message
					_storyId ? data : Data::StoryStatistics(), // story
				};
				if (_contextId || _storyId) {
...
void InnerWidget::fillRecentPosts(not_null<Ui::VerticalLayout*> container) {
	...
	const auto addMessage = [=](
		...
		const auto fullRecentId = Data::RecentPostId{
			...
			maybeItem ? maybeItem->fullId() : FullMsgId(), // messageId
			maybeStory ? maybeStory->fullId() : FullStoryId(), // storyId
		};
		...
		button->setClickedCallback([=] {
			// XP walk: designated -> positional (C7555). ShowRequest: info,
			// history, messageStatistic, storyStatistic. Took theirs.
			_showRequests.fire({
				{}, // info
				{}, // history
				fullRecentId.messageId, // messageStatistic
				fullRecentId.storyId, // storyStatistic
			});
		});
