...
base::unique_qptr<Ui::PopupMenu> PublicForwardsController::rowContextMenu(
	...
	menu->addAction(text, crl::guard(parent, [=, peerId = peer->id] {
		...
		_requestShow({ { peerId, MsgId() } }); // messageId
...
void CreditsRow::rightActionPaint(
	...
	_rightMinorText.draw(p, Ui::Text::PaintContext{ // XP walk: designated -> positional
		QPoint(xMinor, y + st::creditsHistoryRowRightMinorTop), // position
		outerWidth, // outerWidth
		outerWidth, // availableWidth
	});
	_rightText.draw(p, Ui::Text::PaintContext{ // XP walk: designated -> positional
		QPoint(
			xMinor - _rightText.maxWidth(),
			y + st::creditsHistoryRowRightTop), // position
		outerWidth, // outerWidth
		outerWidth, // availableWidth
	});
}
...
void CreditsRow::paintStatusText(
	...
	// XP walk: designated -> named-local (C7555; PaintContext elisionLines
	// non-contiguous). Took theirs' descriptionSkip positioning.
	auto context = Ui::Text::PaintContext();
	context.position = QPoint(x, y - st::creditsHistoryRowDescriptionSkip);
	context.outerWidth = outer;
	context.availableWidth = available;
	context.elisionLines = 1;
	_description.draw(p, context);
}
...
, _context([&]() -> Ui::Text::MarkedContext {
	...
	return { {}, std::move(customEmojiFactory) }; // XP walk: designated -> positional (customEmojiFactory@1)
}()) {
...
void CreditsController::applySlice(const Data::CreditsStatusSlice &slice) {
	...
	auto create = [&](
		...
		// session, entry, subscription, context, rowHeight, updateCallback.
		const auto descriptor = CreditsRow::Descriptor{
			...
			((!s || !s.title.isEmpty())
				? computeListSt().item
				: st::boostsListBox.item).height, // rowHeight
			[=](not_null<PeerListRow*> row) {
				delegate()->peerListUpdateRow(row);
			},
		};
