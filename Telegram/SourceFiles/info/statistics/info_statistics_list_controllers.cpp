...
void CreditsRow::rightActionPaint(
	...
	// XP walk: take theirs' positioning; designated -> positional (C7555).
	// Ui::Text::PaintContext: position, outerWidth, availableWidth.
...
void CreditsRow::paintStatusText(
	...
	// XP walk: designated -> named-local (C7555; Ui::Text::PaintContext, with
	// non-contiguous elisionLines): position, outerWidth, availableWidth,
	// elisionLines.
	auto context = Ui::Text::PaintContext();
	context.position = QPoint(x, y - _description.minHeight());
...
, _firstSlice(std::move(d.firstSlice))
, _context(Core::TextContext({ _session })) { // XP walk: designated -> positional
