		...
		if (already != end(_views)) {
			...
			if (unused != end(_views)) {
				views.push_back(base::take(*unused));
					...
					// XP walk: designated -> positional (C7555); View::button@0.
					views.push_back({ createGiftButton() });
			}
		}
...
void InnerWidget::refreshCollectionsTabs() {
	...
	if (!_collectionsTabs) {
		...
		_collectionsTabs = std::make_unique<Ui::SubTabs>(
			...
			Ui::SubTabs::Options{ selected, true },
			std::move(tabs),
			context);
...
, _descriptor(Descriptor{
	{}, // XP walk: designated -> positional (C7555); filter@0 default, collectionId@1
	controller->giftsCollectionId(),
}) {
