...
rpl::producer<CreditsAmount> AddCurrencyAction(
	...
	const auto icon = state->helper.paletteDependent({ [=] { // XP walk: designated init -> positional (C7555, PaletteDependentEmoji)
...
Section DetailsFiller::makeCommunityLink(not_null<PeerData*> peer) {
	...
	public:
		...
		base::unique_qptr<Ui::PopupMenu> rowContextMenu(
			...
			// submenuSt6, hideRequests7, addTopShift8, isSeparator9,
			// isAttention10.
			Ui::Menu::CreateAddActionCallback(result.get())({
				tr::lng_filters_menu_add(tr::now), // text
				nullptr, // handler
				&st::menuIconAddToFolder, // icon
				nullptr, // separatorSt
				[&](not_null<Ui::PopupMenu*> submenu) { // fillSubmenu
					FillChooseFilterMenu(_window, submenu, history);
				},
				{}, // make
				&st::foldersMenu, // submenuSt
			});
			return result;
		}
