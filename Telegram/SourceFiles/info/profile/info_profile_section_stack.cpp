...
void SectionStack::add(Section section) {
	...
	_rows.push_back({
		RowType::Block, // type
		std::move(section.widget), // widget
		std::move(section.shown), // shown
	});
}
...
void SectionStack::addPlainSeparator() {
	...
	_rows.push_back({ RowType::PlainSeparator }); // type
}
...
void SectionStack::addTextSeparator(
	...
	_rows.push_back({
		RowType::TextSeparator, // type
		nullptr, // widget
		std::move(shown), // shown
		std::move(text), // text
		std::move(setup), // textSetup
	});
}
