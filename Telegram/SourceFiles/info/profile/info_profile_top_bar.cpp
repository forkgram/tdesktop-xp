...
[[nodiscard]] PatternColors CalculatePatternColors(
	...
	if (collectible && collectible->patternColor.isValid()) {
		...
		auto result = !edgeColor
			...
			: collectible->patternColor;
		return {
			// XP walk: take theirs (result); designated -> positional (C7555;
			// patternColor@0, useOverlayBlend@1).
			std::move(result),
...
void TopBar::paintUserpic(QPainter &p, const QRect &geometry) {
	...
	if (_uploadOverlay && _uploadOverlay->shown()) {
		_uploadOverlay->paint(p, geometry, {
			st::defaultUserpicButton.uploadProgressLine, // lineWidth
			st::defaultUserpicButton.uploadProgressMargin, // margin
			st::historyFileThumbRadialFg, // progressFg
			st::songCoverOverlayFg, // overlayFg
			&st::userpicUploadCancel, // cancelIcon
		});
	}
}
...
void TopBar::paintAnimatedPattern(
	...
	if (_basePatternImage.isNull()) {
		...
		// XP walk: take theirs (resetTransform); designated -> positional
		// (C7555; textColor@0, required<QColor>).
...
void TopBar::updateStoryOutline(std::optional<QColor> edgeColor) {
	...
	if (_hasLiveStories) {
		...
		for (const auto &storyIdDates : source->ids) {
			...
			_storySegments.push_back({
				isUnread ? unreadBrush : readBrush, // brush
				!isUnread ? widthSmall : widthBig, // width
			});
		}
	}
}
...
TopBarActionButtonStyle TopBar::mapActionStyle(
	...
	if (c) {
		...
		return TopBarActionButtonStyle{
			...
			Window::Theme::IsNightMode()
				? std::nullopt
				: std::make_optional(st::windowShadowFgFallback->c), // shadowColor
		};
	}
}
