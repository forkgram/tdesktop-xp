...
void PickUntilBox(not_null<Ui::GenericBox*> box, Fn<void(TimeId)> callback) {
	...
	Ui::ConfirmBox(box, { {}, [=] {
			...
			box->closeBox();
		}, {}, tr::lng_emoji_status_for_submit(), tr::lng_cancel() });
}
...
void EmojiStatusPanel::show(Descriptor &&descriptor) {
	...
	if (descriptor.backgroundEmojiMode) {
		...
		// XP: range-v3 0.12 can't iterate views::concat here; use two plain loops.
...
void EmojiStatusPanel::create(const Descriptor &descriptor) {
	...
	_panel = base::make_unique_q<ChatHelpers::TabbedPanel>(
		...
		object_ptr<Selector>(
			...
			Descriptor{
				...
				// show, st, level, mode, customTextColor, features. v4.13.0 added
				// channelStatusMode handling to st and mode (Mode::ChannelStatus).
	...
	auto emojiChosen = _panel->selector()->emojiChosen(
		...
		return Chosen{ {}, {}, data.messageSendingFrom };
	});
